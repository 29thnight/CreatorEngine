...
`ce`에 **컨테이너 타입은 들어가지 않는다**.

**예외 — 공용 인프라의 RAII 손잡이(2026-09-20 확정).** `ce::profile_scope` 는
타입이지만 `ce::` 안에 둔다. 이유는 그것이 컨테이너가 아니라 **자유 함수를 쓸 수
없는 자리의 자유 함수**이기 때문이다 — 구간을 여는 일과 닫는 일은 한 문장이어야
하고(짝 불균형을 문법적으로 불가능하게 만드는 것이 도입 이유다) 그것을 표현하는
C++ 수단은 소멸자뿐이다. `ce::marker<"...">()` 는 같은 묶음의 자유 함수라 원래
규칙 그대로다. 판정은 §2 의 ④⑥ — `std` 기제(RAII·범위)에 물리는 타입 없는 도구.
이 예외를 컨테이너나 상태를 지닌 객체로 넓히지 않는다.
