...
### ④ 카메라 등록 정리 — 구현 완료 2026-08-09

- **`GetCamera(1)` 공유 폐지.** 모든 `CameraComponent`가 슬롯 1을 잡던
  하드코딩(비면 만들고, 차 있으면 그 카메라를 공유)을 걷고, 컴포넌트마다
  자기 카메라를 만들어 등록한다. 게임 카메라 둘이 한 Camera 객체를 번갈아
  덮어쓰던 다중 카메라 원천 불가가 여기서 풀렸다. PIX 검증 v1에서 이
  하드코딩이 실제로 혼선을 만든 것을 실측했다.
- **고정 10슬롯 폐지.** `AddCamera`가 빈 슬롯이 없으면 늘린다. 등록 실패는
  "카메라가 있는데 화면이 없다"로만 드러나는 부류다 — 상한이 필요한 곳은
  표시 뷰(kMaxLiveCameraViews)이지 등록이 아니다.
- **세대 ID로 ABA를 닫았다.** 등록마다 단조 증가 세대를 발급하고
  (`Camera::m_generation`), 뷰 배정이 포인터+세대로 매칭한다
  (MultiCameraRenderPlan §14 후속 과제 완료).
- **뷰 상한 초과를 관측 가능하게.** 초과로 건너뛴 수를 세서 status에 낸다.
- `GetLastCamera()`("마지막 등록 활성")는 유지 — 명시 지정(메인 카메라
  표시)은 다중 게임 카메라 씬이 실제로 나올 때 함께 정한다.

### ④-b 기즈모 뷰 분리 · 아이콘 알파 — 2026-08-09

**기즈모가 게임 뷰에 그려지고 있었다.** 노드 목록이 뷰 공용이라 그리드·
와이어프레임·아이콘·라인이 두 뷰 모두에서 섰다. `LiveFrameBinding`에
`isEditorView`(뷰 배정이 회전·교체가 있어 인덱스로는 정체를 모른다 —
카메라 포인터로 판정)를 실어 네 노드의 declare가 에디터 씬 뷰에서만
서게 했다. 건너뛰면 LDR 슬롯이 그대로 남아 게임 뷰는 포스트 체인 결과를
표시한다 — modifies 규약이 그대로 성립한다.

**아이콘의 투명 영역이 에디터 배경색 사각형으로 보였다.** 원인은 텍스처도
블렌드 RGB도 아니고 **대상 알파 채널 오염**이었다: 공용 블렌드 상태가
`SrcBlendAlpha=ONE·DestBlendAlpha=ZERO`라 반투명 패스가 src.a를 대상
알파에 그대로 써 넣었고, 그 대상이 공유 텍스처로 복사된 뒤 ImGui가 알파
블렌드로 그리면서 뷰포트에 구멍이 뚫렸다(픽셀 실측 (56,56,56) = ImGui 패널
배경). `ZERO·ONE`(대상 알파 보존)으로 바꿨다 — 화면에 남는 알파는 "덮어 쓴
비율"이 아니라 "이 텍스처를 표시할 때의 불투명도"라 1로 남아야 한다.
투명 큐·UI 등 blendEnable 사용자 전부에 같은 이유로 적용된다.

검증: 창 캡처로 씬 뷰(그리드+깨끗한 아이콘)와 게임 뷰(오버레이 없음)를
확인. `dx12.selftest`의 아이콘 검증(1x1 흰색 R≈0.5)은 RGB 블렌드만 보므로
이 알파 규약 변경과 무관하게 성립한다.
