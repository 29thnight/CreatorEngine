...
### 2.3 저작 코퍼스 (2026-09-04)

| 포맷 | 개수 | 비고 |
|---|---|---|
| PNG | 583 (`Dynamic_CPP` 510) | **전부 8비트**. RGBA 345 · RGB 237 · 팔레트 1 |
...
| 역할 | 라이브러리 | 시점 | 근거 |
|---|---|---|---|
| PNG·JPG·BMP·TGA·HDR 디코드 | **stb_image** | T1a | 이미 vcpkg 의존. WIC 이 비-Windows 에서 PNG 를 막는다 |
...
★ **이 구성이면 DirectXTex 가 0이 된다.** 남는 외부 의존은 stb 둘(이미 있음) +
bc7enc 하나이고, 셋 다 헤더·소스 몇 개라 이식이 자명하다.
...
| T1a | **583장 A/B 바이트 대조** — DirectXTex 와 stb_image 로 각각 디코드해 픽셀 일치 | 한쪽 채널 순서를 뒤집으면 빨개지는가 |
...
1. **`bc7enc_rdo` 의 vcpkg 포트 유무.** 없으면 벤더링이고, 그러면 T1a 에 빌드 배관이
   붙는다. 확인하지 않았다
2. **stb_image 의 HDR 채널 수.** `stbi_loadf` 가 float 3ch 인지 4ch 인지에 따라
   RGBA32F 로 맞추는 비용이 갈린다. 19장 x 128MB 라 무시할 수 없다
3. **583장 A/B 대조의 실제 결과.** stb 와 WIC 가 같은 PNG 를 같은 바이트로 디코드할
   것으로 보지만 재지 않았다. 감마·색공간 청크 처리가 다를 수 있고, **다르면 T1a 의
   전제가 무너진다.** 착수 첫 행동이 이 측정이어야 한다
