...
★★ **오늘 서 있는 capability 는 `ExecutesUserCode` 하나다(LC7).** 나머지 열둘은 쓰이는
자리가 아직 없고, 212 개에 지금 적으면 관측이 아니라 지어내기가 된다 — LC3 이 인자
스키마를 비워 둔 것과 같은 판단이다. `ExecutesUserCode` 만 먼저 선 이유는 LC7 이 L3 등급
B 를 열면서 그 경계가 **실제로 존재하게** 됐기 때문이다. descriptor 의
`executesUserCode` 와 discovery 의 `user_code` 열이 그것이고, 나머지는 자기 슬라이스가
올 때 그 슬라이스를 아는 사람이 세운다.
...
### 9.1 전수 표기 완료 (2026-09-04 LC6)

212 개 전부에 §9 분류와 **라이브 축**이 붙었다. `DescriptorSeed` 의 필수 필드라
빼먹으면 컴파일이 거부한다 — LC3 이 요약을 선택으로 뒀다가 205 개 중 78 개가 빈 채로
지나간 것과 같은 실수를 반복하지 않기 위해서다.

| 분류 | 수 | 근거 |
|---|---|---|
| `probe` | 126 | RenderTest·Diagnostics TU 소속 + `experiment.*` + 검사 접미사 |
| `engine_service` | 66 | 나머지 · 라이브 조회/조정 · 읽기 전용 조회 |
| `editor_operation` | 14 | SceneObject 의 저작 명령(§9 예시와 일치: play·duplicate·undo·object.*) |
| `raw_fixture` | 6 | `*.authoring.probe` — Undo 를 의도적으로 우회한다 |

★ **분류를 이름이 아니라 "어느 도메인 TU 에 등록돼 있는가"에서 뽑았다.** 그 사실은
LC6 의 이동으로 확보된 것이라 추측이 아니다. 규칙마다 판단 사유를 출력해 감사할 수
있게 했고, 검토에서 세 갈래가 고쳐졌다.

- 람다로 등록된 21 개가 검토 없이 기본값으로 흘러가고 있었다. 전부 계측·검증 표면이다.
- `component.list`·`play.state`·`undo.state`·`prefab.status`·`prefab.overrides` 는
  이름이 mutating 처럼 보이지만 **읽기 전용**이다. 접두사로만 정하면 잘못 앉는다.
...
가 프로브마다 에디터를 새로 띄우고 있어서 그렇게 읽었다. 켜져 있는 에디터에 HTTP 로
64 개를 붙여 보니 **61 개가 그대로 돌았다.** 하네스가 프로세스를 새로 띄우는 것은
격리 관행이지 재시작 요구가 아니었다. 관찰(하네스가 이렇게 부른다)에서 성질(라이브가
안 된다)을 추론한 것이 틀렸다.

★ **측정 범위를 적어 둔다.** 실제로 붙여 본 것은 render 64 개뿐이다. 나머지 148 개의
`live` 는 **그 상태에서의 기본값이지 실측이 아니다.** 나중 측정이 이를 뒤집으면
`verify-cli-registry-golden.ps1` 이 의도적 변경으로 잡는다.
...
★ **완료(2026-09-05).** 왕복은 약 **2.9 초**이고 그중 86% 가 빌드다 — 실측표와 남긴
빈칸은 §13 의 LC7 절에 있다. `script.invoke` 는 서비스에서 `--allow-user-code` 없이는
403 이고(§8), 표식 없는 메서드는 `script.invoke_not_callable` 로 거부된다.
...
| 완료 기준 | 증거 |
|---|---|
| facade/Pump wiring 중심 | `ConsoleCommandSystem.cpp` **12,820 → 4,068 줄**(-68%). 핸들러 190 개가 `Commands/` 아래 7 개 TU로 |
| domain 하나당 빌드 | 일곱 도메인을 한 파일씩 옮기고 매번 빌드·골든 확인. **골든 일곱 번 다 불변** |
| GUI/서비스 의미 차이 미분류 0 | §9 분류 212 개 전수 + 라이브 축. `play` 의 `m_isGameMode` 결함은 고쳤고, Undo 차이는 래칫으로 분류(§9.2) |
| Player 링크 변화 0 | `Player.vcxproj` 빌드 통과. 참조는 `HostImGuiPresentation` 뿐으로 그대로다 |

★ **서명 이행은 36%(190 중 68)에서 멈춘다.** RenderTest 56 개와 Core 8 개,
`undo.state`·`play.state` 를 옮겼다. 나머지는 도메인 단위로 이어가되, 그것은 각
명령의 결과 의미를 정하는 일이라 파일 분리와 같은 슬라이스에 넣지 않는다(§12.3).
`result_bearing` 열이 골든에 있어 진척이 값으로 보인다.

★ **인자 스키마는 여전히 비워 둔다.** LC3 이 "domain 을 아는 사람이 LC6 에서
채운다"고 미뤘지만, 190 개의 인자 타입을 한 번에 적는 것은 이 슬라이스에서도
지어내기가 된다. 도메인 파일이 갈린 지금은 파일 단위로 나눠 할 수 있게 됐다는
것이 LC6 이 만든 조건이다.
...
| 완료 기준 | 증거 |
|---|---|
| 에디터 재시작 0회로 C# 변경 반영 | `verify-cli-script-invoke.ps1` 이 프로브의 `Ping()` 을 **실행마다 다른 토큰**으로 바꿔 빌드·리로드하고 그 토큰을 받는다. 상수를 두고 "리로드 성공" 만 보면 리로드가 아무 일도 안 해도 초록이라, 반환값이 달라지는 것으로 판정한다 |
| 표식 없는 메서드 호출 0 | 프로브에 **표식만 없는** `Unmarked()` 를 두고 부른다. 실패 여부가 아니라 `script.invoke_not_callable` 코드로 판정한다. 표식을 붙였다 떼는 A/B 로 표식이 판정 주체임을 확인했다 — 붙이면 값이 나오고, 떼면 거부된다 |
| 리로드 실패가 반쯤 교체된 상태를 안 남김 | 앞 슬라이스(`ScriptAssemblyLoader.Validate`)와 `verify-cli-script-reload.ps1` |
| `ExecutesUserCode` 없는 경로로 B 도달 불가 | 규약이 아니라 **문**으로 세웠다 — 아래 ★ |

★ **"도달 불가" 를 규약으로 두지 않았다.** 처음 설계는 "이 capability 를 가진
핸들러에서만 `ClrHost::InvokeCallableStatic` 을 부르기로 한다" 였다. 그러면 지키는지
확인할 방법이 소스를 눈으로 훑는 것뿐이고, 두 번째 호출자가 생기는 날 아무도 모른 채
경계가 사라진다. 대신 `ExecuteParsed` 가 descriptor 의 `executesUserCode` 가 참일 때만
`ClrHost::UserCodeScope` 를 열고, 그 창 밖의 호출은 `NotPermitted` 로 **거부**한다.
빠뜨림의 방향이 뒤집힌다 — seed 표시를 잊은 명령은 조용히 열리는 것이 아니라 소리 내어
실패한다.

★★ **capability 는 열셋 중 하나만 세웠다.** §6.2 는 `RequiresDX12` 등 열셋짜리
비트마스크를 그리지만, 나머지 열둘을 지금 212 개에 적는 것은 관측이 아니라 지어내기가
된다(LC3 이 인자 스키마를 비워 둔 것과 같은 판단이다). `ExecutesUserCode` 는 다르다 —
오늘 그 경계가 실제로 존재하고, 그것이 서는 자리가 이 슬라이스다. golden 에 `user_code`
열이 생겼고 `yes` 는 **하나**다. 둘이 되는 날 그 diff 가 말한다.

★★★ **서비스는 별도 플래그 없이 이 명령을 받지 않는다(§8).** `--allow-user-code` 가
없으면 403 이다. `--command-service` 안에는 씬을 열고 자산을 저작하는 것까지 들어 있지만
그것들은 전부 **엔진이 쓴 코드**가 하는 일이고, `script.invoke` 는 엔진이 쓰지 않은
코드를 부른다. 두 결정을 한 플래그에 묶으면 서비스를 켜는 모든 실행이 뒤엣것에 동의한
셈이 된다. 배치·stdin 경로는 이 플래그를 보지 않는다 — §8 이 통제를 요구하는 것은
프로세스 경계 **밖**에서 오는 호출이고, 그것이 서비스다.

#### 왕복 실측 (§10.2 의 "그 왕복 시간이 실측으로 기록된다")

x64-Debug · 에디터를 **끄지 않은 채** `POST /command` 로 왕복. 3 회 측정의 대표값:

| 단계 | 시간 |
|---|---|
| 소스 수정 | 6–16 ms |
| `dotnet build GameScripts` | **2,427–2,729 ms** |
| `script.reload` | 388–421 ms |
| `script.invoke` | 7–9 ms |
| **왕복 전체** | **2,830–3,174 ms** |

★ **왕복의 86% 가 빌드다.** 착수 전 예상은 리로드(어셈블리 언로드·재적재·인스턴스
복원)가 가장 비쌀 것이었는데, 실측은 리로드 388ms 대 빌드 2.4 초였다. 그러니 이 왕복을
줄이려는 다음 사람이 손댈 곳은 리로드 경로가 아니라 빌드다. 호출 자체는 8ms — 리플렉션
조회를 캐시하지 않는 선택(캐시하면 collectible ALC 가 붙들려 언로드가 막힌다)의 비용은
왕복에서 보이지 않는다.

#### 이 슬라이스에서 하지 않은 것

- **호출 가능 목록 조회를 만들지 않았다.** "어떤 메서드를 부를 수 있나" 를 물으려면
  지금은 소스를 읽어야 한다. §10.2 가 요구하지 않았고, 거부가 안전한 방향이라 미뤘다.
  필요가 실측되면 그때 `script.callable` 을 연다.
- **인자 타입은 여섯이다** — `string`·`bool`·`int`·`long`·`float`·`double`. 그 밖의
  파라미터를 받는 메서드는 표식이 있어도 인자 변환에서 거부된다. 지금 넓히면 쓰지 않는
  변환 규칙만 는다.
- **`UserCodeScope` 의 거부 방향에는 런타임 시험이 없다.** 창 밖에서 부르는 호출자가
  오늘 존재하지 않아 태울 시나리오가 없다. 그 문은 미래의 두 번째 호출자가 조용히
  통과하지 못하게 하려고 있는 것이지, 오늘의 무언가를 막고 있는 것이 아니다.
...
**상태: 진행 중 (2026-09-05). 전송 계층은 섰고, 이관은 1/15 다.**

| 완료 기준 | 오늘 |
|---|---|
| 배치 JSONL 과 서비스 JSON 이 schema v1 공유 | **닫혔다.** `--result-format jsonl` · `--result-file`. 봉투를 두 곳에서 조립하지 않고 `CommandService::BuildResultEnvelope` 하나가 만든다 |
| 소비자의 한국어 verdict regex 0 | **1/15.** `Invoke-Dx12Suite.ps1` 이관 완료. 남은 14 개는 `cli_consumer.ratchet.json` 이 눈금으로 잡는다 |
| 무인증 경로 0 | **닫혔다.** `verify-cli-consumer-contract.ps1` 이 토큰 검사와 라우트 분기의 **순서**를 본다 |
| direct exit write 1 곳 | **유지.** 상한 16 그대로 — 아래 ★★ |
| void command handler 0 · terminal result 없는 command 0 | **140/212 미이행.** 아래 ▲ |

★ **`Invoke-Dx12Suite.ps1` 이 세 번째로 틀렸던 자리가 닫혔다.**

  그 파일 머리말은 같은 자리에서 난 오독 셋을 적어 두고 있다 — ① help 를 읽어
  35 중 26 만 실행 ② 등록 형태가 바뀌어 35 를 **0** 으로 읽음 ③ 판정 어휘를
  통과/실패 둘로만 읽어 계측 검사(`완료`)를 실패로 셈. ①②는 LC3 이 `commands.list`
  로 닫았고, ③이 남아 있었다.

  실측으로 대조했다 — `dx12.encoderbench`(계측 프로브):

  | | 판정 | 근거 |
  |---|---|---|
  | 구 파서 | `완료` | `[CLI] dx12.encoderbench 완료` 문자열 |
  | 신 JSONL | `통과` | `status=succeeded code=ok` |

  어휘가 셋이라는 사실도, 그 셋이 언제 바뀌는지도 스크립트는 알 방법이 없었다.
  이제 값이 `CommandStatus` 라 맞출 어휘가 없다.

  ★ **기존 `verdicts.csv` 기준선은 무효다.** 판정 이름(`완료` → `통과`)과 열 구성
    (`Line` → `Status`·`Code`)이 함께 바뀌었다. `-Baseline` 대조 축도 사람용
    문자열에서 계약 값으로 옮겼다 — 문안을 다듬는 것만으로 기준선이 붉어지던
    자리였다. 다음 실행에서 기준선을 다시 뜬다.

★★ **exit spine 게이트가 이 슬라이스의 코드를 먼저 잡았다.**

  `--result-format` 의 잘못된 값과 `--result-file` 열기 실패에 `SetExitCode` 를
  직접 썼더니 상한이 16 → 18 로 올랐다. 눈금을 올리는 대신 두 자리를 session 으로
  돌렸다 — 인자 오류는 명령이 아니라 낼 `CommandResult` 가 없지만, 그렇다고 exit
  code 를 직접 쓰면 "쓰는 곳은 session 하나" 가 깨지고 뒤의 성공이 앞의 실패를
  지우던 옛 구조로 한 걸음 돌아간다. 실측으로 확인했다 — 잘못된 형식은 `2`,
  열 수 없는 결과 파일은 `5`(§5.4).

★★★ **정적 게이트가 처음에 아무것도 안 보고 통과했다.**

  무인증 경로 검사가 라우트를 `request.path ==` 로만 찾았는데 실제 코드는 Yoda
  비교(`"/health" == request.path`)다. 그래서 **라우트를 하나도 못 찾은 채** 초록이었다 —
  무인증 경로가 있어도 통과했을 것이다. 두 어순을 다 찾게 고치고, **하나도 못
...
일곱 전부를 결과형으로 옮겼다(`create` · `rename` · `duplicate` · `parent` ·
`rootref` · `transform` · `property`). 조기 반환 29 개를 셋으로 갈랐다:

  사용법 · 알 수 없는 타입      → `InvalidArguments` (2)
  활성 씬 없음                  → `PreconditionFailed` (3)
  오브젝트/컴포넌트 없음 · 자기 자신을 부모로 · 순환 · 생성/복제/설정 실패 → `Fail` (4)

"자기 자신을 부모로" 와 "순환" 을 `InvalidArguments` 가 아니라 `Fail` 로 낸 것은,
그 판정이 인자만으로 나오지 않고 **씬의 계층을 봐야** 나오기 때문이다. 문법은
멀쩡한 요청이고 거절된 것이다.

★ **실측으로 이행 전후를 대조했다.** 같은 스크립트를 이행 전 바이너리(Release)와
  이행 후(Debug)에 각각 태웠다:

  | | stdout | exit |
  |---|---|---:|
  | 이행 전 | `오브젝트를 찾을 수 없음` · `자기 자신을 부모로 삼을 수 없음` | **0** |
  | 이행 후 | **같은 두 줄** + 판정 줄 둘 | **4** |

  두 stdout 의 diff 가 **덧붙은 판정 줄 둘뿐**이다 — 지워지거나 고쳐진 줄이 0.
  핸들러가 찍는 줄은 한 글자도 바뀌지 않았고, `PublishResult` 가
  `[CLI] object.rename failed (object.rename.not_found) 오브젝트가 없다` 를 더한다.

★★ **Undo 특성화는 그대로다.** `verify-cli-editor-operation.ps1` 이 재는 11 개의
  판정이 래칫과 일치하고(생성만 남기고 나머지는 안 남김), 무의미성 방지도
  통과한다(오브젝트 4 → 7). 서명 이행이 Undo 계약을 건드리지 않았다는 뜻이다.
...
22 개 전부를 결과형으로 옮겼다. 15 개는 아직 facade(`ConsoleCommandSystem.cpp`)에
있고 7 개는 `SceneObjectCommands.cpp` 에 있다 — **옮기지 않았다.** 파일 분리와 서명
이행을 같은 커밋에 넣지 않는다(§12.3).

★ **이 이행이 산 것을 실측으로 보였다.** 같은 시나리오를 이행 전 바이너리와
  이행 후 바이너리에 각각 태웠다:

  | | stdout | exit |
  |---|---|---:|
  | 이행 전(2026-09-04 Release) | `[scene.transformbulk] probe=FAIL skeleton=0` | **0** |
  | 이행 후 | 같은 줄 | **3** |

  probe 가 스스로 `FAIL` 을 찍고도 프로세스가 0 으로 끝나고 있었다. 세어 놓고
  판정하지 않은 상태였고, 자동화는 그것을 성공으로 읽었다.

★★ **핸들러가 찍는 줄은 한 글자도 바꾸지 않았다.** 이 도메인의 probe 출력은
  `verify-transform-*.ps1`·`verify-render-proxy-dirty.ps1` 여섯이 정규식으로 읽는다.
  다만 stdout 이 통째로 불변인 것은 아니다 — 실패·precondition 결과에는
  `PublishResult` 가 `[CLI] <명령> <status> (<code>) <메시지>` 를 **덧붙인다**(이행
  전에는 전부 `LegacyUnreported` 라 그 줄이 없었다). 덧붙는 줄이지 고쳐 쓰는 줄이
  아니라 기존 정규식은 그대로 맞고, 여섯 게이트의 판정이 이행 전후로 같음을 확인했다.

★★★ **하마터면 낡은 바이너리의 초록을 받을 뻔했다.** `verify-transform-*.ps1` 넷은
  `-Exe` 기본값이 **Release** 경로다. 인자 없이 돌렸더니 9 월 4 일자 Release
  바이너리 — 이 변경이 없는 것 — 로 검증되어 전부 초록이었고, bench 수치도
  135,657us 가 아니라 3,986us 였다. Debug 바이너리를 명시해 다시 돌려야 이 이행이
  검증된다. (LC8 이 같은 함정을 `build.ps1` 쪽에서 만났다 — 구성 표식을 넣은 이유다.)

★ **판정을 새로 세운 곳이 하나 있다: `scene.hierarchycheck`.** 쌍불일치·고아·
  순회미도달·Store불일치를 세기만 하고 판정하지 않고 있었는데, 그 넷은 "순회가
  서브트리를 통째로 빠뜨리는" 상태이고 그것이 뼈 61 개를 순회 밖에 두었던 결함이다.
  이제 합이 0 이 아니면 실패다. 최상위(-1표기)는 **판정에 넣지 않았다** — 같은 뜻의
  표기가 둘이라는 이미 알려진 상태이고(트랙 E), 순회를 깨뜨리지는 않는다.

★ 조회는 실패가 아니다. 토글 넷(`dirtytraversal`·`bonecache`·`transformstats`·
  `transformwritestats`)을 인자 없이 부르는 것은 **현재값 조회**이고, `bonedump` 의
  "도달한 뼈 0" 은 그 진단이 답해야 할 상태다. 이것들을 실패로 내면 상태를 물어본
  실행이 붉어진다.

  한 슬라이스에서 140 개를 바꾸는 patch 는 검토할 수 없고, 그 안에 섞인 거동 변경
  하나를 아무도 못 본다 — §15 의 위험 표와 §12.3 이 같은 이유로 금지한 종류의
  일괄 변경이다. 도메인 단위로 이어간다.

  ★ 그때까지 **`legacy_unreported` 를 성공으로 접지 않는다.** 이관된 소비자
    (`Invoke-Dx12Suite`)가 그 상태를 `무판정(legacy)` 으로 따로 센다. 통과로
    접으면 이행이 얼마나 남았는지가 집계에서 사라지고, 그러면 §18 의 이 줄은
    닫혔는지 아닌지를 아무도 모르게 된다.
