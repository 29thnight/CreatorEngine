...
| **C** 시스템 스케줄 | ✅ 완결 | C1 · C2(0·1·2) · C3(1~4차) · C4 | 별건 1: 6단계 트레이스 라벨 |
...
1. **6단계 트레이스 라벨 규약** — 아래 C3 4차의 별건. 작고, **L3의 자를 바로잡는
   일이라 순서상 L3보다 먼저다.** 지금 기준선은 "무엇이 실제로 불렸는가"를
   정확히 말하지 못한다.
2. **E5-R3** — `UIManager::CurCanvas`/`SelectUI`. R2와 같은 논거이나 "DDOL
   오브젝트가 선택된 채 씬을 건너는" 시나리오 확인이 선행.
3. **P4-b · P4-c** — 둘 다 골든 재기준선을 동반한다.
4. **L3** — 큰 트랙. 컴포넌트당/스크립트 묶음당 독립 커밋.
5. **E7-c** — `UISystemRedesignPlan` U7이 선행인데 **그 문서 자체가 개정 대상**이다
...
| 게이트 | 현재 값 |
|---|---|
| 생명주기 순서 | **93 사건** (C3 4차에서 92→93 재기준선) |
