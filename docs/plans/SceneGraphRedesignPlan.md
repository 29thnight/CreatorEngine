...
| **E** Entity 정체성 | 🔶 잔여 소수 | E1~E4 · E5-0 · E5-R2 · **E6** · E7-a · E7-b | E5-R3 · E7-c · (차단) E5 분류 B/C · (별도 트랙) E5-d |
...
1. **E5-R3** — `UIManager::CurCanvas`/`SelectUI`. R2와 같은 논거이나 "DDOL
   오브젝트가 선택된 채 씬을 건너는" 시나리오 확인이 선행.
2. **P4-b · P4-c** — 둘 다 골든 재기준선을 동반한다.
3. **L3** — 큰 트랙. 컴포넌트당/스크립트 묶음당 독립 커밋. **C5가 자를 먼저
   세워 뒀다** — 6단계 축 전부가 기준선에 기록되므로 이관이 훅을 빠뜨리면 보인다.
4. **E7-c** — `UISystemRedesignPlan` U7이 선행인데 **그 문서 자체가 개정 대상**이다
