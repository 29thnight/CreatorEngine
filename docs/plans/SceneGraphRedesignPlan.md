...
## 0.05 전제 — 레거시 호환성 없음 (사용자 지시, 재설계 착수 이래 상시)

**기존 저작 자산은 전부 폐기 대상이다** — 프리팹 208개 · 씬 12개. 앞으로의 테스트
자산은 CLI(`Academy_4Q.exe --script`)로 저작한다.

✅ **2026-08-21 실행 완료.** 추적 자산 **215개**를 지웠다(프리팹 206 · FT 씬 9 —
94,288줄). 실측으로 드러난 것 셋:

- **추적 프리팹 206개는 저장소 안에서 이미 고아였다.** 추적 씬 9개가 가진
  `m_prefabFileGuid` 92건이 **전부 널 GUID**다 — 프리팹을 참조하는 추적 씬이 하나도
  없었다. 문서가 센 "씬 12"는 추적 9(`FT_*`) + **미추적 3**(`Test1`·`Test2`·
  `UITestScene` — `.gitignore`가 무시)이었다. 미추적분은 git에 없으므로 폐기의
  대상이 아니다(다른 기계에는 애초에 없다).
- **`FT_*` 씬 9개 중 넷은 생성물이었다.** `Tools/featuretest/build-scenes.ps1`이
  `FT_Primitives`·`FT_Shadow`·`FT_Material`·`FT_Lights`를 만든다. 그 하네스는 씬
  부재를 이미 명시 처리한다("build-scenes.ps1을 먼저 돌릴 것"). 나머지 다섯
  (`FwdAlpha1`·`Transparent`·`TransparentShadow`·`TransparentShadowLow`·`ShadowLow`)은
  손으로 복사해 만든 것이고 **참조하는 하네스가 없다**(만드는 법은
  `MultiCameraRenderPlan.md` §에 남아 있다).
- **게이트 하나가 자산과 운명을 같이했다** — `verify-authored-rects` 은퇴(아래 §0.1).

따라서 아래 셋은 **차단 사유도 비용 항목도 아니다**:

- "기존 자산의 형상이 바뀐다" · "골든 재기준선이 따라온다" · "저작된 값이 유실된다"

⚠ **이 문서가 그 전제를 두 번 어겼다** (2026-08-20 정정). P4-b를 *"저작 자산 6종이
깨진다"*로 착수 직전 중단했고, P4-c를 *"자산 218개 재기준선"*으로 보류했다. 둘 다
폐기 정책 아래에서 비용이 0이며, 오히려 **아무도 그 위에 저작하기 전이 스키마를
고칠 적기**다. 판정 방향이 반대였다.

→ 앞으로 차단으로 올려도 되는 것은 **코드 구조 문제뿐**이다(배선 부재 · 훅 부재 ·
스레드 안전성). 자산 축과 코드 축을 반드시 갈라서 판정할 것.
...
재설계의 필수 잔여는 없다. S4 dirty 커밋은 **`TransformUpdatePlan.md`(PHASE 8.75)
트랙 T4가 승계했다** — 재개 근거가 "렌더 컴포넌트 200~400개"에서 "T2 위상 정렬
순회가 moved list를 공짜로 준다"로 바뀌어, 트리거를 기다리지 않는다(2026-08-25).
...
| **S** 스토어·Transform | ✅ 완결(잔여 이관) | S1 · S1-b+S3 · S2 · **광원 슬롯 복원 결함**(8-21) | S4 dirty 게이트 → **`TransformUpdatePlan.md` T4로 이관**(8-25) |
...
1. ✅ **자산·게이트 CLI 이전 — 12/12 완결 + 저작 자산 폐기 실행** (2026-08-21).
   게이트의 자를 CLI 저작본 위로 옮긴 뒤 자산 215개를 지웠다(§0.05).

   ⚠⚠⚠ **"5종"도 "8종"도 "10종"도 과소 계상이었다.** **세 번 연속** 같은 방향으로
   틀렸고, 매번 "이번엔 전수를 셌다"고 적은 뒤에 틀렸다. 마지막 둘(생명주기 순서 ·
   ASan)은 **폐기를 실행하고 회귀가 붉어져서야** 드러났다.

   → 결론: 자산 의존은 **조사로 확정할 수 없다.** 인자·기본값·자리표시자·간접
   경로가 매번 다른 곳에 숨었다. **끊고 돌려 보는 것**만이 확정한다. 폐기가 곧
   가장 강한 자였다.

   두 번째 누락은 **DDOL 게이트 2종**이다(2026-08-21, 폐기 직전에 발견). 둘 다
   목적지 씬을 `FT_Material`로 잡고 있었는데, 그 이름이 **시나리오 파일이 아니라
   검사 스크립트의 파라미터 기본값**에 있었다. 앞선 전수 실측은 *"시나리오의
   `scene.switch` 인자를 뽑아"* 대조했으므로, `{{DEST_SCENE}}` 같은 **자리표시자로
   치환되는 인자는 원리적으로 잡히지 않았다.**

   → 조사 축을 하나 더 세운다: **게이트 스크립트의 `param()` 기본값**도 자산 의존이다.

   ★ 그 조사에서 **더 나쁜 것**이 함께 나왔다. `verify-ddol-canvas`의 출발 씬
   기본값 `UITestScene`은 **`.gitignore`가 무시하는 파일**이었다. 즉 이 게이트는
   이미 **이 기계에서만 돌고 있었고** 다른 기계에서는 `"씬이 없다"`로 죽는다.
   `ui_regression`이 개인 폴더 GLB를 가리키던 것과 같은 부류인데, 그때 조사에서
   놓쳤다 — 같은 눈먼 지점이 두 번 같은 결과를 냈다.

   (아래는 첫 전수 실측 — 2026-08-20, 시나리오 인자 기준)

   | 게이트 | 의존 | 상태 |
   |---|---|---|
   | 트랜스폼 값 왕복 | Test1 씬 | ✅ **이전 완료** — 게이트가 저작 명령을 생성한다 |
   | 계층 표기 불변식 | Test1 · FT_Material 씬 | ✅ **이전 완료 + 감도 추가** — 갓만듦/왕복후 두 축(아래) |
   | 프리팹 왕복 · 리플렉션 골든 | BTProbe | ✅ **이전 완료** — 둘 다 BT 그래프를 안 쓴다(아래) |
   | BT 스모크 | BTProbe 프리팹 | ✅ **이전 완료** — FileGuid 지원을 세워 열었다(아래). BT 그래프 자산은 폐기 대상이 아니다 |
   ...
   | **저작 배치 대조** | 저작 프리팹 12종의 `m_worldRect` | ⛔ 이전 불가 — **후계 신설로 대체**(아래) |
   | DDOL 캔버스 재등록 | `UITestScene`(**미추적**) · `FT_Material` | ✅ **이전 완료**(8-21) — 두 씬 모두 시나리오가 만든다 |
   | DDOL 스크립트 이송 통지 | `FT_Material` | ✅ **이전 완료**(8-21) — 빈 씬을 저장해 목적지로 쓴다 |

   ★ **"BTProbe 3종"은 실은 1종이었다** (2026-08-20 실측). 셋이 같은 자산을 쓰길래
   묶여 보였지만, **BT 그래프를 실제로 쓰는 것은 BT 스모크 하나뿐**이다:

   | 게이트 | BTProbe에서 필요한 것 | 판정 |
   |---|---|---|
   | 프리팹 왕복 | **개수만 센다** — 컴포넌트 있는 프리팹이면 무엇이든 | ✅ 이전 |
   | 리플렉션 골든 | 골든은 **타입 레지스트리 덤프**(`GetAllTypeNames`+`type->create()`)라 씬 무관. 프리팹은 `perf.reflect`의 재료일 뿐 | ✅ 이전 |
   | BT 스모크 | **BT 그래프가 실제로 로드되고 틱해야 한다**(트리 3개·틱 465회·경계 크로싱) | ⛔ 불가 |

   → 골든은 이전 전후로 **diff 0이 그대로**였다(타입 77·직렬화 77). 재료를 완전히
   바꿨는데 덤프가 동일하다는 것이 *"골든이 정말 씬 무관인가"*의 실증이다.

   ✅ **BT 스모크도 옮겼다 — 막던 것은 BT 저작 표면이 아니라 FileGuid였다**
   (2026-08-20 실측으로 판정 정정).

   처음엔 "BT 그래프 저작 CLI가 없어 불가"로 봤다. 실제로 막고 있던 것은 달랐다:

   | 짐작 | 실측 |
   |---|---|
   | BT 그래프를 CLI로 만들어야 한다 | **아니다** — `.bt`·`.blackboard`는 **폐기 대상이 아니다.** 저작 자산(프리팹·씬)이 아니라 손으로 만든 **게이트 픽스처**다(노드 ID `900000001~4`, 스크립트명 `BTProbeAuthoredValue` 등 검사 전용) |
   | 프리팹만 CLI로 만들면 된다 | 맞다. 그런데 **`ApplyReflectedProperty`가 FileGuid를 지원하지 않았다** — *"지원하지 않는 프로퍼티 타입 (FileGuid)"* → 소환해도 트리 0개 |

   `BehaviorTreeComponent`는 자산을 **guid로만** 찾고 이름 폴백이 없다. 그래서
   FileGuid 지원을 세우자 그대로 열렸다 — 이전 후 실측이 **트리 3개 · 틱 462 ·
   건너뜀 0 · 경계 0.85**로 저작 자산을 쓰던 때와 같다.

   ★ **이 구멍은 BT만의 것이 아니었다.** 머티리얼·메시 등 **자산을 가리키는 모든
   필드**가 FileGuid다. 즉 CLI로는 자산 참조 컴포넌트를 저작할 수 없었다.

   ★ guid는 시나리오에 박지 않고 **`.meta`에서 읽어 채운다** — 박아 두면 재발급 때
   조용히 어긋난다.
   ...
   대상도 함께 줄어든다"*고 적는다. 자산 폐기와 함께 자동으로 죽는다.

   ✅ **후계 신설 완료 — UI 레이아웃 골든**(스위트 **18번째**, 2026-08-20).
   앵커 프리셋 8종(모서리 4·중앙 1·스트레치 3) · 3단 중첩 · 버튼 1을 CLI로 저작해
   `ui.rect` 출력을 골든으로 고정한다. 성질은 바뀐다("에디터 정답지와 일치" →
   ...
   **골든은 "지금 값"을 정답으로 굳히므로 뜨기 전에 사람이 검산해야 한다.**
   ...
   이송(`Detach`→로드→`Attach`)을 태우려면 **건너갈 씬 파일이 실제로 있어야 한다.**
   그래서 이 둘만 유일하게 "디스크의 씬"을 요구했다. 해법은 `scene.new` +
   `scene.save`로 **빈 씬을 시나리오 안에서 만들어 두고** 그리로 이송하는 것이다.
   빈 씬이라 목적지에 캔버스가 0개임이 **구조적으로** 보장된다 — 예전에는
   *"수정 전 실측이 0개였다"*는 관찰에 기대고 있었다.

   ⚠ **이 방식은 거짓 통과 구멍을 하나 연다.** 이전 실행이 남긴 씬 파일이 그대로
   있으면 `scene.save`가 실패해도 `scene.switch`가 성공해 게이트가 통과한다.
   두 겹으로 막았다: 검사 스크립트가 **실행 전에 그 파일을 지우고**, 판정 0이
   `[CLI] 씬 저장:` 로그를 **명시적으로** 확인한다. 후자가 없으면 저장 실패의 모습
   (`AddedToScene` 1회)이 배선 결함과 구분되지 않는다.

   → 이전 후 실측: 캔버스 이송 전 1 · 후 1 **`[DdolCanvas(0)]`**(이름이 찍히므로
   그 "1개"가 이송분임이 확정된다) · 스크립트 훅 순서 **13단계 그대로**.

   ⛔ **`verify-authored-rects` 은퇴 확정** (2026-08-21). 예고대로 자산과 함께 죽었다.
   스위트는 **18 → 17종**. 잃은 것을 정직하게 적는다:
   ...
   ★ **이전은 자산 독립만 주는 게 아니다 — 없던 감도를 얻는다** (2026-08-20 실증).
   저작 씬은 **로드 경로를 한 번 통과한 값**이라 로더의 리맵이 표기를 정규화해
   놓는다. 그래서 저작 자산에 기대는 한 **생성 경로의 결함은 원리적으로 안 보인다.**
   계층 표기 게이트를 CLI로 옮기며 측정 지점을 ①갓 만든 직후 ②저장·재로드 후로
   나누자, 되돌림 음성 시험에서 정확히 그 비대칭이 나왔다:
   ...
   ★★ **이전이 CLI 저작 표면의 구멍 넷을 드러냈다.** *"CLI 저작 표면은 이미 서
   있다"*(§0.05)는 **옮겨 보기 전까지 검증된 적이 없었다** — 실제로 옮기자 매번
   무언가가 없었다:

   | 구멍 | 막고 있던 것 | 메운 것 |
   |---|---|---|
   | `object.parent` 부재 | 계층 있는 픽스처 전부 | E8에서 신설 |
   | `object.create`에 UI·Canvas 없음 | **UI 오브젝트 저작 자체** | 타입 둘 추가 |
   | `ui.pos` 부재 | UI 배치(전부 중앙에 겹침) | 명령 신설 |
   | `ApplyReflectedProperty`의 FileGuid 미지원 | **자산 참조 컴포넌트 전부**(BT·머티리얼·메시) | 타입 분기 추가 |

   → 교훈은 하나다: **저작 표면이 선다는 것은 실제로 저작해 봐야 안다.** 명령 목록이
   있다고 표면이 닫힌 것이 아니다 — `ui.*`도 `bt.*`도 있었지만 둘 다 관측·설정이지
   생성이 아니었다.
   ...
   **왜 오래 숨었나** — 게이트가 **종료 코드를 보지 않았고**, 저작 프리팹은 UI 자식을
   ...
   ✅ **`ui_regression`의 외부 의존도 끊었다** (2026-08-20). 의존이 **둘**이었다 —
   개인 폴더 GLB와, `scene.new` 없이 쓰던 **기본 씬의 `Root` 오브젝트**.

   ★★ **②가 이 절의 판단 하나를 정정한다.** 8/8을 마치고 *"이제 폐기해도 게이트가
   살아남는다"*고 적었는데, 그 판단은 게이트가 무엇을 **소환**하는지만 보고 무엇을
   **전제로 시작**하는지는 보지 않은 것이었다. 위 조사표는 `scene.switch`·
   `prefab.instantiate`·`model.load` 인자를 훑어 만들었는데, **"씬을 새로 만들지
   않는다"는 그 방법으로 안 걸린다.** 자산 의존을 셀 때 *참조 목록*만으로는 부족하고
   **시작 상태 전제**도 함께 봐야 한다.

   ~~⚠ 별건 — `ui_regression.txt`가 저장소 밖 개인 경로에 의존한다~~ (해소):
...
3. ✅ **U7 Navigation 재설계 → E7-c** (2026-08-21). `Navigation::navObject`의 전역
...
5. ✅ **S4 dirty 게이트 → 이관 완료** (2026-08-25). `TransformUpdatePlan.md`
   (PHASE 8.75) **트랙 T4**가 승계했다. 옛 트리거(렌더 컴포넌트 200~400개)와 옛 선행
   (ImGui 드로어 get/set 전환)은 **둘 다 폐기**한다 — 재조사에서 근거가 바뀌었다.
   ① 입력이 생겼다: T2(위상 정렬 순회)가 "이번 프레임에 움직인 슬롯" 목록을 부산물로
   만들어 낸다. ② 차단 사유가 좁아졌다: `m_Material`/`m_bitflag`/`m_isEnableLOD`의
   `=` 대입 실측에서 **런타임 우회는 `ClrHost.cpp:1182` 한 곳**이고 나머지는 에디터
   또는 로드 시점이다. ⚠ 단 이 실측은 세 필드의 대입만 본 것이라 전수가 아니며,
   전수 재조사가 T4의 0단계다(이 저장소는 "전수를 셌다" 뒤에 세 번 틀렸다).
...
- ~~**P4-b · P1 잔여 · S4 — 셋이 같은 벽**~~ → **정정: 벽이 하나가 아니라 둘이고,
  ...
  | 쓰기 경로 | 형태 | 무엇을 막나 |
  |---|---|---|
  | **CLI·리플렉션** | `ApplyReflectedProperty` **단일 함수** (`ConsoleCommandSystem.cpp:318`·`414`) — `object.property`가 전부 이것을 지난다 | **아무것도 안 막는다.** 이미 길목이다 |
  | **에디터 ImGui** | 드로어가 `&meshRenderer->m_bitflag`처럼 **원시 포인터를 ImGui에 넘겨** 메모리를 직접 덮는다 | 사람이 인스펙터로 저작하는 경우 · **S4 dirty 게이트** |

  게이트 자산을 CLI로 저작하는 이상(§0.05) **게이트가 지나는 것은 위쪽 하나뿐이다.**
  즉 P4-b·P4-c와 그 게이트는 ImGui를 건드리지 않고 끝까지 세울 수 있다. ImGui 쪽
  get/set 전환은 **사람 저작의 충실도** 문제로 남고, 스키마 작업의 선행이 아니다.

### 회귀 세트 현황

**18/18 전체 통과** (종료코드 0). 구성이 2026-08-21에 한 번 바뀌었다 —
`verify-authored-rects` **은퇴**(저작 자산과 함께)하고 **광원 슬롯 복원**이
신설돼 수는 그대로 18이다.

★ **폐기가 게이트 셋을 드러냈다.** 저작 자산을 실제로 지우자 그 위에 서 있던
게이트들이 차례로 붉어졌다 — 조사로 세 번 찾지 못한 것을 삭제 한 번이 찾아냈다.
**"의존을 조사한다"보다 "의존을 끊고 돌려 본다"가 강한 자다.**

| 드러난 것 | 어떻게 |
|---|---|
| DDOL 게이트 2종의 `FT_Material` 의존 | 폐기 **직전** 조사(파라미터 기본값 축을 새로 세워서) |
| 생명주기 순서 게이트의 `FT_Material`·`UITestScene` 의존 | 폐기 **후** 회귀가 붉어져서 — 파라미터 축을 세우고도 전수를 안 돌린 탓 |
| ASan 게이트의 `Test1`·`Test2` 의존 | 위를 계기로 그제서야 돌린 전수(`param()` 기본값 일괄 조사) |
| **`Scene::GetLight`의 off-by-one** | 생명주기 게이트를 CLI 저작본으로 옮기며 **라이트 둘짜리 3D 씬을 처음 만들자** 즉시 크래시 |
...
이 계획 착수 시점 대비 검사 8종이 늘었다
(스크립트 부착 1회 · 중첩 프리팹 정체성 · DDOL 캔버스 재등록 · DDOL 스크립트 이송
통지 · 계층 표기 불변식 · 프리팹 오버라이드 기록 · 프리팹 인스턴스 복제 ·
**중첩 프리팹 정의 전파**).

★ **스위트 밖 게이트는 이제 없다.** `verify-prefab-nested-update.ps1`이 P4-b
착지와 함께 GREEN이 되어 편입됐다(2026-08-20). 착지 전에는 의도적 RED로 밖에
두었다 — 상시 실패 게이트는 스위트 전체를 붉게 만들어 진짜 회귀를 가리기 때문이다.

기준선 값:

| 게이트 | 현재 값 |
|---|---|
| 생명주기 순서 | **221 사건** (2026-08-21 재기준선 — CLI 저작본으로 이전. 직전은 200, 저작 씬 `FT_Material`↔`UITestScene`). **단계 6종 구성은 그대로**(OnInitialized·OnAddedToScene·OnUninitializing·OnRemovingFromScene·OnEndSimulation·Update) — 계약이 보존됐고 수만 재료 차이로 늘었다. 컴포넌트 타입은 5종→**6종**(TextComponent 추가), Update를 내는 타입은 2종→**5종**으로 넓어졌다 |
| 광원 슬롯 복원 | 라이트 **3개** 씬이 저장·재로드·재생 완주 · 계층 오브젝트 5 · 종료 0 (신설 — RED는 `0xC0000409`) |
...
- ✅ **E5 — shared_ptr 축소** (2026-08-21): Scene 슬롯이 `unique_ptr<Entity>`로
  ...
  **"다른 씬의 같은 슬롯"이 구조적으로 차단**된다. `m_sceneId`는 `const`이고
  ...
  ✅ **E5-R2 — `Scene::Canvases`/`CanvasMap` 핸들 전환** (2026-08-19, `75daba59`):
  ...
  **fail-closed** — DDOL 이송(`DetachGameObjectHierarchy`)은 오브젝트를 살려 둔 채
  ...
  ✅ **게이트 신설 — 그리고 그 자가 진짜 결함을 하나 잡았다** (2026-08-20).
  `scene.ddol <이름>` CLI를 세워 이송 경로를 시나리오에서 태울 수 있게 했다
  (`DetachGameObjectHierarchy` 호출자는 `SceneManager`의 씬 로드 경로 2곳뿐이라
  회귀 세트가 이 경로를 **단 한 번도 태운 적이 없었다**).

  자를 세우자마자 나온 것: **DDOL로 이송된 캔버스가 새 씬의 목록에 영영 들어가지
  않았다.** `Canvas`의 캐시 등록이 `OnDeserialized`에만 있어서다 — 이송 재부착은
  역직렬화를 타지 않는다. 오브젝트는 살아 넘어오므로 `ImageComponent` 연결 같은
  다른 신호는 멀쩡해 보이고(`ui.status`의 "Image 1/1 연결"), 캔버스 목록만 비어
  `UIManager::Update`가 `CurCanvas`를 못 세운다 — UI 내비게이션이 조용히 죽는다.

  → 등록을 `Canvas::OnAddedToScene`으로 옮겼다. **바로 그 자리 주석이 이미 답을
  적고 있었다**: UITickSystem이 씬 편입/이탈 훅을 쓰는 이유로 *"DDOL 오브젝트가
  씬을 건널 때도 매번 다시 불려야 하기 때문"*이라 적혀 있는데, 캐시 쪽만 그 결론을
  못 받고 있었다. 이탈 쪽은 훅이 필요 없다 — 핸들이 fail-closed라
  `UIManager::Update`가 해석 실패분을 그 자리에서 걷어낸다(**R2가 이걸 가능하게 했다**).
  `Scene::AddCanvas`는 멱등하게 만들었다(재부착으로 같은 캔버스가 여러 번 들어올 수 있다).

  음성 시험 3상태: 수정 전 코드 = 이송 전 1·이송 후 0(겨냥한 결함의 정확한 모습) ·
  등록 전부 무력화 = 0·0 · 수정 후 = 1·1.

  ✅ **E5-R3 — `UIManager::CurCanvas`/`SelectUI` 핸들 전환** (2026-08-20): 분류 D.
  **차단 사유가 이미 사라져 있었다.** 그 자리 주석이 "EntityHandle로 못 바꾼다"고
  적으며 든 근거는 *"활성 씬이 바뀐 뒤 옛 핸들의 index·generation이 새 씬의 무관한
  오브젝트와 우연히 일치해 유효한 것처럼 풀린다"*였는데, **E5-0이 `sceneId`를
  넣으면서 `Scene::Resolve`가 씬 불일치를 세대 검사보다 먼저 거른다.**
  - 필드를 private `EntityHandle` 둘로 내리고 접근자 넷을 뒀다
    (`GetCurCanvas`/`GetSelectUI`/`SetCurCanvas`/`SetSelectUI` + Clear 둘) —
    **저장은 핸들, 사용은 해석.** 소비자 7파일 14곳이 전부 이 관문을 지난다.
  - `UIManager::CheckInput`이 손으로 하던 "활성 씬이 바뀌면 리셋"(오브젝트를 lock한
    뒤 그 `GetScene()`을 활성 씬과 비교)이 **구조로 대체돼 사라졌다.** R2에서
    `RemoveCanvas`가 그랬듯, 손으로 지키던 규약이 타입의 성질이 된다.
  - `Canvas::OnUninitializing`의 수동 정리도 `GetCurCanvas() == m_pOwner` 비교로 단순화.

  ★ **해석 실패가 안전한 이유**: 이 선택 상태는 `UIManager::Update`가 매 프레임
  캔버스 목록에서 다시 세운다. 다른 씬의 것으로 판정되면 그 프레임에 무시되고 다음
  프레임에 자가 복구된다 — 사용자 확정 설계의 *"이벤트를 놓치더라도 핸들 검증으로
  안전하게 무시"* 가 그대로 성립하는 자리다. 그래서 "DDOL 오브젝트가 선택된 채 씬을
  건너는" 시나리오의 미확인이 더 이상 착수 조건이 아니었다.
...
- ✅ **E6 — GameObject → Entity 심볼 리네임** (사용자 결정 2026-08-16, **K1-b
  ...
  ✅ **읽기 별칭(§5) 마감 + 그 자리에서 드러난 선행 결함** (2026-08-19, `75daba59`).
  ...
  **회귀 12/12 복귀** — 트랜스폼 해시 `f593139644a26cf1`(재설계 전과 동일) ·
  ...
  ★ **리네임이 19개월 묵은 중복을 드러냈다** (2026-08-20, `cfc88f40`·`164d6bdd`).
  `ScriptBinder/BlackBoard.h`와 `Blackboard.h`가 **둘 다 추적되고 있었다** —
  2025-06-23 두 갈래에서 서로 다른 Blackboard 구현이 각각 만들어져(81줄 차이)
  병합 때 두 경로가 함께 들어왔다. Windows는 대소문자를 안 가려 실물 파일이
  하나뿐이라 증상이 없었고, 두 인덱스 항목의 내용이 수렴해 같은 블롭이 된 뒤로는
  더더욱 조용했다(패배한 구현은 조용히 덮여 사라졌다).
  E6가 한쪽만 갱신하면서 내용이 갈라지자 **트리가 영구 dirty**가 되어
  `git pull --rebase`가 막혔다(`--autostash`도 stash 직후 같은 상태가 재생성돼
  빠져나올 수 없다). 정본을 `BlackBoard.h`로 통일하고 중복 인덱스 항목을 제거했다.
  같은 자리에서 **혼합 인코딩**도 드러났다 — 한 파일 안에 UTF-8 3줄과 CP949 1줄이
  섞여 있어 리네임 도구가 CP949 구간에서 실패해 한글 주석을 깨뜨렸다(코드가 아니라
  주석이라 빌드는 지나갔다). 구판에서 줄 단위로 복원했다.
  이어서 계열 철자를 클래스와 일치시켰다(`BLackBoardValue.h`의 대문자 L 오타 포함) —
  이 저장소의 리플렉션 생성기가 "리플렉션 클래스 = 동명 헤더"를 전제하므로 그
  일치는 취향이 아니라 요구 사항이다.
- ✅ **E7 — 저장된 GameObjectType 상태 소멸** (2026-08-21 완결; 생성 의도 enum은 존치):
  ...
  - ✅ **E7-c — 필드 제거** (2026-08-21): §0.05에서 저작 자산이 폐기되어 과거의
    "218개 일괄 변환" 대상은 0이 됐다. U7을 먼저 다음처럼 재설계해 코드 의존을 끊었다.
    - `Navigation`의 영속 참조는 평면 프리팹 인덱스가 아니라 **소스 UI 기준 계층 경로**
      (`parentHops` + 각 단계의 `childOrdinals`)다. 씬·일반 프리팹·중첩 프리팹에서 같은
      규칙을 쓰고 전역 ID 검색을 하지 않는다.
    - 저장 직전 `OnBeforeSerialize` 훅에서 경로를 다시 계산하므로 저작 후 reparent도
      반영된다. 구 `navObject` YAML은 씬 로드 시 해소 후 새 경로로 치유하고, 프리팹은
      인스턴스화 전 복제한 YAML 안에서 로컬 경로로 승격한다.
    - `Prefab::InstantiateRecursive`의 UI ID 예외를 제거해 모든 노드가 새 instanceID를
      받는다. 같은 프리팹 두 인스턴스의 링크는 각자 자기 계층 안에서만 풀린다.
    - `m_gameObjectType` 멤버·리플렉션·`GetType()`·저장 상태 기반 switch를 제거했다.
      새 파일은 공간 컴포넌트 조합으로 생성 형상을 추론하고, 구 파일의 필드는 로더가
      읽기 호환 힌트로만 소비한다. Bone 판정은 `BoneComponent`가 정본이다.
    - 신규 `ui.navprobe`가 새 스키마, 두 인스턴스 격리, 새 ID, UI/Canvas 공간 컴포넌트,
      구 YAML 승격을 한 번에 검증한다. 리플렉션 골든 **77타입·diff 0**, 전체 회귀
      **20개 섹션 통과**. 해상도 스윕의 기존 D3D12 resize 래칫(2/7)은 별건으로 동일하다.
...
- ✅ **E8 잔여 — 생성 경로가 통일에서 빠져 있었다** (2026-08-20, 자산·게이트 CLI
  ...
  **갓 만든 씬은 한 번도 잰 적이 없었다.** 자를 CLI 저작본으로 옮기니 저작 자산이
  ...
  ✅ **후속 완료 — `scene.hierarchycheck`가 이제 CLI 저작 씬도 잰다** (2026-08-20,
  자산·게이트 CLI 이전 2/8). 측정 지점을 ①갓 만든 직후 ②저장·재로드 후로 나눴고,
...
- ✅ **S1-b — TransformComponent 컴포넌트화** → **S3와 병합해 완료**(아래 `S1-b + S3`
  항목이 이행분이다. 이 절은 그 병합 판정에 이른 재측정 기록이다 — 2026-08-18). 원래 적힌 차단 사유(MetaGenerator가 "리플렉션 클래스 = 동일명 헤더 +
...
- 🔶 **S4 — 소비자 직결·변경분 커밋** (2026-08-18, **고정 비용분만 완료**):
  ...
  **재개 트리거**: 저작 씬의 렌더 컴포넌트가 상시 200~400개를 넘길 때(현재 실측
  ...
  ★ **이관 — 위 트리거는 폐기됐다** (2026-08-25). `TransformUpdatePlan.md`(PHASE 8.75)
  **트랙 T4**가 이 슬라이스를 승계한다. 두 가지가 바뀌었다. ① **입력이 생겼다** —
  T2(위상 정렬 순회 배열)가 "이번 프레임에 움직인 슬롯" 목록을 부산물로 만든다.
  즉 게이트를 위해 따로 만들 자료구조가 없다. ② **차단 사유가 좁아졌다** — 위
  *"우회가 구조적으로 열려 있다"* 서술을 재조사하니, 세 필드의 `=` 대입 중 **런타임
  경로는 `ClrHost.cpp:1182`(C# `Material.Instantiate`) 한 곳**이고 나머지는
  `ImGuiDrawHelperMeshRenderer`(에디터) 또는 `ModelSceneBridge`(로드 시점 — 프록시가
  그 뒤에 생성되므로 무해)다. 한 곳은 명시 마킹으로 닫힌다.
  ⚠ **다만 이 실측은 전수가 아니다** — 세 필드의 `=` 대입만 봤고 멤버 함수 경유
  변경·LOD 계열 타 필드·리플렉션 쓰기 경로는 확인하지 않았다. 전수 재조사가 T4의
  0단계이고, 결과가 "런타임 우회 1건"을 부정하면 트랜스폼 전용 델타 커맨드로
  후퇴한다(대안은 `TransformUpdatePlan.md` §4 T4).
...
- ✅ **P4 — 중첩 프리팹 (핵심 승격 후 완주)**: 프리팹 안의 프리팹 인스턴스를
  ...
  **6건**이다: `Bridge` · `CriticaHitEffect` · `ItemSlotPrefab2`(2단) ·
  ...
  - ✅ **P4-b — 중첩 프리팹을 참조 노드로 (완주)** (2026-08-20).
    `SerializeRecursive`가 중첩 인스턴스 루트를 펼치지 않고
    `{정의 GUID + 오버라이드}` 참조 노드로 굽고, 소환 시점에 그 프리팹의
    **현재 정의**로 푼다. 게이트 RED→GREEN 전환이 증명이다.

    | 슬라이스 | 내용 |
    |---|---|
    | **S1** | 게이트 재작성 — 판정 4 **방향 반전** + 판정 8(오버라이드 보존) 신설. 의도한 지점에서 RED 확인 |
    | **S2** | `PrefabUtility::ApplyRecordedOverrides` — `m_valueYaml`을 **읽어 쓰는 최초 경로** |
    | **S3** | 참조 노드 굽기(`SerializeNestedReference`) + 해석(`InstantiateNestedReference`) |
    | **S4** | 정의 영속화 결함 수술(아래) · 스위트 편입(17번째) |

    ★ **게이트가 "그대로 GREEN이 되는" 것이 아니었다.** 원래 판정 4는 *"구운 직후
    파일에 옛 값(false)이 박혔는가"* — 즉 **펼친 스냅샷을 전제로 보호**하는 항목이라,
    P4-b가 성공할수록 실패하게 돼 있었다. 착지와 함께 "펼친 리터럴이 **없고** 참조
    마커가 **있어야** 한다"로 뒤집었다. 계획서가 적어 둔 *"RED→GREEN 전환이 곧
    증명"*은 판정 6에만 해당했고, 자가 검증 축은 함께 재작성해야 했다.

    ★ **절반만 하고 완주로 착각할 뻔했다 — `m_valueYaml`을 읽는 코드가 없었다.**
    P-write가 기록을 세웠지만 소비는 **배제 집합(이름)**으로만 했다. `PrefabOverride.h`가
    *"되돌릴 때 이 문자열을 다시 파싱해 적용한다"*고 적어 둔 그 경로가 미구현이었다.
    참조 노드는 컴포넌트를 담지 않으므로 되먹임 없이는 중첩 인스턴스의 로컬 수정이
    **매 소환마다 전부 유실**된다 — 게이트가 그걸 안 보니 통과했을 것이다. 그래서
    판정 8을 함께 세웠다. **한 오브젝트에서 두 축이 갈리게** 설계한 것이 요점이다:
    `m_isEnableLOD`(오버라이드 없음)는 정의를 따라가고, `m_shadowCast`(오버라이드
    있음)는 로컬을 유지한다. 같은 필드로 재면 둘이 서로를 가린다.

    ★ **Entity 자신의 프로퍼티는 일부러 되먹이지 않는다.** 과도기 시딩이
    `m_index`·`m_parentIndex` 같은 **엔진 장부까지** 오버라이드로 오기록한 이력이
    있다(P-write 실측 8건 중 7건). 갓 만든 오브젝트에 되먹이면 계층이 그 자리에서
    깨진다. 참조 노드가 Entity 필드를 그대로 싣고 있어 되먹일 이유도 없다.

    ★★ **P4-b가 잠재 결함 둘을 드러냈다** (새로 만든 것이 아니다):

    | 결함 | 내용 |
    |---|---|
    | **`prefab.update`가 정의를 영속화하지 않았다** | 새 정의를 만들어 **인스턴스에만** 적용하고 디스크에도 캐시에도 안 썼다. `LoadPrefabFullPath`는 캐시가 있으면 파일이 바뀌어도 캐시를 주므로, 이후 그 프리팹을 다시 읽으면 **갱신 전 정의**가 돌아온다. P4-b 이전에는 아무도 정의를 다시 읽지 않아 드러나지 않았다 — 참조 노드가 **소환 시점에 정의를 재조회**하면서 보였다(실측: 판정 4는 GREEN인데 판정 6만 RED) |
    | **`SavePrefab`의 캐시 `erase`가 위험했다** | 캐시 항목이 `unique_ptr`라 지우면 `Prefab` 객체가 파괴되는데, 살아 있는 인스턴스들이 `obj->m_prefab`으로 그 객체를 **raw 포인터로** 들고 있다. 기존 주석은 호출자 포인터만 걱정했다. **erase → 데이터 교체**로 바꿔 포인터를 살리고 정의만 새것으로 넣는다 — 그 하나가 P4-b의 재조회 전제도 함께 세운다 |

  - ⛔ ~~**P4-b — 지금 하면 자산이 깨진다**~~ (2026-08-20 실측, 착수 직전 중단 — **판정 철회**).
    `SerializeRecursive`가 중첩을 통째로 펼치는 것을 참조 노드
    `{정의 GUID + 오버라이드}`로 바꾸는 것이 P4-b인데, **그 "오버라이드"가 비어 있다.**

    | 실측 | 값 |
    |---|---|
    | `m_prefabOverrides` 항목을 가진 프리팹 | **0 / 208** |
    | 항목을 가진 씬 | **0 / 12** |
    | 갓 저장한 형상(리플렉션 골든) | `m_prefabOverrides: ~` — 필드는 방출되나 항상 빈 값 |

    즉 중첩 서브트리를 접는 순간 그 인스턴스의 **위치·이름·컴포넌트 값이 전부
    사라진다.** 에러도 로그도 없이. 대상은 지금 저작된 중첩 자산 6종
    (`Bridge`·`CriticaHitEffect`·`ItemSlotPrefab2`·`TutorialObj`×3벌·`TutorialObj3`·
    `UI_CanvasesBoss`)이다.

    ★ **선행은 P4-a가 아니라 P1의 잔여였다.** 코드가 자기 입으로 적어 둔다:
    *"오버라이드 기록의 정본은 에디터가 프로퍼티를 바꾸는 시점에 `m_prefabOverrides`에
    직접 쓰는 것이어야 하지만, **그 배선은 후속 슬라이스다**"*(PrefabUtility.cpp).
    현재 채워지는 유일한 경로인 `SeedOverridesFromSnapshot`은 ① `UpdateInstances`
    (인스턴스에 적용) 안에서만 돌고 ② 기준으로 삼는 `m_prefabOriginal`이 **비직렬화**라
    씬을 재로드하면 근거 자체가 없다. 저장 시점에는 아무도 안 부른다.

    ★ **저장 시점 diff로 우회하는 안도 그대로는 안 된다.** `SeedTypeOverrides`를
    저장 때 정의와 비교하도록 돌리는 방법이 있으나, 그 함수도 `UpdateInstances`도
    **자식을 전혀 순회하지 않는다**(아래 함정 기록). 중첩 인스턴스가 자기 자식을
    손댄 경우가 그대로 유실된다. 즉 "형식 전환"이 아니라 **완전한 재귀 diff를
    새로 세우는 작업**이다.

    ★ **S4·P1과 뿌리가 같다 — "값이 쓰이는 길목"이 없다.** 에디터의 제네릭
    리플렉션 드로어는 `DrawEnumProperty(int* instance, …)`처럼 **원시 포인터로 메모리를
    직접 덮는다**(ReflectionImGuiHelper.h). S4의 dirty 게이트가 같은 이유로 보류됐고
    (`ImGuiDrawHelperMeshRenderer`가 `&m_bitflag`를 ImGui에 그대로 넘긴다), P1의
    "변경 시점 기록"도 같은 벽에 막혀 있다. **셋은 각각의 슬라이스가 아니라 하나의
    선행 작업**이다 — 쓰기 경로에 훅을 심거나 드로어를 get/set으로 바꾸는 것.
  - ✅ **P4-c — 닫혔다** (P-write S2가 함께 해결. 2026-08-20 3차 정정 — 이 항목은
    한동안 `🔶 재개 가능`으로 남아 있었으나 **상태 표시가 낡은 것이었다**).

    실측 근거: `CollectComponentOverrideNames`가 슬롯을 인자로 받아
    `ov.m_componentSlot >= 0 && ov.m_componentSlot != slot`이면 건너뛰고
    (`PrefabUtility.cpp:114`), 호출부가 `ComputeComponentSlot(obj, comp)`으로
    **실제 순번**을 넘긴다(`:185`). 즉 배제 집합이 타입 전체가 아니라 그 순번의
    컴포넌트에만 걸린다 — 아래 서술한 결함이 실제로 해소된 상태다.

    ⚠ **교훈**: 한 슬라이스(P-write S2)가 다른 항목(P4-c)을 함께 닫을 때 그쪽
    상태 표시를 갱신하지 않으면, 문서가 **이미 된 일을 남은 일로** 계속 광고한다.
    이 계획서가 C2에서 같은 일을 겪었다(*"원 문구는 이미 된 일을 하라고 적혀 있었다"*).

    아래는 착수 당시 실측 기록으로만 유효하다 — P3가 P4로 미뤄 둔 잔여였고,
    오버라이드 판정이 타입명 단위였다.

    **대상은 실재한다** — 저작 자산 220개·엔티티 436개를 훑어 한 엔티티에 같은 타입
    컴포넌트를 둘 이상 얹은 것이 셋 나왔다: `GameManager.prefab`의 `ModuleBehavior` **6개**,
    `MonsterAEvent`·`MonsterBEvent`의 **2개씩**. 그 인스턴스에서 스크립트 #3의 프로퍼티를
    손보면 배제 집합이 **6개 전부**에 걸려, 프리팹 갱신이 나머지 다섯에 새 값을 못 민다.
    ...
    **그런데 고칠 대상인 기록 자체가 비어 있다.** 오버라이드 목록을 채우는 유일한
    ...
    → 빈 구조에 순번 칸을 늘리는 일이다. **선행은 P4-b와 동일** — 오버라이드가 실제로
    기록되는 길(에디터 쓰기 시점 기록 = 쓰기 길목)을 먼저 세우는 것. 그 하나가
    P4-b·P4-c·P1 잔여·S4를 함께 연다.
...
- ✅ **P-write — 오버라이드 기록 배선 (S0~S5 완주)** (2026-08-20). P4-b·P4-c의
  단일 선행이었다. 착수 전 실측: `m_prefabOverrides`에 `push_back`하는 지점이
  `SeedOverridesFromSnapshot` **하나뿐**이고, 그 기준인 `m_prefabOriginal`이
  `reflect()`에 없어 **비직렬화**라 씬을 다시 열면 근거가 사라졌다.

  ★ **게이트를 먼저 세운 대가로 부수 결함 셋이 나왔다.** 셋 다 코드만 읽어서는
  안 드러났고 — 설계 조사 에이전트 4명이 데이터 모델·쓰기·소비 경로를 정확히
  짚고도 놓쳤다 — 돌려야 보였다.

  | 발견 | 내용 |
  |---|---|
  | **CLI 프리팹의 널 GUID** (S0.5) | `CreateFromGameObject`가 FileGuid를 안 매겨 `.meta` guid가 `00000000-…`이 되고, 인스턴스가 프리팹을 못 가리켰다. 폐기 정책 아래 CLI가 유일한 저작 경로라 치명적이었다 |
  | **`prefab.update`의 정체성 미승계** | `CreatePrefab` 산출물은 널 GUID라 `UpdateInstances`가 널 키로 조회해 **조용히 0건 적용**. 안 고쳤으면 게이트가 아무것도 못 쟀다 |
  | **스냅샷 노드 공유** | yaml-cpp가 참조 의미라 한 프리팹의 **모든 인스턴스가 스냅샷 노드 하나를 공유**했다. 첫 인스턴스 처리가 그 노드를 새 데이터로 덮어, 나머지의 시딩이 *"현재값(옛) vs 스냅샷(새)"*를 비교해 **프리팹이 방금 바꾼 필드를 사용자 오버라이드로 오기록** → 갱신이 **첫 하나에만** 먹었다. `MetaYml::Clone`으로 절단 |

  이행:

  | 슬라이스 | 내용 |
  |---|---|
  | **S0** | `prefab.overrides`·`prefab.update` 관측 명령 — 배선 전 빨간 기준선 |
  | **S1** | 게이트 신설, **의도한 지점에서 RED** 확인 후 스위트 밖에 둠 |
  | **S2** | `PrefabOverride::m_componentSlot`(P4-c) + `ComputeComponentSlot` **정본 헬퍼** |
  | **S3** | `RecordPropertyOverride` + `object.property` 훅 — 판정 3 RED→GREEN |
  | **S4** | `object.transform` 훅 — 리플렉션 세터를 우회하는 유일한 다른 CLI 경로 |
  | **S5** | 스위트 편입(16번째) |

  ★ **정본이 서니 추론이 물러났다.** 목록이 비지 않게 되는 순간
  `UpdateInstances`의 과도기 시딩(`empty()` 조건)이 안 돈다. 그 시딩은
  `m_name`·`m_instanceID`·`m_index`·`m_parentIndex` 같은 **엔진 장부까지 사용자
  수정으로 오기록**했다(실측 8건 중 7건). InstA의 기록이 8건 → 1건(뒤에 트랜스폼
  3건 추가로 4건)으로 줄었다.

  ★ **`m_componentSlot`의 `-1`은 "타입 전체"**다. 순번 필드가 없던 시절 데이터가
  역직렬화되면 기본값 `-1`이 되어 예전 행동 그대로 동작한다. 리플렉션 골든은
  그 한 줄(`m_componentSlot: -1`) 삽입으로 재기준선했다(폐기 정책상 비용 0).

  ⬜ **남긴 것**: `m_prefabOriginal`·`SeedOverridesFromSnapshot` 폐기. 기록이 없는
  인스턴스가 아직 그 경로를 탄다(실측 InstB 7건). 걷어내려면 그 창까지 게이트로
  덮은 뒤여야 한다 — 순서를 앞당기면 배선 전 창에서 지금 고친 버그를 재현한다.

  ⚠ **자가 네 번 틀렸고 전부 엔진을 오진하는 방향이었다** (기록). 블록 정규식이
  앞 오브젝트 값을 물어옴 · 출력 덤프를 12줄로 잘라 결정적 항목을 놓침 ·
  `ComputeComponentSlot`을 뽑아 두고 아무도 안 부름(세는 자리가 셋이 됨) ·
  `(\S+)`가 인라인 맵을 `{x:`에서 자름. 매번 다음 측정이 잡았지만 그 사이에
  잘못된 진단이 두 번 보고됐다. **자를 먼저 검증하라는 규율은 자 자신에게도 적용된다.**
...
- ✅ **C3 (3차 — 트레이스 발생지 이관 + UIButton·PlayerInput)** (2026-08-18,
  `4bf92189`): **C3가 진행될수록 생명주기 트레이스의 커버리지가 조용히 줄고
  있었다.** 시스템으로 간 컴포넌트는 `Scene::RegistryTick`을 지나지 않으므로
  기록이 사라진다 — 검사 씬에 그 6종이 없어 눈에 안 띄었을 뿐이다. 다음 회차가
  RegistryTick의 틱 디스패치를 걷어낼 예정이었으므로 **그 전에 발생지를 시스템으로
  함께 옮겼다.**
  - `TraceTypeName`이 Scene.cpp 익명 네임스페이스에 있었다. 기록 지점이 하나일 땐
    충분했지만 흩어진 뒤에는 각 시스템이 복제해야 하고, 그러면 기준선 대조의
    전제("두 경로가 같은 문자열을 남긴다")부터 흔들린다 → `Lifecycle::Trace::TypeNameOf`로 승격.
  - 시스템 6개의 틱 9곳에 `LIFECYCLE_TRACE` 삽입. 이 단계는 기준선 중립이고 실제로 92사건 동일.
  - UIButton → `TickInteraction`(UITickSystem), PlayerInputComponent → `TickInput`(PlayerInputSystem 신설).

  ★ **PlayerInput만 소비자가 하나 더 있었다.** `Scene::InternalPauseUpdateForUI`가
  타입 포인터로 직접 틱한다(게임이 멈춰도 입력은 살아야 한다). 가상 `Update`로 뒀다면
  그 호출이 기반의 빈 `Component::Update`에 조용히 붙어 **일시정지 중 입력이 죽었을**
  것이다 — C3 2차에서 Text/SpriteSheet로 실제로 겪은 실패 양식이라, 진입점 이름이
  시스템과 그 우회로가 **공유하는 계약**임을 헤더에 못 박았다.
...
- ✅ **C3 (4차 — 기준선 4종)** (2026-08-18, `2eec89b7`): LightComponent ·
  ...
  **재기준선 92 → 93 — 판정 기준을 착수 전에 못 박았다**: (phase,type,object)
  ...
  ✅ **여기서 드러난 별건 → C5로 닫았다** (아래).

- ✅ **C5 — 트레이스 라벨을 실제 훅 이름으로 + 기록 없던 3단계 계측** (2026-08-20):
  `Lifecycle::Phase`가 옛 3훅 이름(Awake·Start·OnDestroy)에 굳어 있었고, 6단계 중
  **셋은 아예 기록이 없었다**(`OnAddedToScene`·`OnEndSimulation`·`OnRemovingFromScene`).
  그래서 기준선이 "무엇이 실제로 불렸는가"를 말하지 못했다 — C3 4차의 Canvas
  `Awake` 사건이 그 실례다.
  - 라벨 정정: `Awake`→`OnInitialized` · `Start`→`OnBeginSimulation` ·
    `OnDestroy`→`OnUninitializing`. 열거 순서도 한 컴포넌트가 겪는 시간 순으로.
  - **틱 셋(FixedUpdate·Update·LateUpdate)은 그대로 둔다** — 훅이 아니라 프레임
    페이즈이므로 이름이 옳다(C4에서 `Scene`의 틱 셋을 안 건드린 것과 같은 사유).
  - 기록 신설 5곳: 드레인의 `OnAddedToScene`, 파괴 축소의 `OnEndSimulation`·
    `OnRemovingFromScene`, **DDOL 이송의 부착·이탈 양쪽**, 프리팹 즉시 소멸 경로의
    축소 삼단. 예전 주석은 "대응하는 옛 훅이 없어 기록에 남기지 않는다"고 적었는데,
    그 침묵이 곧 검증 공백이었다.

  ★ **재기준선 93 → 200 — 매핑을 먼저 검증하고 떴다.** 구 기준선에 이름 대응
  (Awake→OnInitialized 등)을 적용한 뒤 신규 실측과 다중집합 대조:

  | 판정 | 결과 |
  |---|---|
  | 사라진 사건 | **0건** |
  | 새로 생긴 사건 | 107건 — **전부** 신규 계측 3종 |
  | 내역 | OnAddedToScene 43 · OnEndSimulation 32 · OnRemovingFromScene 32 |

  기존 `OnInitialized` 43 · `OnUninitializing` 32와 **1:1로 짝이 맞는다** — 코드
  형상이 예측한 그대로다. 즉 정보 손실 0에 커버리지만 늘었다.

  ★ **자가 옛 라벨을 이름으로 찾고 있었다.** `verify-script-add-awake-once.ps1`의
  보조 판정이 트레이스에서 `	Awake	`를 정규식으로 찾는다 — 라벨을 고치자 0건이
  되어 실패했다. 게이트가 **관측 대상의 이름을 하드코딩하면 그 이름을 고치는 순간
  거짓 실패**가 난다. 실패 메시지가 가리키던 `scene->Awake`도 C4에서 이미 개명된
  이름이었다(둘 다 정정).

  검증: Debug 전체 빌드 오류 0 · 회귀 **12/12** · 트랜스폼 해시 `f593139644a26cf1` 유지.
  ...
  실경로다. 선행 가드로 닫았다(2026-08-19, `75daba59`).

- ✅ **C4 — `Scene`의 레거시 생명주기 이름 정리** (2026-08-19, `75daba59`):
...
- ✅ **L3 — 이관과 브리지 철거** (2026-08-18, `cd78b63d` + 선행 `ed9a797b`):
  레거시 훅 3종과 브리지를 걷고 살아있는 소비자 **95곳을 전부** 새 6단계 훅으로
  옮겼다(엔진 23종 77곳 + C# 14파일 18곳). Component 가상 14→11.
  ★ **처음 센 436곳이 틀렸다** — Dynamic_CPP의 285곳은 컴파일 대상이 아니다
  (솔루션 7개 프로젝트 어디에도 없고 include하는 `ModuleBehavior.h`가 트리에
  존재하지도 않는다). 죽은 코드는 삭제를 막지 못한다.
  재확인(2026-08-20): 엔진 측 레거시 오버라이드 **0건**, C# `Behaviour`도 6단계 +
  OnEnable/OnDisable뿐. ② BehaviourGenerator 제외 목록도 `ed9a797b`에서 처리됐다.

  ✅ **잔여 1단계 완료 — DDOL 이송 신호가 C#까지 닿는다** (2026-08-20). 아래
  "착수 전에 풀어야 할 설계 문제"의 답으로 **이송 경로 전용 통지**를 골랐고,
  그것을 C(드라이버 일원화)가 그대로 확장할 형태로 세웠다.

  ★ **사용자 결정(2026-08-20) — 방향은 C다.** 최종형은 `BehaviourRegistry`가 자체
  큐를 걷고 6단계 전부를 네이티브에서 받는 것이다. 드라이버가 하나면 이중 발화가
  **표현 불가능**해진다. 그래서 이번 슬라이스는 DDOL 전용 통로가 아니라 **범용 단계
  디스패치**를 놓았다 — 나중에 걷어낼 우회로가 아니라 C의 전송로 그 자체다.
  - `ScriptLifecyclePhase.h`(네이티브) ↔ `BehaviourRegistry.LifecyclePhase`(C#):
    6단계 번호. 경계를 넘는 것이 int 하나라 컴파일러가 불일치를 못 잡는다는 것을
    양쪽 주석에 못 박았다.
  - `ClrHost::DispatchLifecycle(instanceId, phase)` + 관리 측 `DispatchLifecycle` —
    관리 측은 `Invoke`를 지난다(스크립트 예외가 씬 전환 한복판에서 네이티브로
    올라가면 안 된다).
  - 지금 태우는 것은 `Scene::AttachExistingGameObject`/`DetachGameObjectHierarchy`
    두 곳뿐이다. C는 **호출 지점이 늘어나는 것**이지 이 전송로가 바뀌는 것이 아니다.

  ★★ **자를 세우자마자 더 큰 결함이 나왔다 — 스크립트가 자기 오브젝트를 잃고 있었다.**
  `ClrHost::NotifySceneUnload`가 `ScriptObjectRegistry::Clear()`를 부르는데, 그 함수가
  **살아남는 DDOL 오브젝트의 핸들까지 전부 죽였다**(모든 슬롯의 세대를 올린다).
  결과: DDOL 스크립트는 살아서 계속 도는데 `GameObject`가 죽는다 —
  `GameObject.Name`이 빈 문자열이 되고 그 경유 API가 전부 무응답이 된다.
  조용하다: `SweepOrphans`가 `Clear`보다 **먼저** 돌아 그 스크립트를 고아로 잡지도
  않는다. 실측 증거 둘 — ① 이송 재부착 통지가 `IsAlive=false`로 거부됨
  ② 종료 시 훅 로그의 오브젝트 이름이 빈 문자열.
  → `Clear()`가 DDOL 오브젝트를 건너뛰게 했다. 등록 해제의 정본은 `Entity::Destroy()`
  이고(그 파일 상단 주석) 그 경로는 이송을 지나지 않으므로, 살아남는 것을 건드리지
  않아도 죽은 슬롯이 새지 않는다.

  검증(게이트 신설, 회귀 13→14): 생성 시 Awake·AddedToScene 각 1 · 이송 시
  ...
  **네이티브 `ScriptObjectRegistry::Clear`가 반대편에서 같은 짓을 하고 있었다**(핸들을
  ...
  C#으로 전달되지 않는다. `SimulationScope`의 구독 해지·재구독이 씬 이송과
  일관되려면 필요하다.

  ★ **착수 전에 풀어야 할 설계 문제 하나**(2026-08-20 조사): 훅을 그대로 전달하면
  **이중 발화**한다. ⓐ `OnAddedToScene`은 관리 측 드레인(`BehaviourRegistry`)이
  이미 부르므로 신규 생성분이 두 번 받는다. ⓑ `OnRemovingFromScene`은
  `Scene::FlushPendingDestroy`가 **모든 파괴에서** 부르는데, 그 뒤 `DestroyBehaviour`
  → 관리 측 TearDown이 같은 훅을 또 부른다. 즉 "ScriptComponent가 두 훅을
  오버라이드" 안은 그대로는 성립하지 않는다 — **DDOL 이송 경로
  (`DetachGameObjectHierarchy`/`AttachExistingGameObject`)에서만 통지**하거나,
  관리 측에 상태 가드를 두어야 한다.
...
- **예외 5 (E6·E7)**: 심볼 리네임과 저장 enum 소멸의 파일 흔적. 구파일의 클래스명
  노드(`GameObject`)는 리플렉션 읽기 별칭으로 흡수하고(K1-b UUID가 정본이 된
  뒤라 이름은 표시용일 뿐이다), 구 `m_gameObjectType` 키는 로더가 생성 힌트로만 읽는다.
  새 저장본에는 필드가 없고 부재 시 컴포넌트 조합으로 판정한다. 어느 쪽도 기존 파일을
  깨지 않는다.
