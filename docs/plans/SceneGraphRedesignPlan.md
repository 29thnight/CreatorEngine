...
1. 🔶 **자산·게이트 CLI 이전 — 착수, 6/8 완료.** 저작 자산 폐기에 앞서 게이트의
   ...
   | BT 스모크 | BTProbe + BT/BlackBoard 자산 | ⛔ **이전 불가 — CLI에 BT 저작이 없다**(아래) |
   ...
   ⛔ **BT 스모크는 CLI에 저작 표면이 없다.** `BehaviorTreeComponent`는 붙일 수
   있지만 트리 구조는 **별도 자산**(`m_BehaviorTreeGuid`·`m_BlackBoardGuid`)에 있고,
   CLI 명령은 `bt.status`/`bt.reset` **둘뿐**이다 — 관측·리셋이지 저작이 아니다
   (`ui.*`가 UI에 대해 그랬던 것과 같은 형태다). BT 그래프 저작 CLI가 서기 전에는
   옮길 수 없다.
