...
| **L** 생명주기 | 🔶 잔여 1 | L1 · L2 · **L3 본체** · L4 | L3 잔여 — DDOL 이송 신호를 C#까지(이중 발화 설계 선행) |

### 지금 열려 있는 것 (착수 가능 순)

1. **P4-c** — `PrefabOverride` 순번 필드. 골든 재기준선 동반. 다만 P4-b가 막힌 것과
   같은 토대 위에 있어 실효는 `UpdateInstances` 경로에 한정된다.
2. **L3 잔여** — DDOL 이송 신호를 C#까지. **자는 이제 다 있다** — C5가 6단계
   축을 기준선에 올렸고, `scene.ddol`이 이송 경로를 시나리오에서 태운다. 남은 건
   이중 발화를 어떻게 막을지의 설계 결정이다(위 L3 항목).
...
**13/13 전체 통과.** 이 계획 착수 시점 대비 검사 3종이 늘었다(스크립트 부착 1회 ·
중첩 프리팹 정체성 · DDOL 캔버스 재등록). 기준선 값:
