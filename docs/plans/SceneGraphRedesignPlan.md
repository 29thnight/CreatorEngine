...
착수 2026-08-16 이후 닷새째. 트랙 7개 중 **G·K·C·L·P가 완결**이다.
...
- ⬜ **P4 — 중첩 프리팹 (선택 → 핵심 승격)**: 프리팹 안의 프리팹 인스턴스를
  ...
  **6건**이다: `Bridge` · `CriticaHitEffect` · `ItemSlotPrefab2`(2단) ·
  ...
  - 🔶 **P4-c — 재개 가능** (2026-08-20 2차 정정). "골든 재기준선이 따라온다"는
    보류 사유가 §0.05 위반이었다 — 폐기할 자산의 재기준선은 비용이 0이다. 아래
    실측(중복 대상 3종·기록 0건)은 그대로 유효하고, 선행은 P4-b와 같은 **오버라이드
    기록 배선** 하나다. P3가 P4로 미뤄 둔 잔여 — 동일 타입 컴포넌트가 여럿일 때 오버라이드
    판정이 타입명 단위다(`CollectComponentOverrideNames`가 `m_componentType` 문자열로만
    모은다, `PrefabUtility.cpp:93`).
