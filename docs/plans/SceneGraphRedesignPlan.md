...
| **P** 프리팹 | 🔶 잔여 2 — **둘 다 차단** | P0~P3 · P4-a | **P4-b·P4-c 모두 차단** — 같은 벽(오버라이드 기록 부재) |
| **L** 생명주기 | ✅ 완결 | L1 · L2 · **L3 완주** · L4 · L5-a · L5-b | — |

### 지금 열려 있는 것

**없다 — 이 계획서 안에 순수하게 열려 있는 작업은 남아 있지 않다** (2026-08-20 재측정).
남은 셋은 전부 다른 문서·다른 인프라·명시 트리거를 기다린다.

1. **E7-c** — `m_gameObjectType` 제거. `UISystemRedesignPlan` U7이 선행이고 **그 문서
   자체가 개정 대상**이다(전제 5개가 이 트랙에 의해 무효화됐다 — E7 항목의 대조표).
   → 다음에 손댈 것이 있다면 **U7 개정**이지 E7-c가 아니다.
2. **P4-c** — `PrefabOverride` 순번 필드. **"착수 가능"이라는 앞선 판정은 틀렸다**
   (아래 재측정). P4-b와 같은 벽이다.
3. **S4 dirty 게이트** — 트리거 명시 보류(렌더 컴포넌트 200~400개).
...
- **P4-b · P1 잔여 · S4 dirty 게이트 — 셋이 같은 벽이다.** "값이 쓰이는 길목"이 없다:
  에디터의 리플렉션 드로어가 원시 포인터로 메모리를 직접 덮는다. 셋을 각각 풀 수
  없고, 쓰기 경로에 훅을 심거나 드로어를 get/set으로 바꾸는 선행 작업 하나가 셋을
  동시에 연다.
...
- ⬜ **P4 — 중첩 프리팹 (선택 → 핵심 승격)**: 프리팹 안의 프리팹 인스턴스를
  ...
  **6건**이다: `Bridge` · `CriticaHitEffect` · `ItemSlotPrefab2`(2단) ·
  ...
  - ⛔ **P4-b — 지금 하면 자산이 깨진다** (2026-08-20 실측, 착수 직전 중단).
  ...
  - ⛔ **P4-c — `PrefabOverride` 순번 필드: P4-b와 같은 벽이다** (2026-08-20 재측정으로
    판정 정정). P3가 P4로 미뤄 둔 잔여 — 동일 타입 컴포넌트가 여럿일 때 오버라이드
