...
착수 2026-08-16 이후 닷새째. 트랙 7개 중 **G·K·C·L이 완결**이다.
**P의 선행(오버라이드 기록)이 P-write로 완주해 P4-b가 열렸고, 그 게이트는 이미
세워져 의도적 RED 상태다**(`verify-prefab-nested-update.ps1` — 스위트 밖).
...
| **P** 프리팹 | 🔶 잔여 1 — **착수 준비 완료** | P0~P3 · P4-a · P-write(S0~S5) · P4-c · **복제 등록 결함** | **P4-b** — 선행 해소 · 게이트 세워짐(의도적 RED) |
| **L** 생명주기 | ✅ 완결 | L1 · L2 · **L3 완주** · L4 · L5-a · L5-b | — |

### 지금 열려 있는 것 (착수 가능 순)

1. **P4-b — 중첩 프리팹을 참조 노드로.** 선행(오버라이드 기록)이 P-write로 해소됐고
   **자도 이미 서 있다** — `Tools/regression/verify-prefab-nested-update.ps1`가
   의도적 RED로 대기한다(판정 6). 설계 3안 조사가 끝나 있다(트랙 P의 P4-b 절).
2. **자산·게이트 CLI 이전** — 저작 자산 폐기에 앞서 게이트 5종의 자를 CLI 저작본
...
★ **스위트 밖에 게이트가 하나 더 있다**: `verify-prefab-nested-update.ps1`.
P4-b가 미착지라 **의도적으로 RED**이고, 그래서 `run-all.ps1`에 넣지 않았다
(상시 실패 게이트는 스위트 전체를 붉게 만들어 진짜 회귀를 가린다).
P4-b가 착지해 GREEN이 되면 편입한다 — 그 RED→GREEN 전환이 곧 증명이다.
...
| **(스위트 밖) 중첩 정의 전파** | 판정 6 **RED** — `OuterAfter`의 중첩 자식 `m_isEnableLOD` false(기대 true). P4-b가 열 자리 |
