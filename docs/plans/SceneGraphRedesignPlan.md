...
| **P** 프리팹 | 🔶 잔여 1 | P0~P3 · P4-a · **P-write(S0~S5)** · **P4-c** | **P4-b** — 선행(오버라이드 기록)이 이제 서 있다 |
| **L** 생명주기 | ✅ 완결 | L1 · L2 · **L3 완주** · L4 · L5-a · L5-b | — |

### 지금 열려 있는 것 (착수 가능 순)

1. **P-write — 오버라이드 기록 배선** (신설, P4-b·P4-c의 단일 선행).
   `ApplyReflectedProperty`가 프리팹 인스턴스의 프로퍼티를 바꿀 때
   `m_prefabOverrides`에 기록한다. 지금 그 목록에 **쓰는 지점이 시딩 함수 하나뿐**이고
   (실측: `push_back` 0건), 그 시딩의 기준인 `m_prefabOriginal`이 `reflect()`에 없어
   비직렬화라 씬을 다시 열면 근거가 사라진다 — 자산을 전부 버려도 남는 코드 결함이다.
2. **P4-b** — 중첩 서브트리를 `{정의 GUID + 오버라이드}` 참조 노드로.
3. **P4-c** — `PrefabOverride` 순번 필드(대상 실재: `GameManager` `ModuleBehavior` 6개 등).
4. **자산·게이트 재기준선** — 폐기와 함께 게이트 5종의 자를 CLI 저작본 위에 다시 세운다
   (`verify-prefab-nested`·`verify-prefab-roundtrip`·`verify-reflection-golden`·
   `verify-transform-roundtrip`·`verify-authored-rects`). **폐기보다 이것이 먼저다** —
   자를 잃은 채 스키마를 바꾸면 무엇이 깨졌는지 못 본다.
5. **E7-c** — `m_gameObjectType` 제거. 자산 사유(218개 형상 변경)는 §0.05로 소멸했고,
   **코드 사유만 남는다**: `Prefab.cpp`의 `if (type != GameObjectType::UI)`가 Navigation의
   instanceID 참조를 지키고 있어 `UISystemRedesignPlan` U7이 선행이다. 그 문서 자체가
   개정 대상이다(전제 5개 무효화 — E7 항목의 대조표).
6. **S4 dirty 게이트** — 트리거 명시 보류(렌더 컴포넌트 200~400개). 선행은 **ImGui
   드로어 get/set 전환**이고, 이건 위 넷과 달리 실제로 ImGui 축이다.
...
**16/16 전체 통과.** 이 계획 착수 시점 대비 검사 6종이 늘었다(스크립트 부착 1회 ·
중첩 프리팹 정체성 · DDOL 캔버스 재등록 · DDOL 스크립트 이송 통지 · 계층 표기
불변식 · **프리팹 오버라이드 기록**). 기준선 값:
...
- ⬜ **P4 — 중첩 프리팹 (선택 → 핵심 승격)**: 프리팹 안의 프리팹 인스턴스를
  ...
  **6건**이다: `Bridge` · `CriticaHitEffect` · `ItemSlotPrefab2`(2단) ·
  ...
  - 🔶 **P4-b — 재개 가능. 아래 "차단" 판정은 §0.05 위반이다** (2026-08-20 정정).
    자산이 깨지는 것은 **폐기 정책 아래에서 비용이 아니다.** 아래 기록은 실측으로서만
    유효하고, 판정으로서는 무효다. 코드 축에서 실제로 남는 선행은 **오버라이드를 쓰는
    배선** 하나이며, 그것은 `ApplyReflectedProperty`(CLI 단일 길목)에 기록을 다는
    것으로 게이트까지 닫힌다 — ImGui는 선행이 아니다(§"막혀 있는 것" 표).
