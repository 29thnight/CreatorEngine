...
- [ ] Release cold load·warm load·reimport 시간 예산 충족 — **미충족 2026-09-03**: B1 10/14·B2 12/14 초과([기준선 §4.5](ModelAssetBigBangCutoverBaseline.md)). 소스 디코드 자체는 14/14 예산 안이고, 초과분은 (a) cooked 로드가 embedded texture PNG를 런타임에 다시 디코드하는 비용(512² 1장 ≈ 12.8 ms, scene 69장 667 ms — 로드의 93~98%), (b) 저작 트랜잭션의 corpus collision 재스캔(저작마다 ≈ 6.6 ms)·stage 재판독·source 전체 재해시(scene 35 MB 203 ms)다. B3 씬 로드(Test1 15.2 / FT 11.8 ms)·B4 부팅 31.1 ms는 통과. corpus 재임포트(14/14, 신원 324개 보존)로 전 corpus reimport 시간도 함께 쟀다 — 14개 합계 3.1 s(최소 Prim 62 ms, 최대 scene 1,755 ms).
...
- [x] frame CPU record와 GPU pass timing 비회귀 — **기준선 기록 2026-09-03**: corpus 재임포트(CEMC v4→v5, 14/14) 뒤 `dx12.scene`(FT_Primitives + Gunner)에서 메시 업로드 10개·1,075 KB가 전량 generation 경로, 커버리지 49,684, VRAM 1,145 MB를 `mbc11_perf_archive.json`에 기록했다. 이후 실행은 ×1.10 안이어야 한다(첫 기록이라 비회귀 판정은 다음 실행부터). 게이트는 B6가 붉은 실행을 archive로 굳히지 않는다.
