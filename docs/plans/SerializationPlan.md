...
D2의 로컬 old-v5 sidecar 61개는 selective transaction으로 재이주해
...
| D2 | schema v4 `non-canonical-only` manifest가 canonical 154개를 보존하고 old-v5 61개만 재발급했다. apply는 64파일/참조 76건이며 backup은 `%TEMP%/CE_D2Apply_c88550be8d0845b4aa5fcd5210fc2c29`. Strict 결과는 meta/parsed 215/215, UUIDv4 215, invalid/non-v4/missing/duplicate/policy violation 0, `d2Ready=true`. AssetCooker 결정성·실패 원자성, prefab 9, material 2, 현존 scene 8 왕복이 초록이다 | **현 로컬 코퍼스 재이주 완료 · 정본 scene 14 입력은 별도 복구 필요** |
...
**D2 — GUID 정체성 수술 (Y-1·Y-2, 2일) — ✅ 현 로컬 코퍼스 재이주 완료
(2026-09-02, 정본 scene 14 입력 별도)**
