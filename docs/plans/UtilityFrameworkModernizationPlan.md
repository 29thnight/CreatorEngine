...
| H-h 빈 문자열 정책 4갈래 | **닫힘** | H5를 "합법"으로 결정했다(아래 §H5). 어설션 4종 제거, 기본 생성자가 `""`의 해시를 갖게, `SetString`은 대입에 위임 → 갈래가 하나다 |
...
#### H5. 빈 문자열 정책 통일 — **결정: 합법** (2026-09-05)

**결정 이전에 결함이 하나 섞여 있었다.** 기본 생성자가 `= default`라 `m_hash`가
0으로 남는데 `""`의 해시는 FNV offset basis(14695981039346656037)다. 그래서
`HashingString()`과 `HashingString("")`이 **같은 내용인데 다르다고 판정**됐다 —
빈 상태가 둘이었다. 정책과 무관한 결함이므로 어느 쪽으로 정하든 고쳐야 했다.

실측:

| 항목 | 값 |
|---|---|
| 기본 생성을 쓰는 멤버 | **1곳** — `Scene::m_sceneName`. 나머지는 전부 기본값 문자열을 갖는다(`m_name{"Object"}`·`m_tag{"Untagged"}`·`m_layer{"Default"}`·`textureFilePath{"None"}`) |
| 저작 코퍼스의 HashingString 빈 값 | **0건**. 14개 씬의 `name: ""`은 전부 `AssetBundle::name`이고 그것은 `std::string`이다 |
| 현 "금지"의 강제력 | **0**. `assert`는 Release에서 사라지고, `const char*` 경로는 빈 문자열에 검사가 없으며, `SetString`은 무검사였다 |

**합법으로 정한 이유**는 강제 수단이 없어서가 아니라 강제할 자리가 여기가 아니기
때문이다. 이름·태그는 저작 UI와 역직렬화가 매 입력마다 만드는 값이고, 빈 값은
잘못된 프로그램이 아니라 "사용자가 지웠다"는 정상 입력이다. 값 타입이 던지면
호출부마다 방어가 붙는다 — 실제로 인스펙터에 가드를 하나 넣어야 했다.
"이름 없는 엔티티를 만들지 마라"는 상위 정책이고 그 층이 이미 막고 있다.

착지한 것:

- 빈 문자열 어설션 4종 제거, `<cassert>` 소비자 0이 되어 함께 제거
- 기본 생성자가 `""`의 해시를 계산한다 → 빈 상태가 하나
- `SetString`은 `operator=(string_view)`에 위임한다 → **네 번째 갈래 소멸**
- 널 포인터는 여전히 던지되 기준을 정밀화했다: `data() == nullptr && size() != 0`.
  빈 뷰(`std::string_view{}`)는 읽을 것이 없으므로 정상 입력이다
- 저작 UI의 빈 이름 커밋 가드는 **UX 판단으로 유지**한다(이름이 비면 하이어라키에서
  그 오브젝트를 다시 고를 수 없다). 타입 정책과 별개라고 주석에 적었다

게이트: 프로브의 빈 문자열 축이 **Debug에서도 돈다는 사실 자체가 강제 수단**이다 —
어설션이 되살아나면 Debug 실행이 죽어 빨개진다(변이로 확인). 정적 래칫은
`assert(...empty())` 재도입과 `= default` 복귀를 함께 막는다.

**사각지대**: `data()==nullptr && size()!=0` 가드는 런타임으로 잴 수 없다. 그런
뷰를 만드는 것 자체가 UB라 MSVC 디버그 STL이 `string_view` 생성자에서 먼저
죽는다(시도했다가 게이트가 STATUS_BREAKPOINT로 죽었다).
