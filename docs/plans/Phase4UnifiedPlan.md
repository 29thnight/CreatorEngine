...
**신설 2026-09-01 · 재분할 2026-09-03 · PHASE 4.5 분리 2026-09-14 · 활성 69행 352.5일 · 완료 12.5일 + 진행 기성 3.5일 · 잔여 336.5일**

기존 단일 PHASE 4에는 현재 PBR 배선 수정, Blender형 Material Graph, RenderGraph·라이트맵·
일반 SRP·후처리·차세대 GPU 기능이 한데 섞여 있었다. 이 문서는 그것을 네 완료선으로
분리한다.
...
| **4.5** | 모션 벡터·jitter·히스토리와 Temporal Upscaling·Frame Generation | 17 | 92 | 미착수 |
| **4.75** | RenderGraph·라이트맵·일반 SRP·shadow/probe/post·GPU 기능 | 32 | 208.5 | 4일 완료 |
...
| 범위 | 정본 |
|---|---|
| PHASE 4/4.25/4.5/4.75 순서·공수·완료선 | 이 문서 |
...
- `BASE-0` 공통 밀봉 하네스 — PHASE 4.75 공통·GPU 설계 게이트(현 §7.1)에서 이관.
...
이 페이즈가 4.75보다 앞에 서는 이유는 두 가지다. 첫째, `BASE-0` 계측 하네스가 여기서
**계측 무해화 계약**까지 확정돼야 이후 모든 렌더 게이트가 해상도와 프레임 종류를 선언한다.
둘째, 모션 벡터는 RenderGraph 재작성과 독립이며 TAA·모션 블러·SSGI 품질 개선의 공통 입력이라
`RG`를 기다릴 이유가 없다.

**벤더 이름을 기능 이름으로 쓰지 않는다.** 구 `4-5 DLSS 구상`은 이 페이즈로 흡수되며 DLSS는
`TU3`/`FG3` 백엔드 하나의 이름으로만 남는다.

### 1.4 PHASE 4.75 — 남은 renderer/SRP/GPU 기능

기존 PHASE 4에서 위 두 범위가 아닌 항목을 모두 이곳으로 이동한다.

- GPU-driven/Stochastic Lighting/DXR 설계 게이트 (`BASE-0`은 PHASE 4.5가 소유한다).
...
PHASE 4.5   BASE-0
              ...
              └─ TR1 → TR2 → TR3 → TU0 → TU1 ∥ TU2 → TU3 ∥ TU4 ∥ TU5
                                                          └─ FG0 → FG1 ∥ FG2 → FG3 ∥ FG4
                                                                              → TFG9
    ↓
PHASE 4.75  (BASE-0을 PHASE 4.5에서 입력으로 받는다)
...
PHASE 4.5는 PHASE 4.75의 `RG6`·`Q0`를 선행으로 받지 않는다. 자체 보간기를 만들지 않으므로
...
PHASE 4.75 공통·GPU 설계 게이트(현 §7.1)에서 이관했다. PHASE 4.75는 이것을 입력으로 받는다.

### 6.2 트랙 TR — 시간축 기반, 25일
...
## 7. PHASE 4.75 — 남은 렌더 파이프라인, 208.5일

### 7.1 공통·GPU 설계 게이트 — 7.5일
...
| `4-6` | 공통 의존 그래프·수직 슬라이스·구현 페이즈 | · | 4-2~4-4 | 0.5 |

구 `4-5 DLSS 구상`은 PHASE 4.5로 분리됐다. `4-6`의 의존 그래프는 PHASE 4.5가 확정한
시간축 입력과 업스케일 계약을 **읽기 전용 입력**으로 받는다.

### 7.2 RenderGraph·queue — 113일 + Q0 미산정
...
`Q0`은 0일 표기가 완료가 아니라 미산정이다. `4-2~4-4`도 구상 공수만 포함하며 실제 GPU
기능 구현 공수는 `4-6` 뒤 추가한다. 352.5일은 새 기능 전체 완료를 보증하는 상한이 아니다.

267.5일에서 352.5일로 늘어난 85일은 PHASE 4.5 92일에서 구 `BASE-0` 6일과 구 `4-5` 1일을
뺀 값이다. **이 증가분의 대부분은 벤더 SDK 통합이 아니라 모션 벡터·해상도 계약·계측
