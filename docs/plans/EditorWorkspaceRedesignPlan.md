...
- 오른쪽을 Hierarchy/AssetBundle/Content Browser/Inspector로 다시 분할한다.
...
### 1.4 표시 문자열이 persistence ID다

`DockBuilderDockWindow`와 `ImGui::Begin`이 아이콘, 공백, 표시 이름을 포함한 문자열을 그대로
ID로 쓴다. 예를 들어 Scene/Game 이름의 뒤쪽 정렬 공백까지 같아야 한다. 아이콘·번역·간격만
바뀌어도 `imgui.ini`의 창 정체성과 dock 위치가 끊길 수 있다.
...
모든 상태는 최초 `todo`다. 문서 작성은 구현 진행으로 세지 않는다. 총 초기 추정은 **23일**이며,
W0 기준선 뒤 조정한다.

### W0 — 현 레이아웃·성능·시각 기준선과 실패 게이트 (P0, 1일)

- 현재 `imgui.ini`, 핵심 window title/flags, open state inventory를 고정한다.
- 1920×1080/2560×1440, 100%/150% DPI의 shell screenshot을 캡처한다.
- Editor UI CPU, ImGui vertices/indices/draw commands, target별 GPU ms를 기록한다.
- layout fixture가 실제로 깨졌을 때 실패하는 `verify-editor-workspace.ps1` canary를 만든다.

**판정:** 빈 측정·빈 screenshot으로 통과하지 않으며, legacy layout과 performance baseline이 이
문서 또는 별도 analysis 산출물에 기록된다.
...
- logical px × DPI × user scale 단일 경로를 만든다.

**판정:** token sample은 기준 hex와 일치하고, font file 부재로 editor가 뜨지 않는 경로가 없으며,
100↔150% scale 왕복에서 double scaling/blur/layout drift가 없다.
...
- persistent window ID를 `###Editor.*` 상수로 통일한다.
- ToolPanel의 `NoMove`와 implicit `AlwaysAutoResize`를 제거한다.
- 일반 panel close/open과 Window 메뉴 재열기를 연결한다.
...
사용자 파일을 잃지 않고 기본 preset으로 복구된다.
...
- 기존 Editor/Game presentation key를 mode별로 소비한다.
- optional Game Preview와 visible target demand를 연결한다.
...
### W5 — Play/Pause/Eject 표시·입력 상태 머신 (P0, 3일)
...
- gizmo/picking/game input/cursor/focus의 단일 owner를 만든다.
- Stop 뒤 prior document/focus/selection 정책을 복원한다.

**판정:** `verify-play-roundtrip.ps1`을 유지하면서 viewport target과 input owner 단정을 추가하고,
Play 실패·Alt-Tab·Eject·Pause·Stop 반복에서 cursor/gizmo가 잘못 남지 않는다.
...
- visible-row clipping, lowercase/search cache, icon/label formatting cache를 측정 기반으로 적용한다.
- `HierarchyStore` 단독 정본 불변식을 source gate로 고정한다.
...
- `Tools/regression/verify-editor-workspace.ps1`
- `Tools/regression/verify-editor-theme-golden.ps1`
- `Tools/regression/verify-editor-viewport-mode.ps1`
...
- Play/Stop transaction이 실제로 일어났음
...
6. title/icon/번역 변화가 workspace identity를 깨뜨리지 않는다.
...
12. PHASE 21 완료 표시는 위 구현·runtime gate 뒤에만 갱신한다. 이 문서 작성만으로는 0%다.
