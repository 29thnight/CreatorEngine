...
W7-5 를 닫으면 이 수치가 따라 내려간다. 되돌릴지, W7-5 뒤로 미룰지, 그대로 둘지는
사용자 결정 사항으로 남긴다 — 게이트는 어느 쪽이든 수치를 이름과 함께 낸다.
...
> **2026-09-15 — 우선순위가 올라갔다.** 사용자 요청으로 Content Browser 가 기본 배치의
> 앞 탭이 되면서(W2-4 절 끝) 이 비용이 **기본 상태에서 상시** 든다. W2-4 의 창 단위 장부로
> 다시 재니 `###Editor.ContentBrowser` 혼자 **2.446 ms · chrome 프레임의 82.6%** 이고,
> UI CPU 중앙값이 W0 기준선의 **9.6 배**다. 이전에는 이 창이 뒤 탭이라 그려지지 않아
> 보이지 않았을 뿐, 비용 자체는 그대로였다. W7-5 는 이제 "언젠가" 가 아니라 기본 배치의
> 성능을 직접 정한다.
