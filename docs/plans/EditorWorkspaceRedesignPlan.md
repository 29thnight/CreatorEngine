...
**문제.** 창 24개가 두 기구로 갈려 있다. `ContextRegister` 10곳은 표를 거쳐 그려지고,
...
| M4 | 창 선언 어휘와 셸의 프레임 소유 · 창 24개 이관 · 게이트와 `editor.windows` 덤프 · 변이 증명 | 3일 |
| | 합계 | **4.5일** |

M4의 3일은 내역이 있다. 선언 어휘와 셸의 프레임 소유가 1일, 창 24개 이관이 1.5일, 게이트와
