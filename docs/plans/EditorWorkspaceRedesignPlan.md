...
| W2-I | **done** | **여섯 줄 전부 닫혔다(2026-09-18).** RectTransform·중첩/배열·전용 드로어 15/15·Import Settings·활성 정책·편집/저장 회귀가 모두 착지했고, W2-I5 가 폭 × 배율 행렬(단정 3144 · 배율 넷 × 폭 넷 · 전환 경계 18)과 편집 왕복(단정 40)으로 회귀를 건다. 그 자가 넘침 0 뒤에 숨어 있던 **1 px 입력칸** 넷을 찾아 고쳤다. ★ **2026-09-16 재기준선** — 잔여를 문장에서 **이름 열둘**로 바꿨다. 그 과정에서 자를 한 번 고쳤다: `EditorPropertyRow` 는 파일 이름이고 코드가 부르는 어휘는 `editor::widgets::` 이며, 전용 드로어 열은 별도 파일이 아니라 `InspectorWindow.cpp` 안의 멤버라 **파일 단위 세기가 12 를 0 으로 읽었다.** 일반 리플렉션·C# 노출 필드 경로는 이미 정본을 지난다 |
...
| W2-I3 | 일반 리플렉션·중첩 필드·공유 드로어 이관 | 1일 | 수치·문자열·bool·enum·벡터가 같은 규칙을 사용하고 Inspector 밖 소비자도 회귀 없음 | done — **일반 경로와 C# 노출 필드는 정본을 쓴다**(`ReflectionTypedDraw.h`·`EditorAxisField3`·`DrawManagedScripts` 5 건). **2026-09-17 중첩/배열도 닫았다** — 합성 자극물 줄 31 → 55(타입 정의에서 센 값), 240 넘침 323 → 0 px. 원인 하나는 공통 줄 정본의 항목 폭이었다(아래 절). **2026-09-18 W2-I5 가 나머지를 닫았다** — 원소 줄이 버튼 몫을 판정 뒤에 빼 240 폭에서 값 칸이 1 px 이던 것과 `drag_property_floats` 의 전환 부재를 고쳤고, 배율 넷 × 폭 넷 행렬(단정 3144)과 편집 왕복(단정 40)이 `Inspector 밖 소비자`(Import Settings·C# 노출 필드)를 드로어 검사 15/15 와 함께 회귀로 잡는다 |
| W2-I4 | 전용 컴포넌트·자산 Import Settings 이관 | 1.5일 | Sound 등 고정 폭 제거, 긴 참조·보조 버튼·허용된 두 열 묶음·저장 동작 확인 | done — **전용 드로어 15/15 · Import Settings 까지 이관.** 아래 재기준선의 표가 이름을 전부 적는다. Import Settings 의 대상 파일은 인스펙터가 아니라 `DrawYamlNodeEditor.cpp` 다. **2026-09-17 자를 열었다** — `editor.inspector width` 와 `verify-inspector-drawer-layout.ps1`(이관 목록에만 단정, 착수 기준선은 아래). **같은 날 전용 드로어 열둘 이관 — 14/14, 네 폭 넘침 0, 소스 축 고정 폭 0.** 이어서 Import Settings(`DrawYamlNodeEditor.cpp`)도 옮겼다 — 15/15, 펼친 채 네 폭 넘침 0(`editor.inspector expand on`). **W2-I4 의 대상 표면 0**. **2026-09-18** 완료 기준의 마지막 축인 *"저장 동작"* 을 `verify-inspector-edit-roundtrip.ps1` 이 실물로 잰다 — 값 칸을 눌러 끌고 씬 저장/재로드·프리팹 소환에서 값이 보존되는지 본다 |
| W2-I5 | 폭·배율·편집·중복 렌더 회귀와 잔재 점검 | 1일 | 아래 검증 행렬 및 §8 성능 gate 충족, 미이관 표면 0 | done — **2026-09-18 행렬·경계·편집 왕복을 자로 열었다**. 값 칸 장부가 넘침 0 뒤에 숨어 있던 **1 px 입력칸**과 전환하지 않는 `vector3` 를 찾아 고쳤다. `verify-inspector-layout-matrix.ps1`(단정 3144 · 배율 4 × 폭 4 · 전환 18) · `verify-inspector-edit-roundtrip.ps1`(단정 40) · 변이 8 종(아래 절) |
...
##### 2026-09-18 W2-I5 폭 × 배율 행렬 · 전환 경계 · 편집 왕복

★ **넘침 0 은 "칸이 있다" 가 아니었다.** 배치 검사 셋이 전부 초록인 채로, 240 폭에서
배열 원소의 입력칸이 **1 px** 이고 일반 리플렉션 경로의 `vector3` 칸이 **24 px** 였다.
넘침은 `본문 오른쪽 끝 − 작업 영역 끝` 이라 칸이 **안쪽으로** 눌리는 결함에는 언제나 0 이다.
판정문은 *"숫자 최소 폭"* 과 *"0-size 입력칸은 실패"* 를 따로 적어 두었는데 자가 하나뿐이었다.

값 칸 장부를 열었다(`editor.inspector` 의 `fields`·`axisFields`·`minFieldWidth`·
`minAxisWidth`·`minLineValue`·`fieldDigest`·`narrowestField`, 하한은 `valueMin`·
`axisValueMin`). 하한은 폰트에서 나오므로 **배율을 저절로 따라간다** — 검사가 논리 px
리터럴을 들지 않는다. 가장 좁은 칸이 있던 **줄 이름**도 함께 낸다(수만 있으면 "어딘가
1 px" 에서 멈춘다).

그 자가 찾은 결함 넷을 고쳤다 —

- **배열 원소 줄이 버튼 몫을 판정 뒤에 뺐다.** `^`·`v` 두 개를 전환 판정 **뒤**에 빼니 값 칸이 최소 가독 폭 아래로 눌려도 줄이 내려가지 않았다(240 폭에서 1 px). 버튼 수를 `aux_button_count` 로 넘겨 판정이 그 몫을 알게 했고, 줄의 배치를 **그 줄이 실제로 선 자리**에서 다시 잰다(원소는 접힘 머리 안이라 들여쓰기만큼 좁다).
- **보조 버튼 예약이 버튼 사이 간격을 빼먹었다.** `aux_reserve` 가 `FrameHeight × 개수` 뿐이라 `SameLine(0, inner)` 의 간격만큼 모자랐고, 원소 줄이 정확히 `버튼 수 × inner` 만큼 넘쳤다.
- **`vector3` 는 축 전환을 묻지 않았다.** 값 타입 드로어(`InspectorDrawer<math::vector3>`)가 `stacked` 를 늘 거짓으로 넘겨, 240 폭에서 칸 셋이 24 px 였다. 판정을 위젯 안에 **바닥**으로 넣었다 — 계약이 쓰는 식과 같으므로 계약을 지킨 호출자에게는 변화가 없다.
- **`drag_property_floats` 도 전환이 없었다.** 한 줄을 둘·넷이 나누는 위젯인데 언제나 가로로 나눴다. 축 위젯과 같은 하한으로 내려가게 했다.

**행렬 검사**(`verify-inspector-layout-matrix.ps1` · 단정 3144): 사용자 배율 1·1.25·1.5·2
(이 기계의 모니터 1.5 와 곱해 실제 1.5·1.88·2.25·3.0) × 폭 240/320/480/720 에서
넘침 0 · 값 칸 ≥ `valueMin` · 축 칸 ≥ `axisValueMin` · 줄 폭 ≥ `axisValueMin` 을 단정하고,
**논리 불변**을 건다 — 같은 논리 폭에서 잰 칸 폭을 실제 배율로 나눈 값이 배율 사이에 같아야
한다(배율을 받지 않는 고정 픽셀이 섞이면 이 비율이 깨진다). 신원(`fieldDigest`)은 한 회차
안에서 네 폭이 같아야 한다 — 계획서가 주입하라고 이름까지 적은 *"전환 시 ID 변경"* 이 이
단정의 대상이다. **전환 경계**는 224~432 논리 px 를 4 px 걸음으로 훑어 높이가 바뀌는 자리를
찾고(18 곳) 그 **양쪽 모두**에 같은 단정을 건다.

**편집 왕복 검사**(`verify-inspector-edit-roundtrip.ps1` · 단정 40): 값 칸을 **실제로 눌러
끈다**(`editor.nav pointer|press|release`, 좌표는 장부가 낸 첫 값 칸의 사각형). 편집 중
경계 너머로 폭을 바꿔도 배치가 **전환되지 않고**(계약: 편집 중 보류) `activeId` 와 신원과
값이 그대로이며, 손을 뗀 뒤에야 좁은 배치로 넘어간다 — 보류와 해제를 둘 다 단정한다.
끌어 바뀐 값을 `undo` 가 되돌리고 `redo` 가 되살리며, 선택 변경·접기/펼치기·엔티티 활성
왕복과 씬 저장/재로드·프리팹 소환에서 값이 보존된다.

자를 세우며 걸린 것 셋 —
- ★ **포인터 주입은 좌표만 덮고 이동량은 실제 마우스의 것**이었다. hover 와 클릭은 좌표만 보므로 멀쩡했고(`activeId` 도 섰다) **끄는 것만 조용히 죽어 있었다** — `DragBehavior` 가 보는 것은 `io.MouseDelta` 다. 주입이 이동량도 만들도록 고쳤다(W2-3 이 세운 창구의 구멍이었다).
- 같은 자리를 잇달아 누르면 **두 번 클릭**이 되어 칸이 글자 편집으로 들어가고, 그러면 끌어도 값이 안 변한다. 회차 사이에 포인터를 치우고 자리를 옮긴다.
- 폭을 정한 채로는 **키로 칸에 들어갈 수 없다** — 폭 창구가 자식 창이고 자식 창은 ImGui 에서 별도의 탐색 범위다(Tab 열 번에 `navId` 가 한 번도 안 움직였다). `ImGuiChildFlags_NavFlattened` 는 스크롤하는 자식에 쓸 수 없어 길이 아니다. 그래서 편집 자극은 포인터로 하고, 키보드 탐색 계약은 폭 창구 없이 도는 `verify-editor-keyboard-nav` 가 계속 진다.

★ **이 자가 재는 것은 배치지 "화면에 보이는가" 가 아니다.** 인스펙터 도크 패널의 폭은 창을
2400·3400·3800 으로 키워도 **505 px 그대로**다(도크 노드는 절대 폭을 지킨다). 요청이 그보다
넓으면 본문은 그 폭으로 배치되지만 패널 밖은 잘려 화면에 없다 — 배치 계산은 잘림과 무관하니
위 단정은 그대로 성립하되, 사용자가 그것을 본다는 주장은 이 자의 것이 아니다. 자가 스스로
그 사실을 내도록 `visibleWidth` 를 함께 읽어 회차마다 적는다. 편집 왕복 검사는 **보이는 폭
안에서만** 자극한다(잘린 자리를 누르면 아무 일도 없다 — 처음에 그렇게 눌러 `activeId` 가
서지 않았다).

변이 —
- `element-reserve-after-decision` — 원소 줄의 버튼 몫을 판정 **뒤**에 뺀다 → 240·244·248·252 에서 `[0]` 줄의 축 칸이 50~71 px (하한 73). 붉다.
- `axis-floor-removed` — 축 위젯 안의 바닥을 걷는다 → 320 에서 41 px, 경계 224~232 에서 67~71 px. 붉다.
- `multifield-always-horizontal` — `drag_property_floats` 의 전환을 늘 거짓으로 → `Plane` 이 240 에서 31 px, `Lightmap Offset` 이 252~260 에서 60~66 px. 붉다.
- `aux-reserve-drops-spacing` — 예약에서 버튼 사이 간격을 뺀다 → 폭 넷과 경계 전부에서 **넘침 9 px**(= 버튼 수 × `ItemInnerSpacing`). 붉다.
- `label-column-ignores-scale` — 라벨 하한을 논리 px 리터럴로 → 배율 2 에서 축 칸의 논리 폭 63.3 이 배율 1 의 118.7 과 다르다. **논리 불변**이 잡는다. 붉다.
- `field-id-follows-mode` — 값 칸 ID 에 배치 모드를 섞는다 → 252~268 에서 신원이 224 폭과 다르다(계획서가 이름까지 적은 *"전환 시 ID 변경"*). 붉다.
- `editing-hold-removed` — `inputs.editing` 을 늘 거짓으로 → 편집 중인데 배치가 306 → 204 로 전환됐다. 붉다.
- `inspector-edit-skips-undo` — 인스펙터 끌기가 되돌리기 기록을 건너뛰게 → undo 뒤 값 4.7 이 기준 1.5 과 다르고 선택 왕복에서도 어긋난다. 붉다.

복원 빌드 뒤 두 검사를 각각 두 회차 돌려 전부 통과(`matrix-final1/2` · `edit-final1/2`, 배율 1·2 × 폭 4 · 단정 2612).
못 잡는 변이: 화면 픽셀 축. 이 두 검사는 잘림·겹침을 **수**로만 본다 — 실제 그림이 어긋나는
결함은 W8-2 의 chrome 골든과 사람의 확인 몫이다.
같이 돈 검사: 드로어 배치(단정 284 · 이관 16/16) · 공간 정책(85) · 잘라 그리기(46) · 상태 행렬(54) · 키보드 탐색(28) · 테마(64, dx12) · CLI 등록 골든(133 = 133) · CLI 발견 · Content Browser 탐색(130) 전부 통과. §8 성능 계약(`verify-editor-chrome-perf`)의 **성능 축**도 통과 — 인스펙터 avg 0.008 ms, 선택 시 0.055 ms(+0.047), 프레임 avg 0.594 · p95 1.082 ms, 가장 비싼 창은 Content Browser 0.271 ms(프레임의 44.0%)

★ **§8 게이트는 W2-I5 착수 전부터 그림 쪽이 붉어 있었다.** `verify-editor-chrome-perf` 의
성능 단정은 전부 통과하는데 정점 골든이 5602 → 5622(인덱스 +30 · draw +2)에서 막혔고,
`verify-editor-chrome-golden` 도 136 화소에서 막혔다. 이번 변경분 아홉 파일을 **되돌려
다시 빌드해도 같은 수치**가 나오므로 W2-I5 의 결과가 아니다. 아이콘 자산을 `497ad854^`
판으로 되돌려도 같아 아이콘 교체도 아니다. 골든은 09-17 07:13(`686811dd`)에 뜬 것이고
그 뒤 스무 커밋 동안 아무도 이 게이트를 돌리지 않았다. 차이는 눈으로 확인했다 — Content
Browser 검색창의 *"Search this folder"*, 계층 아이콘 경계, 하단 글자의 **가장자리 1 px**
네 덩어리뿐이고 형태가 바뀐 곳은 없다. 두 골든을 다시 떠 게이트를 되살렸고(각각 연속 두
회차 초록: 성능 단정 59 · 그림 25) **어느 커밋이 움직였는지는 별도로 추적한다**..
...
|---|---|---:|---|---|
| W2-V0 | canvas 소비 계약·묶음 측정·표시 모드 계산 | 0.5일 | 같은 크기에서 창 원점이 달라도 상대 배치 동일, W4 정본 외 좌표식 없음 | **done**(2026-09-18) — 완료 기준 두 줄을 읽는 자를 세우며 정본이 둘이던 것이 드러나 셋을 정리했다: `LayoutViewportCanvas` 가 `valid` 와 무관하게 content 를 채우고, 씬뷰의 사설 `imageMin` 사각형을 걷고, 오버레이 `Draw` 가 캔버스만 받는다. 판정은 `verify-scene-viewport-canvas-contract.ps1`(단정 170 · 변이 9/9 포획) — 소스 축은 허용 **횟수**까지 적고 창 프레임 높이 호출을 금지하며, 런타임 축은 창 이동(절대 불변)과 배치 preset(원점이 실제로 움직이는 자극) 둘이다. ★ 창 이동만으로는 뷰포트 좌표에 자극이 닿지 않는다 — 아래 절 |
...
##### 2026-09-18 예열 창구 — 진행이 화면에만 있었다

W2-V0 의 남은 판정을 세우려고 `editor.sceneview` 를 탐침했더니 **캔버스가 통째로
무효**였다(`imageReady=false`, 사각형 전부 0, aspect 1). 같은 프레임의
`editor.viewport` 는 멀쩡했다 — 모드 scene, 게시 프레임 7121, 캔버스 1164x433.
오버레이가 든 사각형도 그 캔버스와 정확히 같았다. 제품 결함으로 읽기 쉬운 그림이다.

아니었다. **탐침이 너무 일찍 물어본 것**이다. 그런데 그 사실을 알 방법이 없었다 —
예열에 대해 답할 수 있는 것은 `render.live.wait` 의 "끝났는가" 뿐이었고, 어디까지
갔는지는 임시 계측을 붙여야만 보였다. `editor.viewport` 의 `publishedFrames 7121`
은 호스트가 게시한 **UI 프레임**이라, 라이브 렌더가 도는 것처럼 읽히기까지 했다.

★ **진행이 화면에만 있었다.** `BootProgress` 는 로딩창에 퍼센트를 그리지만 밖에서
읽을 수단이 없고(정적 private), 창을 보인 뒤(`Complete()`)로는 아무것도 추적하지
않는다 — 정작 긴 구간이 그 뒤에 있다.

그래서 예열 장부(`Engine/Utility_Framework/WarmupLedger.h`)와 `GET /warmup` 을 열었다.
단계 여덟(`process` · `window_shown` · `first_ui_frame` · `render_pipeline` ·
`shader_meta` · `first_live_frame` · `display_texture` · `scene_canvas`)과 각 도달
시각, 마지막 도달 단계, **그 뒤 흐른 시간**(`sinceLastMs` — "지금 무엇을 기다리는가"),
완료 여부를 낸다. 도달하지 않은 단계도 목록에 남긴다 — 도달한 것만 실으면
"무엇이 남았는가" 에 답할 수 없다.

★ **창구가 `/health` 옆에 서는 이유**는 그 자리라야 답할 수 있기 때문이다. 예열
구간은 게임 스레드가 실제로 멈춰 있는 시간이고(렌더 스레드의 파이프라인 구축이
표시 락을 쥐어 씬뷰 본문을 통해 게임 스레드까지 멈춘다), 큐를 타는 창구
(`POST /command`)로는 **끝난 뒤에야** 답을 받는다. 장부는 원자 배열만 쓰고
게이트웨이 구현도 아무것도 기다리지 않는다.

Release 실측(이 기계, 셰이더 캐시 warm) —

| 단계 | 도달 | 직전 단계로부터 |
|---|---:|---:|
| `window_shown` | 1,627 ms | 1.63 s |
| `first_ui_frame` | 1,664 ms | 0.04 s |
| `render_pipeline` | 9,301 ms | **7.64 s** |
| `shader_meta` | 11,220 ms | 1.92 s |
| `first_live_frame` | 18,637 ms | **7.42 s** |
| `display_texture` | 18,756 ms | 0.12 s |
| `scene_canvas` | 18,756 ms | 0 s |

2026-09-14 실측(창 1.7s · 첫 그림 18.5s)과 맞는다. 이제 그 수를 **밖에서 읽는다**.

자를 세우며 걸린 것 —
- ★ **찍는 자리를 한 번 틀렸다.** `first_live_frame` 을 CPU 톤맵 경로의
  `view.completedFrameId` 옆에 두었는데 DX12 는 그 길을 지나지 않아(텍스처를 직접
  공유한다) 190 초를 돌려도 끝내 미도달이었다. `render.live.wait` 의 판정이 읽는
  `renderCompletedFrameId` 자리로 옮겨야 둘이 같은 사건을 말한다. **창구 자신의
  설계(미도달도 싣는다)가 자기 계측의 오배치를 잡았다.**
- 두 단계를 따로 둔 이유: 표시 텍스처가 0 이면 캔버스가 무효가 된다. 한 단계로
  묶으면 "그림이 없다" 와 "자리가 없다" 를 가를 수 없다.

검사 `verify-editor-warmup-endpoint.ps1`(단정 ~570 · 표본 70 여 · 예열 도중 표본
70 여 · 최악 응답 **89~101 ms**) · 변이 6 종 —

- `stages-only-reached` — 도달한 단계만 싣는다 → 실린 단계 3 개. 붉다.
- `mark-overwrites` — 매번 덮어쓴다 → `first_ui_frame` 시각이 1978 → 2292 → 2560 → 2820 ms 로 자란다. 붉다.
- `complete-always-true` — 완료를 늘 참으로 → 예열 도중 표본 0, 도달 3/8. 붉다.
- `warmup-waits-on-game-thread` — 창구가 게임 스레드 큐를 타게 → 예열 도중 응답 **5019 ms**. 붉다. **이 창구의 존재 이유가 이 변이다.**
- `live-frame-wrong-path` — 지점을 CPU 톤맵 경로로 되돌림 → 7/8, `first_live_frame` 미도달. 붉다(실제로 저지른 오배치).
- `last-by-index` — 마지막 단계를 **번호**로 고른다 → **통과했다.** 이 기계의 예열은 순서대로 진행되어 번호 최대와 시각 최대가 늘 같고, 둘이 갈리는 것은 단계를 건너뛴 실행뿐인데 그런 실행을 만들 자극이 없다. 단정은 계약상 옳으므로 남기되 **못 재는 변이**로 적는다.

못 잡는 축: 예열 **앞** 구간. 서비스는 창이 뜬 뒤에 열리므로(`InitializeFromCommandLine`
이 `Show()` 뒤다) 그 전에는 조회할 수 없다 — 장부가 시각을 남겨 나중에 읽힐 뿐이다.


##### 2026-09-18 — 정본이 답할 수 있는 것을 답하게 하고, 그 판정을 세웠다

W2-V0 의 완료 기준은 두 줄이었다: *"같은 크기에서 창 원점이 달라도 상대 배치 동일,
W4 정본 외 좌표식 없음"*. 구현은 2026-09-16 에 닫혔지만 **둘 다 읽을 자가 없었고**,
읽을 자를 세우자 정본이 조용히 둘이 되어 있던 것이 드러났다.

`LayoutViewportCanvas` 가 소스 픽셀이 없을 때 통째로 조기 반환해 `content` 까지 0 으로
돌려주고 있었다. content 는 모드가 **받은 자리**라 소스와 무관하게 늘 아는 값인데도
그랬다. 그래서 씬뷰가 `imageMin` 이라는 이름의 지역 content 사각형을 따로 만들어
배경·안내문·드롭 영역·오버레이에 넘겼고, 오버레이는 그 사각형과 캔버스를 **둘 다**
받으면서 배치는 인자 쪽으로 했다. 값이 같아 보이는 동안에는 드러나지 않는다 —
Scene 이 `fill` 이라 content 와 image 가 같기 때문이고, 정책이 갈리는 순간(crop 은
image 가 content 를 넘는다) 툴바가 화면 밖으로 나간다.

셋을 정리했다. ① `LayoutViewportCanvas` 가 `valid` 와 무관하게 content 를 채운다
(`valid` 의 뜻은 그대로다 — "그릴 소스가 있고 image/clip/uv 가 섰는가"). ② 씬뷰의
사설 사각형을 걷고 전부 `m_canvas` 를 경유한다. ③ 오버레이 `Draw` 가 사각형 인자를
받지 않는다.
...
첫째, 런타임 축을 `SetWindowPos` 로 창을 옮겨 재려 했는데 **뷰포트 좌표가 꿈쩍도
하지 않았다**. ImGui 의 화면 좌표는 OS 창의 클라이언트 좌표계라서, 창을 (40,40) 에서
(260,150) 으로 옮겨도 content 원점은 양쪽 다 (0,99) 다. 그런데 검사는 초록이었다 —
"원점 두 곳" 을 셀 때 **내가 요청한 좌표**를 세고 있었기 때문이다. `GetWindowRect` 로
실제 창 자리를 되묻는 검산과 "content 원점이 둘 이상인가" 단정을 넣자 붉어졌다.

그래서 축을 둘로 나눴다. **축 A**(창 이동)가 지켜야 하는 것은 상대 배치가 아니라
**절대 좌표 그대로**다 — 한 칸이라도 움직이면 어딘가가 창 프레임을 읽고 있다는 뜻이고
그것이 규약 ①이 금지한 출처다. **축 B**(배치 preset)가 원점을 실제로 옮긴다:
`legacy_unity` 는 왼쪽에 패널이 붙어 content 원점 x 가 519 오른쪽으로 가고,
`ui_editing` 은 오른쪽 끝이 115 안쪽으로 온다. 둘이 **함께** 있어야 이빨이 선다 —
`legacy_unity` 만 쓰면 절대 좌표가 오른쪽 거리를 우연히 지켜(두 배치의 `contentMax.x`
가 같다) 통과한다.

요소마다 정렬 기준이 다르므로(왼쪽 툴바는 왼쪽에, 뷰 큐브는 오른쪽에 붙어
`contentMax.x - gizmoCenter.x` 가 세 배치 모두 103.5) 왼쪽 거리·오른쪽 거리·정규화
셋을 다 재고 하나라도 보존되면 통과시킨다. 절대 좌표를 굳히면 셋이 전부 깨진다.

둘째, 소스 축의 **면제 단위가 강제 단위와 달랐다**. 강제는 호출 지점에 걸리는데
허용 목록은 (파일, 함수, 이름) 이라 그 함수 안에 같은 호출을 하나 더 넣으면 통과한다.
그리고 제목표시줄 보정 검사가 `GetFrameHeight()` 와 좌표 이름이 **한 줄 안에** 같이
있는지를 보고 있어, 변수에 담아 다음 줄에서 빼면 어느 줄도 두 조건을 함께 만족하지
않았다. 허용 목록에 **호출 횟수**를 적고, 보정을 쓰는 줄 대신 보정을 **만드는 호출**
(`GetFrameHeight` 계열)을 면제 없이 금지해 둘 다 닫았다.

검사 `verify-scene-viewport-canvas-contract.ps1`(단정 170) · **변이 아홉 중 아홉 포획**.
소스 축을 고치기 전에는 그 둘이 통과했다 — 처음 배치에서 통과한 변이가 곧 검사의
구멍을 가리킨 셈이다.



###### 같은 날 좌표계가 바뀌었다 — 계약이 전제에 묶여 있었다

축 A 를 "절대 좌표 그대로" 로 적고 몇 시간 뒤, 다른 트랙이 멀티뷰포트
(`ImGuiConfigFlags_ViewportsEnable`)를 켰다. 단정 아홉이 한꺼번에 붉었다.

ImGui 는 메인 뷰포트의 `Pos` 를 좌표 원점으로 쓴다. 꺼져 있으면 그것이 (0,0) 이라
화면 좌표가 곧 클라이언트 좌표지만, 켜면 플랫폼 백엔드가 창의 실제 화면 위치를
넣는다. 그때부터 제품이 내는 **모든 화면 좌표**가 가상 데스크톱 기준이 된다.

| | 멀티뷰포트 끔 | 켬 |
|---|---|---|
| `editor.sceneview` contentMin | (0, 99) | (480, 305) |
| `editor.dock` statusCountsSlot | (527, 1620) | (1007, 1826) |

이동량은 **창 이동량 × dpiScale** 이다(창 +220 → 좌표 +330, 배율 1.5).

제품은 내내 옳았고 틀린 것은 계약이었다. W2-V0 의 완료 기준도 "절대 불변" 이
아니라 *"상대 배치 동일"* 이다. 축 A 를 상대 배치로 되돌리고, **좌표계 자체를
단정한다** — `editor.theme` 의 `osViewportsEnabled` 를 읽어 꺼짐이면 좌표 불변을,
켜짐이면 창 이동량 × 배율만큼의 이동을 요구한다. 어느 쪽이든 통과시키면 전환이
조용히 일어나고, 그때 이 축이 무엇을 재고 있었는지 아무도 모른다.

역설적으로 멀티뷰포트가 켜지면서 축 A 가 **비로소 자극이 됐다.** 창을 옮기면
content 원점이 실제로 움직이고, 절대 좌표를 굳힌 요소는 따라오지 않아 상대가
갈린다. 앞 절이 "이 자극은 값에 닿지 않는다" 고 적은 그 축이다.

같은 전환이 `verify-editor-chrome-golden.ps1` 도 붉혔다. 그쪽은 `PrintWindow` 로
찍은 **클라이언트 비트맵**에 제품 좌표를 그대로 써서, 가릴 사각형이 통째로
비트맵 밖으로 나갔다 — "가린 화소 0" 이 되고 가리지 못한 로그 누적 칸이 회차마다
갈려 재현 축이 붉었다. 붉은 이유가 픽셀이 아니라 좌표계인데 그렇게 보이지 않는다.
배치에 `editor.theme` 을 넣어 좌표계를 묻고 `ClientToScreen(hwnd, {0,0})` 을 빼
비트맵 좌표로 옮겼다(꺼져 있으면 보정은 (0,0) 이라 예전 동작 그대로다).

★ 보정을 **읽는 자리 전부**에 넣어야 했다. 가림만 고치고 `environment.json` 과
"가리기가 샜다" 단정을 두었더니 보정 전 좌표와 비트맵 좌표를 나란히 비교하게 되어
`imageMinX: 골든 0 / 지금 480` 로 "환경이 다르다"(픽셀 판정 통째로 건너뜀)와
`x 774 < 1244`(가리기가 샜다)가 남았다. 보정하면 1244 → 764 이고 774 > 764 다.
