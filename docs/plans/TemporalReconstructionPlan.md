...
| 트랙 | 단일 책임 | 행 | 일 |
|---|---|---:|---:|
| `BASE-0` | 공통 밀봉 frame/PNG/HDR/timing/graph stats (PHASE 4.75에서 이관) | 1 | 6 |
...
따라서 PHASE 4.5는 PHASE 4.75의 `RG6`·`Q0`를 기다리지 않는다. **이 전제가 뒤집히는 경우는
...
PHASE 4.75 공통·GPU 설계 게이트(현 §7.1)에서 이관했다. 이 페이즈가 4.75보다 앞에 서고,
`TR0` 계측 무해화가 이 하네스를 직접 소비한다. PHASE 4.75는 `BASE-0`을 입력으로 받는다.
...
- 에디터 뷰포트에 FG를 넣지 않는다. `live_present`가 swapchain present가 아니라는 구조적
  사실을 우회하지 않는다.
- PHASE 4.75의 RenderGraph resource lifetime이나 queue 계약을 이 페이즈가 재정의하지 않는다.
