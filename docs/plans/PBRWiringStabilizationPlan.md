...
## 20. 판정 범위를 DX12 로 좁힌다 — 2026-09-15 결정

**결정(사용자).** PHASE 4 의 **판정**은 DX12 백엔드로만 한다. Vulkan 과의 1:1 픽셀
대응은 **PHASE 4.9 [`BackendParityPlan.md`](BackendParityPlan.md)** 가 소유한다.
다른 세션은 이 페이즈에서 vulkan 을 걱정하지 않는다.

### 무엇을 미루고 무엇을 안 미루는가

미루는 것은 **판정**이지 **배선**이 아니다. 이 구분이 전부다.
...
1. **교차 백엔드 픽셀 판정은 성립하지 않는 질문이었다(§15).** 같은 백엔드끼리도 Δt 만
   벌어지면 허용치를 넘는다 — 구름 그림자·포그의 직전 프레임 혼합·SSGI 누적. 그리고
   캡처는 시뮬레이션 시각을 고정할 수단이 없다(`time.*` 명령이 없다). 허용치를 늘려
   초록으로 만드는 길은 §15 가 이미 거부했다.
2. **PHASE 4 와 무관한 vulkan 결함이 게이트를 끝까지 못 가게 막고 있다(§19).**
   기동 창에서 `gCubeMap`(Set 0, Binding 100)이 기록되지 않은 셋으로 그려진다.
   `VulkanEncoder::SetBindings` 가 이미지 뷰 생성에 실패하면 pending 을 넣지 않고
   돌아가는데 `Draw` 는 그대로 진행한다. **sampler 와도 PBR 배선과도 무관하다.**
3. **진도.** 이 둘 때문에 W8/W9 가 서지 못하고 있었다. 하나를 끝내는 편이 낫다.

### 조용히 사라지지 않게 하는 장치

이 계획의 일관된 요구는 "게이트는 재지 않은 축을 PASS 로 보고하지 않는다" 였다(§18).
그 요구는 **미룬 축에도 그대로 적용된다.**

- 축 회계가 `skipped` 와 `deferred` 를 가른다. `skipped` 는 **이번 기계에 조건이 없어**
  못 잰 것이고(추적 밖 Gunner 자산), `deferred` 는 **다른 계획이 소유하는** 것이다.
  둘을 한 통에 넣으면 뒷사람이 없는 결함을 찾아 헤맨다.
- 요약 줄이 `잰 축 N · 건너뛴 축 M · 미룬 축 K` 를 모두 부르고, 미룬 축에는
  소유 계획 이름을 붙인다.
- PASS 줄에 `(dx12 판정)` 을 박는다. 범위를 이름에 적지 않으면 게이트 이름이 범위를
  속인다.
- `-Backend dx12,vulkan` 으로 언제든 되돌려 잴 수 있다. 코드를 지운 것이 아니다.

### 이 결정이 바꾸지 않는 것

§19 가 남긴 `vulkan-samplermodes` 실측은 유효한 기록이다 — `draws 18 · bindings 18 ·
