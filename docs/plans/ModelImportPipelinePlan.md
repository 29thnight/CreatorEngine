...
> 기존 GUID 보존·legacy 병행·Assimp fallback·experiment A/B를 전제로 한 지시는 실행하지
> 않는다. 아래 내용은 구현 이력, 과거 측정, 게이트 오진 증거로만 보존한다.

2026-08-25 신설(당시 PHASE 24), 2026-08-25 **당시 PHASE 4 로 편입**,
...
| **B4-pre** ✅ | 포즈 무결성 축 — 그림이 아니라 **그림의 입력**을 잰다(유한성·인덱스 범위·크기 상한·포즈별 digest). "하네스 한계" 판정은 **철회** — glTF inverseBind 전치 결함이었고(2026-09-02 규명·수정), 시각 축(`dx12.scene` + `animpose`)이 열렸다 | B4a |
| **B4-vis** ✅ | 라이브 스키닝 시각 축 게이트 `verify-skin-pose-visual`(bind·place·drop 세 팔, 2026-09-02) — B4b 되돌림이 요구한 선행 ①·② | B4-pre |
| **B4b** ✅ | 틱 단일화 — legacy 재귀 틱 373줄 폐기, 파리티를 experiment 단독으로 축소. **대입은 남긴다**(대조 arm 보존). 2026-09-01 되돌림 → **2026-09-02 시각 축 위에서 재착지**(에디터 드롭 경로 experiment 이중화 포함, 아래) | B4-vis |
...
**I6-B4b 되돌림 (2026-09-01) — 라이브 렌더를 재는 게이트가 없어서 두 번 틀렸다.**

B4b와 그 후속 수정을 **둘 다 되돌렸다**(`cf9c1838`·`77c1ade3`). 근거는 사용자 보고
둘이고, 둘 다 **헤드리스 게이트가 전부 초록인 상태에서** 나왔다.

| 보고 | 내가 한 것 | 결과 |
|---|---|---|
| ① 드롭한 애니메이션 모델이 안 그려진다 | 드롭 경로(`LoadCachedModelShared`)를 experiment 로더로 이중화 | 그려지긴 하는데 **메시가 뒤죽박죽**(본 바인딩이 어긋난 모양) · `SU_Mythic`은 여전히 안 보임 |
| ② 위 증상 | — | **되돌림** |

★ **되돌린 이유는 "원인을 모르겠다"가 아니라 "판별할 자가 없다"다.** 헤드리스는
프레임을 완성하지 않으므로(`--script` 렌더 0프레임) 라이브 스키닝의 **그림**을 재는
축이 이 저장소에 **하나도 없다**. 실제로 이 세션에서:

- `verify-experiment-vertex-live`는 저장 직전에 **Animator를 꺼 버린다**(비결정성
  때문에 — 시나리오 주석에 그렇게 적혀 있다). 즉 재는 것은 **바인드 포즈**다.
- `experiment.animtick`은 포즈 **산술**을 결정적 표본으로 재지, 그 팔레트가 화면에
  어떻게 적용되는지는 안 잰다.
- `dx12.scene` 커버리지도 바인드 포즈 기준이다.

그래서 "포즈 산술 오차 0 · 커버리지 동수 · 업로드 전량 experiment"가 **전부 초록인
채로** 살아 있는 애니메이터의 그림이 깨질 수 있다. B4b가 걷어낸 legacy 재귀 틱은
그 구간을 덮고 있던 마지막 폴백이었다.

★ **드롭 경로 발견 자체는 유효하다.** `LoadCachedModelShared`가 experiment 로더를
타지 않는다는 것(D34b가 놓친 함수)은 실측으로 확인됐다 — CLI `model.loadcached`로
재현했고, 그 경로만 `[model.dual]`이 없고 `[anim.tick] none`이 나온다. 다만 그
수정은 드롭한 모델을 **experiment 정점·스킨 PSO**로 옮기는 변경이기도 해서,
라이브 스키닝을 재는 축이 서기 전에는 단독으로 landing할 수 없다.

**재착수 전 선행(순서 고정):**

1. **라이브 스키닝 시각 축을 만든다.** 결정적 포즈(고정 시각으로 클립을 샘플링해
   Animator를 그 포즈에 고정)로 `dx12.scene`을 태우고 커버리지·밝기를 잰다.
   비결정성이 문제였으므로 **시간을 고정하는 것**이 열쇠다 — 애니메이터를 끄는
   것이 아니라.
2. 그 축이 legacy 틱이 살아 있는 지금 상태에서 **A/B로 성립함**을 먼저 확인한다
   (legacy 팔이 있는 마지막 구간이다 — 이번에도 그 창을 놓쳐서 되돌리게 됐다).
3. 그다음에 드롭 경로 수정 → B4b 순서로 다시 올린다.

★ **래칫이 67 → 74로 후퇴했다**(창구 밖 38 → 43). 표에 사실대로 적었다.
후퇴를 감추면 다음 착수가 잘못된 기준선에서 시작한다.

★ **교훈 하나를 명시한다.** I6 정찰이 "은퇴 슬라이스의 게이트는 소비 0과 빌드가
막는다로 옮겨야 한다"고 적었는데, 그것은 **정적 축으로 충분하다**는 뜻이 아니었다.
폴백을 걷는 슬라이스는 폴백이 덮고 있던 **런타임 동작**을 먼저 관측 가능하게
만들어야 한다. B3에서 "에디터 드롭 경로는 게이트가 안 재는 구멍"이라고 적어 놓고
그 구멍을 닫지 않은 채 B4b를 올린 것이 이번 실패의 직접 원인이다.

**되돌린 뒤 재측정 (2026-09-01) — 재생은 정상이고, 편집 모드는 원래 안 돈다.**

되돌린 빌드에서 사용자가 확인: **메시는 둘 다 정상 출력, 애니메이션 재생이 안 됨.**
그 둘째 항목을 관측 장치를 만들어 재봤다.

★ **`experiment.animlive` 신설** — 이 저장소에 **살아 있는 애니메이터가 실제로
도는가**를 재는 축이 하나도 없었다(B4b 되돌림의 직접 사유). 애니메이터별로 경로·
선택 클립·경과 시간과 **라이브 팔레트(`m_FinalTransforms`)의 digest**를 찍는다.
시간을 두고 두 번 부르면 digest가 달라져야 재생 중이다.

**실측:**

| 모드 | elapsed | palette digest |
|---|---|---|
| 편집 모드(3회 표집, 각 120프레임 간격) | `0.0000` 고정 | `6A34EE4A` **불변** |
| `play` 뒤 2회 표집 | `17.7573` → `6.4524`(루프) | `95306809` → `E5A70002` |

★ **편집 모드에서 안 도는 것은 설계다.** `EditorMain.cpp`가 편집 모드에서
`GameLogic(0.0f)`을 부른다 — 주석에 "편집 모드는 일시정지가 아니지만 시간도
진행하지 않는 제3의 상태"라고 명시돼 있고, delta 0을 **의도적으로** 넘긴다.
즉 뷰포트에서 안 움직이는 것은 회귀가 아니다.

★ **`--script` 헤드리스도 편집 모드다.** 그래서 이 세션 내내 라이브 재생을
관측할 수 없었던 것이고, `play`를 넣으면 관측된다 — B4b 재착수의 시각 축은
**play 모드에서** 세워야 한다는 뜻이다. 이것이 "결정적 포즈" 선행 항목의
구체적 형태다: `play` → 고정 프레임 수 → `animlive` digest 골든.

**I6-B4-pre 착수 실측 (2026-09-02) — 축은 섰고, 첫 측정이 미해결 결함을 냈다.**

★ **결정성을 어디서 얻는가 — 편집 모드다.** 기존 라이브 게이트는 애니메이션의
비결정성을 **Animator를 꺼서** 피했고(시나리오 주석: "스키닝 산술의 시각 판정은
이 게이트 밖이다"), 그 공백에서 B4b가 두 번 깨졌다. 편집 모드는 `GameLogic(0.0f)`
— delta 0이라 시간이 안 흐른다. 그 상태에서 `m_TimeElapsed`를 한 번 박으면 틱이
매 프레임 **같은 시각의 포즈를 다시 계산**하므로 결과가 안정적이고, 애니메이터를
끄지 않으므로 프록시가 팔레트를 계속 나른다. `experiment.animpose <fraction>`이
그 일을 한다.

★ **첫 측정이 곧바로 갈랐다 — 그리고 그것이 미해결이다.**

| 조건 | dx12.scene 커버리지 |
|---|---|
| Animator **끔**(저장분에 baked — 기존 게이트가 재던 것) | **42411** |
| Animator **켬** + 포즈 고정(0.0 / 0.5) | **65536 (포화)** |

포화는 스케일 0.3·0.05, 위치 1.5·6, **A/B 양팔(experiment/legacy)** 어디서도
안 바뀐다. 즉 캐릭터가 커서 화면을 채우는 것이 아니라, **라이브 팔레트로 그린
스킨 메시가 화면 전체로 번진다**. 0 행렬이 투영에서 `w=0`이 되면 정확히 이렇게
된다.

★ **미해결 — 두 갈래 중 어느 쪽인지 아직 못 갈랐다.**
① 런타임의 포즈 적용이 깨져 있다, ② `dx12.scene` 하네스가 라이브 팔레트로 스킨
메시를 못 그린다. 하네스가 팔레트를 **싣기는 한다**(`EnhancedSceneRendererSelfTest`
3133: `item.bonePalette = proxy->m_finalTransforms.get()`)는 것까지는 확인했다.
가설 하나(바인딩 생성 프레임에 프록시가 0 팔레트를 복사)를 `rebound`에도 dirty를
올려 시험했으나 **측정이 안 바뀌어 기각**했고, 검증 안 된 코드라 되돌렸다.

★ **이것이 B4b의 선행인 이유가 분명해졌다.** 이 값이 설명되기 전에는 "은퇴가
그림을 안 바꿨다"를 말할 자가 없다. 그리고 사용자가 본 두 증상(안 그려짐 →
뒤죽박죽)이 전부 이 구간이다.

**다음 한 걸음:** 포화가 하네스 탓인지 런타임 탓인지 가른다. 방법은 프록시가
실제로 나른 팔레트를 하네스 직전에 digest로 찍어 `animlive`의 값과 대조하는 것이다
— 같으면 하네스가 옳게 받은 것이고 결함은 그 뒤(셰이더/CB), 다르면 운반 구간이다.

**I6-B4-pre 둘째 실측 (2026-09-02) — 내 첫 해석을 철회하고, 유효한 대조로 다시 쟀다.**

★ **첫 시나리오는 측정 도구로 성립하지 않았다.** 직접 짠 시나리오가 대조군
(애니메이터 끔)에서도 **밝기 0.00000 · 커버리지 포화 · `[4/4] 실패`**였다 —
하네스 렌더 자체가 퇴화한 상태라 그 위에서 읽은 "메시가 화면을 덮는다"는
근거가 없다. 철회한다. 기존 라이브 시나리오(드로우 9 · 42411 · 밝기 0.21219)가
**검증된 기준선**이므로 그 위에 얹었어야 했다 — `object.property ... m_isEnabled
false` 한 줄을 `experiment.animpose`로 바꾸는 것이 전부다.

**유효한 대조로 다시 잰 값:**

| 조건(같은 시나리오·같은 드로우 9) | 커버리지 |
|---|---|
| Animator 끔 (기존 게이트) | **42411** |
| Animator 켬 + 포즈 고정 · **experiment 팔** | **65536** |
| Animator 켬 + 포즈 고정 · **legacy 팔**(`CREATOR_EXPERIMENT_VERTEX=0`) | **65536** |

즉 포화는 **A/B 스위치와 무관**하다 — 두 경로가 공유하는 구간의 성질이다.

★ **운반은 결백하다(실측).** 하네스가 실제로 받은 팔레트의 digest를 찍어
`animlive` 값과 대조했다: `6A34EE4A`/`E54799EC` — **완전히 일치**한다. 프록시
복사·수명·dirty 갱신은 옳고, 남은 구간은 팔레트를 받은 **뒤**(스키닝 산술·CB
규약)다.

★ **아직 못 가른 것 하나가 남았고, 그것은 헤드리스로 못 잰다.** ①런타임 스키닝이
깨졌다 ②`dx12.scene` 하네스가 라이브 팔레트를 못 그린다. 둘을 가르는 관측은
**에디터 화면**뿐이다 — 프록시 dirty 수정(`6250fa40`) 이후 실제로 애니메이션이
정상으로 보이면 ②이고, 터지면 ①이다.

★ **이 세션의 방법론 교훈.** 새 측정 축을 세울 때 **검증된 기준선 위에 얹는 것**이
먼저다. 나는 시나리오를 새로 짜서 "대조군이 유효한가"를 안 물었고, 그 결과
퇴화한 렌더 위에서 결론을 하나 만들어 냈다. 대조군의 유효성 자체가 첫 단정이어야
한다.

**I6-B4-pre 완료 (2026-09-02) — 시각 축은 불가 판정, 대신 입력 축을 세웠다.**

★ **사용자 확인으로 갈렸다: 하네스 한계다.** 프록시 dirty 수정 뒤 **에디터에서
애니메이션이 제대로 움직인다**. 즉 런타임 스키닝은 정상이고, `dx12.scene`이
라이브 팔레트로 스킨 메시를 못 그리는 것이다(팔레트는 **옳게 받는다** — 하네스
수신 digest가 `animlive`와 완전히 일치한다). 따라서 **시각 축을 그 하네스 위에
세울 수 없다**. 이것이 이 슬라이스의 첫 결론이다.

★ **그래서 그림 대신 그림의 입력을 잰다.** `experiment.skinbounds`가 팔레트 ×
정점 스킨을 CPU에서 풀어 셋을 본다:

| 축 | 잡는 것 |
|---|---|
| 유한성(NaN/inf) | 팔레트가 쓰레기가 되는 변화 |
| 본 인덱스 범위 | 인덱스가 밀리는 변화 |
| 포즈별 digest 골든 | 포즈 산술이 달라지는 변화 |

B4b의 실패 양식이 정확히 그 셋이다.

★ **단정하지 않는 것 — 결과 기하의 절대 크기.** 곱 순서 두 가지를 다 시험했는데
바인드 대비 **7,115배**와 **22,020배**가 나왔다. 밖에서 팔레트 규약을 맞출 수
없다는 뜻이고, [[encoder-bench-must-measure-real-path]]가 적어 둔 "재구현
대조군은 못 믿는다"의 그 자리다. 크기 비율은 로그에 관측값으로만 남긴다 —
자로 쓰면 안 되는 숫자를 자로 쓰지 않는다.

★ **네 번째 단정이 눈먼 초록을 막는다.** 세 포즈의 digest가 **서로 달라야**
한다. 같으면 팔레트가 굳은 것이고, 그 상태로도 위 세 단정은 전부 통과한다 —
실제로 이 세션에서 그 상태(프록시 미갱신)를 한 번 겪었다.

**실측·재현성:** 포즈 0.0/0.5/0.9 → `C3FF0B7A`/`85CB494C`/`FAA108C7`, 두 실행에서
동일. 곱 순서를 바꾸면 세 값이 전부 달라진다(`079B4C9A` 외) — 이빨 확인.

★ **진단 중 찾은 사실 하나.** 메시 엔티티는 `GetRootIndex`를 안 채운다
(`SetRootIndex`는 본 오브젝트에만 걸린다 — 실측 `withSkin=2` vs `withAnimator=0`).
렌더러에서 애니메이터를 찾으려면 프록시와 같은 규칙으로 부모 사슬을 거슬러야
한다(`ProxyCommand::FindEnabledAnimator`).

**남은 것(B4b 재착수 전):** `dx12.scene`이 라이브 팔레트로 스킨 메시를 못 그리는
원인은 미규명이다. 이 축이 그 자리를 대신하지만, 하네스가 고쳐지면 진짜 픽셀 축을
얻는다 — 별도 항목으로 남긴다.

**I6-B4-pre 원인 규명 (2026-09-02) — 하네스는 옳았고, 팔레트가 폭발해 있었다.**

★ **"하네스 한계 · 에디터 정상" 판정을 철회한다.** 포화의 원인은 `dx12.scene`이
아니라 **glTF 임포터가 inverseBind를 전치된 채 게시**한 것이다. fastgltf의
`fmat4x4`는 열 우선·열 벡터(`value[col][row]` = M(row, col))이고 엔진
`matrix4x4`는 행 우선·**행 벡터**(이동이 `m[3][0..2]`)라 담을 때 전치가 필요한데
`out.m[row][col] = value[col][row]`로 그대로 담고 있었다. `FbxImporter::FbxMatrix`와
legacy `SkeletonLoaderMatrixFromAssimp`는 둘 다 전치한다 — glTF만 빠졌다.

★ **원본에서 독립 유도해 갈랐다.** glb를 파이썬으로 직접 읽어 바인드 팔레트
(inverseBind × 바인드 전역)를 두 규약으로 계산했다:

| 계산 | 바인드 대비 스킨 크기 |
|---|---|
| 올바른 전치(`IBᵀ·Wᵀ`) | **1.0000** |
| 임포터 방식(`IB·Wᵀ`) | **9,612** |
| 원본 `W·IB − I` 최대 편차 | 4.4e-5 (Gunner) · 1.2e-4 (SU_Mythic) |

`experiment.skinbounds`의 7,115배는 "규약을 못 맞춘" 값이 아니라 **폭발한 팔레트를
셰이더와 같은 산술로 옳게 잰** 값이었다. 곱 순서를 바꿔 나온 22,020배도 같은
폭발의 다른 단면이다.

★ **세 증거가 전부 나를 속인 이유.**
- digest 일치는 **운반**만 증명한다 — 폭발한 팔레트도 정확히 운반된다.
- 사용자의 "제대로 움직임"은 **드롭 경로**를 본 것이다. `model.loadcached`는
  legacy Assimp 스켈레톤을 쓰고(`path=legacy bones=62` — 사용자의 animlive
  출력에 그대로 적혀 있었다), 하네스 시나리오의 `model.place`·씬 로드는
  experiment 경로다(`path=experiment bones=63`). 같은 자산, 다른 경로.
- `experiment.model` 패리티 0건은 legacy→experiment **순브리지**(`LoadAndBridge`)를
  대조군으로 써 임포터를 안 태운다 — [[control-group-needs-independent-derivation]]의
  동어반복 그대로. B4b 되돌림 기록의 "메시가 뒤죽박죽"이 이미 이 결함의
  목격담이었다(드롭 경로를 experiment 로더로 바꾸자 나타났다).

★ **`play` 모드에서도 같다** — 포즈 고정 도구의 문제가 아니었다. 실측: 편집+
`animpose 0.0` 7,115배 · `play` 17.06s 7,848배 · `play` 1.01s 7,049배.

**수정과 검증:**

| 항목 | 내용 |
|---|---|
| `GltfImporter.cpp` | inverseBind를 전치해 담는다(`value[row][col]`). flip 판정은 대칭이라 무관 |
| `CookedModelFormat.h` | **v3 → v4**. 바이트 자리가 같아 구버전 캐시를 mtime으로 못 가른다 — v2와 같은 이유 |
| `experiment.gltf` | **바인드 자기 대조** 신설: IR의 노드 TRS 사슬 × inverseBind ≈ I, 임계 1e-2(글TF는 바인드≠레스트를 허용하므로 그런 자산이 오면 note로 내린다). 실측 Gunner 4.8e-5 · SU 1.2e-4 · Cube joints 0 |
| `experiment.skinbounds` | 크기 상한 **4배**를 판정에 넣는다(정상 1배 근방, 결함 수천 배). 실측 1.0057/1.0099/1.0081 |
| `verify-skin-pose-integrity` | 비율 단정 추가·골든 갱신 `C49D5851`/`266E6C06`/`F84B0B3D`(옛 골든은 폭발한 팔레트의 값) |

★ **라이브 스키닝 시각 축이 열렸다** — B4b 되돌림이 요구한 선행 ①이다. 같은
씬·같은 드로우 10에서:

| 조건 | dx12.scene 커버리지 | 판정 |
|---|---|---|
| Animator 끔(바인드) | 42411 | 통과 |
| Animator 켬 + `animpose 0.5` | **49617** | **통과** |

수정 전에는 켬이 65536 포화·실패였다. 이 시나리오(`model.place` → 저장·재로드 →
`animpose` → `dx12.scene`)를 게이트로 세우는 것과 legacy 틱 팔과의 A/B(선행 ②)가
B4b 재착수의 다음 걸음이다.

★ **교훈.** "하네스 한계"라는 판정은 **하네스와 눈이 같은 경로를 봤는가**를 먼저
물어야 한다. 그리고 재계산이 수천 배를 내면 규약 탓을 하기 전에 원본에서 5분
검산할 것 — [[harness-limit-verdict-was-real-defect]].

**I6-B4-vis 시각 축 게이트 (2026-09-02) — B4b 되돌림이 요구한 선행 ①·②.**

`verify-skin-pose-visual`(run-all 편입)이 같은 씬·같은 배치를 세 팔로 그린다:
bind(`model.load` + 저장 전 Animator 끔) · place(`model.load` + `animpose 0.5`) ·
drop(`model.loadcached` + `animpose 0.5` — B4b 회귀가 난 에디터 드롭 경로). 단정은
세 팔 통과·0/포화 아님 · place ≠ bind · place 골든(커버리지 49617 · 팔레트
`384DCD0A`, 두 실행 동일) · bind = vertex-live 기준 42411 · `[anim.tick] none` 0 ·
drop = place 동수.

★ **함정 하나.** 바인드 팔에서 Animator를 재로드 **뒤에** 끄면 프록시가 t=0
팔레트를 이미 들고 있어 바인드가 아니다(실측 49684 — 포즈 팔과 거의 같다).
저장 **전에** 꺼야 42411이 나온다. "Animator 끔 = 바인드"는 프록시가 팔레트를
언제 복사했는지에 달려 있다.

★ **선행 ②(legacy 팔 A/B)의 실측.** B4b 직전, drop 팔은 legacy 틱이었다
(`path=legacy bones=62`, Assimp 스켈레톤·legacy 정점). 커버리지 **51297** vs
experiment 팔 **49617** — 픽셀 동수는 아니지만 둘 다 포화 없이 통과했고, 그림이
갈리는 이유는 정점·스켈레톤 출처(62/63 본, Assimp offset vs glTF inverseBind)다.
이 구간의 동수 단정은 보류하고 값만 기록했다 — B4b가 착지하면 drop도 experiment가
되어 단정이 켜진다(아래).

**I6-B4b 재착지 (2026-09-02) — 시각 축 위에서 같은 diff를 다시 올렸다.**

되돌림 커밋(`d6b574ce`)을 revert해 `cf9c1838`(legacy 재귀 틱 373줄 폐기)과
`77c1ade3`(드롭 경로 experiment 이중화)을 그대로 재적용했다. 충돌 4파일(run-all·
vertex-live 골든·은퇴 래칫 상한·계획서)은 전부 그 뒤의 commits(진단 창구·전치
수정·시각 축)와의 자리 겹침이고 코드 충돌은 없었다.

**실측 — 틱 단일화가 그림을 바꾸지 않았다:**

| 축 | B4b 직전 | B4b 착지 |
|---|---|---|
| place 커버리지 / 팔레트 | 49617 / `384DCD0A` | **49617 / `384DCD0A`** |
| drop 경로 / 커버리지 | legacy / 51297 | **experiment / 49617**(place와 동수) |
| `[anim.tick] none` | 0 | 0 |
| skin-pose-integrity 골든 3종 | 일치 | 일치 |
| 은퇴 래칫 | 70/70 (창구 밖 36/36) | 70/70 (창구 밖 36/36) |
| animtick poseDigest | `1F6758FD`(파리티 축) | `8042DC1C`(experiment 단독 축) |

animtick 골든이 바뀐 이유는 두 가지가 합쳐져서다: B4b가 하네스를 파리티(Linear
강등 사본) 축에서 experiment 단독(Step 보존) 축으로 바꿨고(`093A1FC2`의 사유),
그 위에 전치 수정이 얹혔다. 같은 자산의 **라이브 팔레트 digest는 B4b 전후 동일**
(`384DCD0A`)하므로 골든 이동은 자의 교체이지 포즈의 변화가 아니다.
