...
**근거 수치를 LC6 실측으로 정정한다(2026-09-06).** 이 절이 원래 들던 "2026-09-04
실측: `override void PrePhysics`는 0건인데 `Component` 파생 42개를 매 프레임
순회한다"는 **분모가 비용과 무관하다** —
[생명주기 안정화 LC6](archive/ScriptLifecycleContractHardeningPlan.md)의 착지가
그것을 갈랐다.

| 세는 범위 | `Component` 파생 | `PrePhysics` override | `PostPhysics` override |
|---|---|---|---|
| `GameScripts` 전체 | 47 | 2 | 25 |
| 저작 코퍼스만(회귀 픽스처 41개 제외) | 8 | **0** | 7 |
| `ScriptCore` 내장 | 1 | 0 | 0 |

"`PrePhysics` override 0건"은 **저작 코퍼스 기준으로는 맞다**(전체로 세면 2건이며
둘 다 픽스처다). 틀린 것은 순회 규모 쪽이다 — **프레임 비용은 타입 수가 아니라
런타임 인스턴스 수가 정한다.** 파생 타입이 몇 개든 씬에 붙지 않으면 순회 비용은
0이다. 실측: 저작 씬 16개 중 스크립트가 붙은 것은 3개이고 각각 `ScriptComponent`
2개, `Lifecycle3D` 재생 중 활성 스크립트는 **1개**다.

그러므로 지금 없앨 수 있는 빈 가상 호출은 **프레임당 1회**이고, **이 트랙을 현재의
성능 이득으로 정당화하지 않는다.** LC6이 같은 이유로 자기 성능 변경을 보류했다 —
두 계획서가 서로 다른 분모로 같은 이득을 주장하지 않게 하는 것이 그 트랙의 요구였다.
지금 결론을 뒤집는 조건도 그쪽과 같다: **활성 스크립트 인스턴스가 수백 개 규모가 될
때** 다시 잰다. `script.status`의 활성 스크립트 수가 그 임계를 재는 값싼 지표다.
...
**왜 지금인가.** 근거는 현재 비용이 아니라 **N3-b의 선행**이다.
`NetworkFrameworkPlan` N3-b가 고정 축 훅 `OnSimulationTick(fixedDelta)`을 신설하는데,
그것은 **tick당** 돈다. 감지 없이 추가하면 30fps에서 **인스턴스당 프레임 순회가
2배**가 된다(원래 이 자리에 적혀 있던 42→84는 타입 수로 센 값이라 정정한다 — 배수는
맞고 절대량은 활성 인스턴스 수를 따른다). 지금 인스턴스가 1개라 절대량이 작다는 것은
**나중에 늘었을 때 배수가 그대로 곱해진다**는 뜻이지 이 선행을 면제하지 않는다.
**W8이 그 슬라이스의 선행이며, `ScriptRegistry`를 이 페이즈가 일관되게 소유하도록
PHASE 20이 아니라 여기에 둔다.**

**착수 시점의 판단 재료.** N3-b가 서지 않는 한 이 트랙은 급하지 않다. 반대로 N3-b를
착수하기로 하면, 그때는 활성 인스턴스가 몇 개든 감지를 먼저 세운다.

완료 조건: 훅별 구독 목록이 실제 오버라이드 수에 비례하고, 오버라이드 0건인 훅의
순회가 0이 된다. 생명주기 순서 게이트가 개명 때와 같이 사건 순서를 보존한다.
**프레임 시간 개선은 완료 조건이 아니다** — 위 실측에서 그것은 분해능 아래이며,
이 트랙이 사는 근거는 tick 훅이 붙었을 때의 순회 배수다.
