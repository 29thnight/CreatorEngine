...
1. **V5·V6의 인터페이스를 소비자가 정하게 한다.** 컴파일러 호출만
   ...
   나중에 인터페이스를 다시 찢는다.
2. **PHASE 12.5(빌드 파이프라인)의 셰이더 스텝이 이 파이프라인의 오프라인
...
- **에디터: 요청식 컴파일 + 캐시.** 처음 쓰는 조합만 컴파일하고 콘텐츠 해시
  캐시에 남긴다. **게임 빌드: 전수 사전 컴파일** — PHASE 12.5의 셰이더 스텝이
  이 모드를 부른다.
...
| BuildPipelinePlan (PHASE 12.5) | 게임 빌드의 셰이더 스텝 = 이 파이프라인의 전수 사전 컴파일 모드(3.3) |
