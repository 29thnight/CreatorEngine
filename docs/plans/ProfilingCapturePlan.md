...
**검사.** 코어 프로브 36 → **112 검사**, 변이 3 → **8**. 새 변이 다섯:
`aggregate-flatten`(부모를 안 찾고 전부 루트로) · `aggregate-self-as-total`(자식을
안 뺌) · `aggregate-unsorted`(끝난 순서 가정 노출) · `reader-follow-always`(따라가기를
꺼도 최신으로 점프) · `reader-stale-cache`(선택이 바뀌어도 캐시 유지).
`-Action Window` 는 창 마커를 걷는 변이로 이빨을 확인했다.

**아직 하지 않은 것.** §7.3 의 Timeline(스레드 레인에 스팬을 그리고 확대·이동·
tooltip)은 넣지 않았다. 지금 창이 내는 것은 Frame Overview 와 표 셋이다 —
Timeline 은 스팬 자료가 이미 서 있으므로(`frame_record::events` + `thread_summary`)
그리는 일만 남았고, 그것을 P4 의 GPU 트랙과 함께 세우는 편이 트랙 순서(§7.3)를
두 번 짜지 않는 길이다. §7.4 의 Min/P95/Frames 열도 같이 미뤘다.
