...
**⚠ 열 머리글이 잘린다 — 고치지 못했다.** 화면을 떠 보니 `Total ms` 가
"Tota...", `Frames` 가 "Fra..." 로 나온다. 열을 더하기 **전부터** 있던 잘림이다.
§0.5.21 의 줄 높이와 같은 병으로 보고 폭을 글자에서 재도록
(`CalcTextSize` + `CellPadding`) 고쳐 빌드했는데, 열 경계 픽셀을 세어 보니
**80 px 판과 정확히 같았다**(Marker 886 · Thread 157 · 시간열 107 · Calls 87 ·
Frames 97). 유니티 blob 을 지우고 강제 재빌드해도, `imgui.ini` 에 저장된 열 폭이
없는 것을 확인해도 같았다 — `TableSetupColumn` 에 넘긴 폭이 쓰이지 않는다.
까닭을 못 짚었으므로 **효과 없는 변경을 되돌리고** 그 자리에 여기 적은 사실을
주석으로 남겼다. 고친 척 남기는 것이 고치지 않은 것보다 나쁘다.
