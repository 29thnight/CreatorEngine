...
> **정정.** 처음에는 "둘이 같은 500줄을 만지므로 병합(20일 → 15일)"을 제안했다.
> 확정에 앞서 코드를 전수하니 **근거가 오진이었다.** 두 계획서가 같은 명사
> "19개 노드"를 쓰는 것을 보고 같은 대상이라 읽었는데, **명사는 같고 동사가 다르다.**

| | 심볼 | 총 | 제품 | 게이트 | 위치 |
|---|---|---:|---:|---:|---|
| `RG5` | `AddPass`·`AddSplitPass` | 120 | **38** | 82 | **Pass 구현 파일 전역에 흩어져 있다** — SSGI 4 · VolumetricFog 3 · SSAO/Forward/Decal 각 2 · 나머지 Pass 각 1 · Editor Pass 4 |
| `SRP-1` | `AddNode` | 33 | **33** | 0 | **조립부에 모여 있다** — `EnhancedSceneRenderer.cpp` 15 · `EnhancedLivePipelineDesc.cpp` 10 · Editor 조립 6 |

교집합 파일은 `EnhancedSceneRenderer.cpp` 하나뿐이고 그 안에서도 **다른 줄**이다
(`AddPass`는 `live_present` 2곳 2343·2359, `AddNode`는 조립 15곳).
...
**부수 발견:** `RG5` 계획서의 "제품 28곳·게이트 80곳(총 108)"이 실측 **제품 38·게이트
82(총 120)**다. 제품만 **+36%** — 계획서가 "착수 직전 다시 센다"고 적어 둔 그대로 늘었다.
재산정은 `RG5` 착수 시점의 일로 남긴다.
