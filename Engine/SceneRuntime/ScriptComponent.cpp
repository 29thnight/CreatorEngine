...
void ScriptComponent::PrepareForReload()
{
	...
	// ★ 여기서 id 를 버리지 않는다.
	//
	//   예전에는 여기서 `m_instanceId = -1` 을 했다 — "관리 측이 통째로 내려가므로
	//   파괴를 요청할 필요가 없다"는 전제로. 그 전제는 리로드가 **성공했을 때만**
	//   참이다. 새 어셈블리가 검증에서 떨어지면 관리 측은 내려가지 않고(이전
	//   어셈블리 유지) 옛 인스턴스가 그대로 살아 있는데, id 를 이미 버렸으므로
	//   복원이 그 옆에 **두 번째** 인스턴스를 만들었다. 재생 중이면 그때부터
	//   스크립트가 두 벌 돌았다 — 입력이 두 번 처리되고 코루틴이 두 벌 돌았다.
	//   옛 것은 아무도 거두지 않는다(소유자가 살아 있어 SweepOrphans 도 못 잡는다).
	//
	//   id 는 결과를 안 뒤에 버린다 — RestoreAfterReload 가 그 자리다. 관리 측이
	//   내려가는 동안 네이티브가 이 id 로 관리 측을 부르는 일은 없다: 관리 훅은
	//   Scene 의 드레인·파괴 루프가 부르고, 리로드는 CLI 명령 한 호출 안에서 끝난다.
}

void ScriptComponent::RestoreAfterReload()
{
	// 관리 측이 내려갔으므로 쥐고 있던 id 는 죽은 값이다. 여기서 버리고 새로 만든다.
	// 실패 기억도 함께 지운다 — 리로드는 "등록되지 않은 타입"이 등록되는 대표적
	// 계기다. (검증 실패로 이전 어셈블리가 유지됐을 때는 이 함수를 부르지 않는다 —
	// 그때는 인스턴스도 실패 기억도 리로드 전의 사실 그대로다.)
	m_instanceId = -1;
	m_instanceCreateFailed = false;
