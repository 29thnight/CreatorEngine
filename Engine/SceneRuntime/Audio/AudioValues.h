...
namespace wave
{
    ...
    // 클립을 가리키는 참조.
    //
    // ★★ 이 타입이 AU2 의 이음매다. 지금 안에 든 것은 옛 배선이 쓰던 **파일명
    //   stem 문자열**이다 — 파일 이름을 바꾸면 씬이 조용히 깨지고 다른 폴더의
    //   동명 파일이 충돌한다. 그 결함을 고치는 것은 자산 파이프라인 일(importer ·
    //   `.meta` · cook · pak)이라 이 배선에 묶지 않았다.
    //
    //   대신 문자열을 **여기 한 겹 안에 가둔다**. AU2 가 착지하면 이 클래스의
    //   속만 128-bit identity 로 바꾸면 되고, 소비자는 `ClipKey` 를 값으로 들고
    //   다니므로 손댈 곳이 서비스 경계 한 곳과 컴포넌트 필드 하나로 끝난다.
    class ClipKey final
    {
    public:
        ClipKey() = default;
