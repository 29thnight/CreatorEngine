...
public:
	...
	// 핫리로드 직전에 부른다. 현재 값을 붙들어 둔다 — 인스턴스 id 는 **끊지 않는다**.
	//
	// 리로드가 교체까지 가면 살아 있던 인스턴스가 전부 사라지고, 여기서 챙긴 값으로
	// RestoreAfterReload 가 같은 값의 인스턴스를 되살린다 — 편집 중이던 상태가
	// 유지되는 것이 핫리로드의 목적이다.
	//
	// id 를 여기서 끊으면 안 된다. 리로드는 검증에서 떨어져 **아무것도 바꾸지 않고**
	// 돌아올 수 있고(ClrHost::ReloadOutcome::PreviousKept), 그때 옛 인스턴스는
	// 살아 있다. id 를 미리 버려 두면 복원이 그 옆에 두 벌째를 만든다.
	void PrepareForReload();

	// 핫리로드가 **교체까지 갔을 때** 부른다. PrepareForReload의 짝이다.
	// 검증 실패로 이전 어셈블리가 유지됐을 때는 부르지 않는다 — 되살릴 것이 없다.
	//
