...
namespace meta
{
    ...
    // 인스펙터가 이 필드를 그리지 않는다. 직렬화·복제는 그대로다 — 표시에만
    // 관한 표기다.
    //
    // **기본은 "보임" 이다.** 반대로 두면 새 필드가 인스펙터에서 조용히
    // 사라지고, 그 사라짐은 빌드도 검사도 붉게 만들지 않는다. 이 저장소가 이미
    // 여러 번 겪은 실패 양식이라 방향을 뒤집지 않는다.
    //
    // 이 속성이 생기기 전에는 인스펙터가 필드 이름을 문자열로 비교해 걸렀다
    // (`std::strcmp(name, "m_isEnabled")`). 필드를 옮기거나 이름을 바꾸면
    // 그 규칙이 조용히 풀리는 자리였다.
    struct hidden_attr
    {
    };

    consteval hidden_attr hidden()
    {
        return {};
    }

    // 그리되 **편집을 막는다**. 직렬화·복제는 그대로다.
    //
    // `hidden` 과 다른 자리다. 숨기면 값이 무엇인지 볼 길이 없어지고, 그러면
    // 사람이 다른 경로(로그·디버거)로 확인하게 된다. 유일한 올바른 값이 이미
    // 정해져 있는 필드 — 타입 이름, 식별자, 파생값 — 는 보이되 손이 닿지 않는
    // 것이 맞다.
    //
    // 이것이 없던 동안 `Object::m_name` 이 컴포넌트마다 편집 가능한 글상자로
    // 나왔다. 컴포넌트의 이름은 타입 이름 사본이라 고칠 이유가 없는데, 고치면
    // 씬 파일에 그대로 적혔다.
    struct readonly_attr
    {
    };

    consteval readonly_attr readonly()
    {
        return {};
    }

    // **디버그 모드에서만** 그린다. 기본 인스펙터에서는 없는 것처럼 군다.
    //
    // `hidden` 은 "아무도 볼 일 없다", 이것은 "평소엔 잡음이지만 문제를 쫓을
    // 때는 봐야 한다" 이다. 인스턴스 ID 처럼 내부 식별자가 여기 든다.
    //
    // 판정이 런타임인 것이 `hidden` 과 갈리는 점이다. 속성이 붙었는지는 컴파일
    // 시에 정해지지만 모드는 사람이 켜고 끈다 — 붙지 않은 필드는 검사 코드가
    // 아예 생성되지 않는다.
    struct debug_only_attr
    {
    };

    consteval debug_only_attr debugOnly()
    {
        return {};
    }

    // 라벨이 한 줄을 통째로 쓰고 값이 다음 줄에 온다 (s&box 의 wide mode).
    //
    // s&box 는 이것을 두 갈래로 정한다 — 위젯이 스스로 선언하는
    // `ControlWidget.IsWideMode` 와, 필드에 붙여 강제하는 `[WideMode]`.
    // 이 속성이 뒤쪽이고, 앞쪽은 인스펙터 드로어가 `kWideMode` 로 선언한다
    // (`InspectorDrawer.h`).
    //
    // 좁아서 **내려가는** 것과 구별해야 한다. 그쪽은 폭을 재서 정하는 것이라
    // 창을 넓히면 되돌아오지만, 이것은 선언이라 폭과 무관하게 유지된다.
    // 곡선·그러데이션·긴 문자열처럼 값이 가로 폭을 다 받아야 읽히는 것들이다.
    struct wide_attr
    {
    };

    consteval wide_attr wide()
    {
        return {};
    }
