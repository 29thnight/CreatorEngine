...
public:
	...
	inline const math::vector3& GetExtent() const { return m_Extent; }
	...
	virtual void SetConvertScale(const math::vector3& scale, physx::PxPhysics* physics, unsigned int* collisionMatrix) abstract;

	inline void SetOffsetPosition(const math::vector3& offsetTranslation) {
		m_offsetPsition = offsetTranslation;
	}
	inline const math::vector3& GetOffsetPosition() const {
		return m_offsetPsition;
	}
	inline void SetOffsetRotation(const math::quaternion& offsetRotation) {
		m_offsetRotation = offsetRotation;
	}
	inline const math::quaternion& GetOffsetRotation() const {
...
protected:
	...
	math::vector3 m_scale; // 배율
	math::quaternion m_offsetRotation; // 모델 기준 회전
	math::vector3 m_offsetPsition; // 모델 기준 위치
	float m_radius; // 원형,캡슐 경우 반지름
	float m_halfHeight; // 캡슐일경우 기둥 높이
	math::vector3 m_Extent; //박스일 경우 크기
