...
namespace ce
{
	...
	enum class event_flags : std::uint8_t
	{
		...
		// 길이가 없는 사건(§7.3 의 첫째 트랙). `tick_end == tick_begin` 이고
		// 무엇이 **일어난 순간**만 말한다 — 씬이 바뀌었다, Play 에 들어갔다.
		//
		// ★ 집계는 이것을 Hierarchy·Flat 에 넣지 않는다. 그 두 표가 세는 것은
		//   **걸린 시간**이고 점에는 그것이 없다. 넣으면 total 0 · calls N 인
		//   줄이 시간 순위표에 섞인다.
		//
		// ★ 깊이는 0 이 아니라 **지금 열려 있는 깊이**다. 0 으로 박으면, 트리에
		//   들어갔을 때 스택에서 부모를 밀어내고 그 뒤의 자식들이 루트로
		//   올라온다 — 합계는 0 을 더하므로 수치로는 한 군데도 안 어긋난다.
		//   지금은 트리 밖이라 그 일이 일어나지 않지만, 방어는 둘 다 둔다.
		instant = 1 << 3,
	};
