...
namespace ce
{
	...
	public:
		...
		// 다른 출처의 캡처(파일)를 **처음부터** 연다(P6-3).
		//
		// ★ adopt 와 다르다. adopt 는 **같은 녹화의 다음 스냅샷**을 받는 자리라
		//   선택과 그래프 창을 이어 간다. 파일은 프레임 번호부터 남의 것이라
		//   이어 갈 것이 없다 — 이어 가면 라이브에서 보던 선택이 파일 범위의
		//   가장자리에 잘려 붙고, 그래프는 파일의 앞머리에 서서 최신을 안 보인다.
		//
		// ★ 따라가기를 끈다. 켜 둔 채 열면 **다음 프레임의 sync()** 가 라이브
		//   캡처로 조용히 덮어써, 연 파일이 한 프레임만 보이고 사라진다.
		//   다시 켜면 라이브로 돌아간다 — 돌아가는 길이 따로 없는 이유다.
		//
		// ★ 이 정책을 창이 아니라 여기 둔 것은 **재기 위해서**다. 창에서 네
		//   호출을 조합하면 그 순서가 맞는지를 물을 수단이 눈뿐이 된다.
		void open(capture_session_ptr capture);
