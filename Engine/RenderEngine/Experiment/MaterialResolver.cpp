...
namespace experiment
{
        ...
        ResolvedMaterial& outResolved, std::string& outError)
    {
        ...
        if (material.keywordSelections.size() > meta->keywords.size())
        {
        ...
        {
            if (material.keywordSelections[axis]
                >= meta->keywords[axis].values.size())
            {
                outError = "keyword 선택 인덱스가 축 범위를 벗어난다: "
                    + meta->keywords[axis].name;
                return false;
            }
        ...
        for (const std::string& keyword : material.keywords)
        {
            ...
            std::size_t matchedAxis = meta->keywords.size();
            std::uint16_t matchedValue = 0;
            for (std::size_t axis = 0; axis < meta->keywords.size(); ++axis)
            {
                const auto& values = meta->keywords[axis].values;
                const auto found = std::find(values.begin(), values.end(), keyword);
                if (found == values.end()) continue;
                if (matchedAxis != meta->keywords.size())
                {
