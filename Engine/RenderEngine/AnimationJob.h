...
public:
	...
	// 프레임 안에서만 유효한 관찰용 raw 포인터로 추적한다(등록/해제는 OnInitialized/
	// OnUninitializing이 this로 직접 호출). 수명 불변식은 AnimationJob.cpp의
...
private:
	...
	// shared_ptr이 없다 — 등록/해제 시점(OnInitialized/OnUninitializing)에 정확히 맞춰
	// 정본을 유지한다.
