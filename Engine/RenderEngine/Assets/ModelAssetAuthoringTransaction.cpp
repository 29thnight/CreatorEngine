    ...
    namespace
    {
            ...
            std::string& failure)
        {
                ...
                error))
            {
                ...
                if (parsed.Root()["schemaVersion"].As(0u) != 2u) continue;
                ModelSidecarV2 document;
                std::vector<SidecarIssue> issues;
                if (!ReadModelSidecarV2(text, document, issues)
                    || !ValidateModelSidecarV2Closure(document, header, issues))
                {
                ...
                if (!addDocument(document, entry.path())) return false;
            }
            if (error)
            {
