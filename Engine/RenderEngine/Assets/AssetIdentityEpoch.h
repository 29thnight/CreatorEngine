...
namespace assets
{
    ...
    // MBC4 one-shot issuance. The seed is generated by the same BCrypt CSPRNG as
    // CreateIdentityEpochSeed and the validated document is published with a
    // same-volume, no-replace rename. An existing epoch is never overwritten.
    [[nodiscard]] bool IssueIdentityEpochHeader(
        const std::filesystem::path& path, std::string_view identityEpoch,
        std::string& outError) noexcept;
