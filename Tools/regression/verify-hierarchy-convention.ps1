...
# 실제로 그 사각지대에 결함이 살아 있었다(Scene::CreateGameObject, 커밋 2cc6fecf).
#
...
for ($i = 0; $i -lt 4; $i++) {
    ...
    if ($topInvalid -ne 0) {
        # '갓만듦'에서만 나오면 **생성 경로**가 범인이다(Scene::CreateGameObject).
        # '왕복후'에서도 나오면 로더/Attach 쪽이다. 라벨이 그 둘을 갈라 준다.
        $failed += "$name 최상위(-1표기)가 $topInvalid 건이다 — 루트 규약이 갈렸다(갓만듦에서만 나오면 Scene::CreateGameObject, 왕복후에도 나오면 AttachExistingGameObject/SceneManager 리맵 폴백)"
    }
    if ($mismatch -ne 0) {
