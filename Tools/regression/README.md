...
| `../profiling-validation/Invoke-ProfilingValidation.ps1` | **`Tools/regression/` 밖에 있다.** PHASE 14 프로파일러의 두 축. `-Action SelfTest` 는 상한 초과·이름 예산 고갈·스레드 은퇴·프레임을 넘는 스코프 같은 자기 파괴 픽스처를 돌려 `PROFILE_SELFTEST_OK=true` 로 판정한다(기지 결함 1: `cross-frame/preserve` — P2 의 sealed chunk handoff 가 닫는다). `-Action Stats` 는 **단독으로** 돌려야 기준선이다 — selftest 직후에 재면 `overflow/count` 픽스처가 일부러 낸 1,280 개가 포화·드롭으로 보이는 자기 교란이다. 판정은 텍스트가 아니라 JSON 이다(`status` · `frameEnd>frameBegin` · `lastFrameEvents>0` · `malformedScopes==0` · `[GameThread]` 생존 · 종료 코드). **텍스트로 판정하던 시절 이 축은 9-06 부터 8 일 · 115 커밋 동안 붉은 줄도 없이 죽어 있었다** — 명령은 성공하는데 `reg.Legacy`→`reg.Result` 전환으로 사라진 문자열을 하네스가 `IndexOf` 로 찾고 있었다. 사는 축은 **이벤트 수** 하나뿐이다(`peak==last`). 이름 예산 단정은 **대상이 사라져서** 뺐다 — 새 코어는 마커 id 만 흘리므로 프레임마다 도는 이름 복사가 없고 예산도 누락 계수도 개념 자체가 없다. 계측 지점이 전부 시스템 단위 고정이라 씬에 무엇을 얹어도 상수이고, 그래서 드롭·포화 단정은 어떤 부하로도 자극되지 않는 빈 단정이다. ★ **그 수를 문서에 박지 마라** — 9-15 실측은 편집 27 · 이름 388B 였는데 PHASE 14 임시 계측 12곳이 붙어 있던 2026-09-20 한때는 38 · 573B 였다(걷고 재빌드하니 27 · 388B 로 한 바이트까지 복귀하는 것을 A/B/A 로 확인했고, 그 계측은 같은 날 걷었다). 남의 계측이 드나들 때마다 움직이므로, 단정은 절대 숫자가 아니라 **교체 직전에 그 자리에서 뜬 값과 직후를 맞대는 동수**여야 한다. 그 하나가 마침 P1 완료조건("호출부 대량 수정 없이 새 코어로 이벤트가 들어간다")을 직접 재므로, 코어를 통째로 가는 동안 마커가 조용히 안 이어지는 것을 막는 래칫이 된다. |
| `verify-profile-core.ps1` | PHASE 14 P1+P2 새 프로파일러 코어의 계약. **엔진을 띄우지 않는다** — EngineDiagnostics 가 ProjectReference 0 의 독립 라이브러리이고 서비스가 인스턴스로 서므로 코어만 cl 로 링크해 초 단위로 돈다(옛 코어는 전역 싱글톤 + 함수 지역 static thread_local 이라 이런 검사가 불가능했고, 그래서 selftest 가 라이브 캡처의 프레임 경계를 직접 넘겨 교란해야 했다). Debug·Release 양쪽에서 36 검사: 마커 id 안정성·중첩 깊이와 포함관계·**프레임을 넘는 구간 보존**·8 워커 동시 기록과 은퇴·풀 고갈 시 무정지와 드롭 정확·**두 서비스 격리**·recorder 상태와 얼린 캡처의 불변성·rolling 퇴출. ★ 변이 셋을 함께 돌려 이빨을 증명한다 — 프레임 경계에서 열린 스코프를 닫으면 cross-frame 이, 드롭 계수를 지우면 overflow 가, 모든 마커를 한 id 로 뭉치면 marker 가 붉어져야 하고, 앵커가 사라지면 '대상이 없다' 로 낡음이 드러난다. 처음 쓴 변이 하나(intern_marker 의 중복 제거 제거)는 **통과했다** — 같은 이름의 marker_slot 은 inline 변수라 한 번만 초기화되므로 그 경로는 지금 이중 안전망이지 계약을 지탱하는 코드가 아니었다. 통과한 변이가 죽은 경로를 가리킨 사례다. |
