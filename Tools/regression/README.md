...
| `verify-profile-core.ps1` | PHASE 14 프로파일러 코어를 **엔진을 띄우지 않고** 잰다. `EngineDiagnostics` 가 ProjectReference 0 의 독립 라이브러리이고 서비스가 인스턴스로 서므로, 코어만 cl 로 직접 컴파일해 Debug·Release 각 402 검사를 초 단위로 돌린다(`/W4 /WX` — 새 코어는 경고 0 이 계약이다). 재는 것은 마커 신원 · 중첩과 프레임을 넘는 구간 · 8워커 동시 기록 · 청크 풀 고갈 · 서비스 격리 · recorder 상태 · rolling 보존, 그리고 **P3 의 집계와 reader**(트리 복원 · self/total/max/calls · 다중 프레임 범위 · 스레드 분리 · Flat 접기 · 과거 프레임 선택 · pause 후 불변 · Live Follow · 집계 캐시), 그리고 **P4 의 화면 상태**다 — 분포 열(min · nearest-rank p95 · 나타난 프레임 수), 녹화 중 공개(`live/*`), 프레임 그래프의 창(`graph/*` — 창은 최신을 따라 흐르고, 뒤로 굴리면 그 자리에 서고, 보존 밖으로 안 나가고, **고른 프레임이 창과 같이 움직인다**), 타임라인 시야(`timeline-view/*` · `window/*` — 시야의 기준은 선택이 아니라 **보이는 창**이고, 골라도 배율이 안 풀리며, 창이 미끄러지면 확대 안 했을 때만 따라간다). ★ 완료조건 "Timeline 합계와 Hierarchy inclusive time 이 일치" 는 **정수 tick 으로** 단정한다 — 같은 것을 두 길로 더하므로 정확히 같아야 하고, 부동소수 ms 로 바꾼 뒤 비교하면 어긋나도 "허용 오차" 로 읽힌다. **변이 65 개**가 각각 제 검사에서 붉는지까지 확인하며, 앵커가 소스에서 사라지면 '대상이 없다' 로 붉어져 **검사가 낡은 것을 그때 안다**. ★ 변이마다 코어를 통째로 다시 컴파일하므로 전부 돌리면 두 구성에 30 분이 넘는다 — 바꾼 계약의 변이만 `-Only 'reader-open-*'` 처럼 골라 돌린다(Release 셋에 2 분). 맞는 이름이 없으면 붉고, 골라 돈 판정은 '고른 변이 n / 전체 m' 으로 따로 적힌다. |
