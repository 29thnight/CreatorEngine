...
foreach ($runtimeFile in @("RenderEngine\DataSystem.h", "RenderEngine\DataSystem.cpp")) {
...
$presentation = "EngineEntry\EditorAssetPresentation.cpp"
...
Assert-DoesNotMatch "ScriptBinder\EnhancedGizmoSceneBinding.cpp" `
    '#include\s+"(?:DataSystem|EditorAssetPresentation)\.h"|DataSystems|EditorAssetPresentation::'
Assert-Matches "RenderEngine\EnhancedGizmoSceneBinding.h" `
    'std::shared_ptr<const EnhancedGizmoIconTextures> iconTextures'
Assert-Matches "ScriptBinder\EnhancedGizmoSceneBinding.cpp" `
    'out\.iconTextures\s*=\s*std::move\(iconTextures\)'
Assert-Matches "RenderEngine\Render\Scene\EnhancedSceneRendererLive.cpp" `
    'CaptureEnhancedGizmoSceneData\(view\.camera, collectColliders,\s*gizmoIconTextures'

Assert-Matches "EngineGUIWindow\ContentsBrowserWindow.cpp" `
    'EditorAssetPresentation::Get\(\)\.ResolveFilePresentation'
Assert-Matches "EngineGUIWindow\ImGuiDrawHelperMeshRenderer.cpp" `
