...
# 골든 이력 — 값이 바뀔 때마다 왜 바뀌었는지 적는다:
#   858071B5  파리티 축 시절, legacy Linear 강등 사본의 값
#   093A1FC2  B4b 1차(2026-09-01): 하네스가 실제 스켈레톤(Step 보존)을 재게 됨
#   1F6758FD  2026-09-02 glTF inverseBind 전치 수정(파리티 축 위, B4b 되돌린 상태)
#   8042DC1C  B4b 재착지(2026-09-02): 위 둘이 합쳐진 값 — Step 보존 + 전치 수정.
#             같은 자산의 라이브 팔레트(animpose 0.5 → 384DCD0A)는 B4b 전후 동일
#             (verify-skin-pose-visual) — 틱 단일화가 그림을 바꾸지 않았다는 증거다.
if ($logOn -notmatch 'animtick pass .* poseDigest=8042DC1C') {
