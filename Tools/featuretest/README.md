...
**어두운 셰이딩의 원인은 SSGI 리사이즈였다.** 1920x1080에서는 구·그림자·바닥이
정상으로 나오는데, 창을 리사이즈하면 검게 변하고 실루엣에 붉은 테두리가 생긴다.
SSGI 패스의 기존 로직이 화면 크기 변경을 견디지 못한다.

★ 이걸 알기까지 두 번 틀렸다. 기록해 둔다.

1. "자동 노출이 밝은 하늘을 측광해 뭉갠다" — `render.exposure`로 재니 자동
   노출은 꺼져 있고 노출은 1.3이었다. 기각.
2. "광원 강도 단위 때문이다, 40으로 올리니 보인다" — 그때 밝아진 것의 상당
   부분이 볼류메트릭 포그였고, 애초에 **내 캡처 스크립트가 매번 창을
   리사이즈하고 있었다**. 즉 나는 계속 SSGI 버그를 찍고 있었다.

교훈은 하나다. 재현 절차 자체가 변수를 하나 더 넣고 있었는데 그것을 의심하지
않았다. 지금은 캡처에서 리사이즈를 뺐고, 그러자 강도 1.6에서 정상으로 나온다.

SSGI는 `EnhancedSceneRenderer` 쪽에서 새 로직으로 다시 만든다. 그때까지 캡처는
기본 해상도로 한다.
