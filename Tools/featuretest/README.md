...
※ `render.exposure` 는 **지금 없다.** 이 측정은 DX11 ToneMapPass 를 읽던 시절의
것이고, 그 뒤 명령은 안내 문구만 찍는 껍데기가 되었다가 2026-09-05 에 지워졌다.
위 표는 그때 무엇을 어떻게 알았는지의 기록이라 그대로 둔다 — 다시 재려면 DX12
계측이 먼저 필요하다.
...
**이 문서는 그동안 틀렸다(2026-09-04 정정).** 여기에는 "`run-featuretests.ps1`이
캡처 전에 `render.post fog off`를 건다"고 적혀 있었다. 그 명령은 DX11 SceneRenderer가
소비하던 구 전역 설정만 바꿨고 EnhancedRenderer에는 전달되지 않아, **DX12 전환 이후로는
성공처럼 출력되는 무효 명령**이었다. 즉 지금까지 찍힌 기능 확인용 그림은 전부 포그가
최종 색의 85%를 덮은 채였고, 이 문서와 `-KeepFog` 플래그가 그 반대를 주장하고 있었다.

LC6이 죽은 명령을 표에서 지우면서 호출부도 사실에 맞췄다. 지금은 캡처 전에
경고만 찍는다. **런타임 포그 토글은 Enhanced PostChain 튜닝 API가 생겨야 가능하다.**
그때까지 그림의 포그는 남는다. `-KeepFog`는 아무 차이를 만들지 않는다.
