...
- **typeID가 잘린다.** `GUIDCreator::GetTypeID<T>()`가
  `static_cast<uint32_t>(typeid(T).hash_code())` — 64비트 해시를 32비트로 자른다.
  30종에서 충돌 확률은 낮지만 자를 이유가 없다.
...
5. **컨테이너는 실사용 규모에 맞춘다.** 컴포넌트는 오브젝트당 0~6개다(§1.5).
   ...
   타입 식별은 잘린 해시가 아니라 **순차 인덱스**로 — 배열 첨자로 바로 쓰이고
   비교가 정수 1회가 된다. 예약은 추정이 아니라 측정에서 나온다.
...
- ⬜ **K1 — 타입 식별을 순차 인덱스 + 비트마스크로**
  - `GUIDCreator::GetTypeID<T>()`의 잘린 해시 대신 컴포넌트 전용 순차 인덱스를 둔다
    (`uint8`, 타입당 한 번 계산). 배열 첨자로 바로 쓰이고 비교가 정수 1회다.
  - **타입 30종 < 64이므로 `uint64` 마스크 하나로 "무엇을 가졌는가"를 표현한다.**
    `HasComponent<T>()`가 `m_typeMask & (1ull << idx)` 한 줄이 되고 8바이트로 끝난다 —
    지금은 이걸 위해 해시맵을 조회한다. `GetComponent`도 마스크로 먼저 거른다(없으면
    배열을 아예 안 본다: 씬 58%·프리팹 89%가 컴포넌트 0개다).
  - 리플렉션·직렬화의 `typeID`는 **건드리지 않는다.** 순차 인덱스는 런타임 전용이고
    프로세스마다 값이 달라도 되는 반면, 파일에 적히는 typeID는 안정적이어야 한다.
    둘을 섞으면 씬 파일이 실행 순서에 의존하게 된다.
