...
public:
    ...
    ///
    /// ★ Release 실측이 Debug 실측을 뒤집었다.
    ///
    /// Debug로 재면 기록량 1419에서 1.37배, 2827에서 1.65배로 병렬이 이기는
    /// 것처럼 보였고, 그것을 근거로 이 값을 1024로 잡았었다. Release로 다시
    /// 재니 그림이 달랐다(각 규모 2회):
    ///     기록량   187 → 0.55배 / 0.47배
    ///     기록량  2827 → 0.51배 / 0.57배
    ///     기록량 11275 → 0.81배 / 0.87배
    ///     기록량 45067 → 1.16배 / 0.83배
    ///
    /// 이유는 분명하다. 순차 기록이 Debug 5~6 ms에서 Release 0.36 ms로 15배
    /// 빨라진다(드로우 704 기준). 워커를 깨우고 리스트를 여러 개 제출하는
    /// 비용은 그대로인데 기록이 그만큼 싸지니, 병렬이 이길 자리가 사라진다.
    /// Debug에서 본 교차점은 Debug의 것이었다.
    ///
    /// 그래서 이 값을 32768로 올린다 — 실측한 모든 규모에서 병렬이 이득을
    /// 주지 못했으므로, 데이터가 지지하는 선택은 '거의 항상 순차'다.
    /// 45067에서도 1.16/0.83으로 갈렸으니 그곳조차 경계라고 말할 수 없다.
    ///
    /// ── 이 측정의 한계 ──
    ///
    /// 규모를 늘릴 때 같은 메시 11종을 복제하므로 인스턴싱이 최대로 먹는다
    /// (드로우 11264가 배치 11개로 묶인다). 병렬 기록에는 최악의 조건이다 —
    /// 기록할 것이 배치 11개뿐이라 나눌 일이 거의 없다.
    ///
    /// 실제 씬은 재질이 다양해 배치가 수백 개일 수 있고, 그러면 배치마다 드는
    /// 상태 설정 비용이 기록의 대부분을 차지한다. 그 조건에서는 결론이 또
    /// 달라질 수 있다. 배치가 많은 씬을 만들어 다시 재기 전에는 '병렬 기록은
    /// 값을 못 한다'가 아니라 '이 조건에서는 값을 못 했다'까지만 말할 수 있다.
    static constexpr uint32_t kParallelRecordCostThreshold = 32768;
