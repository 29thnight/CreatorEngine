...
//       일관되고 중복 전치도 없다.
//
//   12. 입력 불일치 — 전제는 맞았다(확인함).
//       DX11 GBufferPass도 proxy->m_worldMatrix를 그대로 쓴다. 비인스턴싱
//       경로는 scene.UpdateModel(proxy->m_worldMatrix)(GBufferPass.cpp:258),
//       인스턴싱 경로는 instanceMatrices.push_back(proxy->m_worldMatrix)
//       (같은 파일 336). 캡처 스냅샷이 복사하는 것과 같은 값이다.
//
// ★ 다만 확인 중에 규약 차이를 봤다(원인인지는 미확인):
//   DX11은 인스턴스 행렬을 전치 없이 넘기고, DX12는 XMMatrixTranspose를
//   거쳐 넘긴다(EnhancedGBufferPass.cpp:330). 셰이더가 서로 다른 규약을
//   쓰면 그 자체로 오류는 아니다 — DX11 인스턴싱은 InstancedVertexShader를
//   쓰고 DX12는 자체 셰이더라 각자 맞춰져 있을 수 있다. 실제로 위치가 거의
