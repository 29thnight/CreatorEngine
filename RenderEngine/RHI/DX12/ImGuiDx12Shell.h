...
// 백엔드에 묶인다). 씬 렌더러는 EnhancedRenderer/DX12로 고정됐지만 ImGui
// 표시 셸은 별도 설정이다. 현재 기본은 안정적인 DX11 셸이며, 이 클래스를
// 켜려면 DX11 스왑체인 소유권 이관을 먼저 해결해야 한다. render.backend
// 명령은 씬 경로를 바꾸지 않는 상태 확인용 호환 표면이다.
//
// ── 스레드 ──
//
