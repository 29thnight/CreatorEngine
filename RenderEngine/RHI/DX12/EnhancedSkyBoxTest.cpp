...
namespace
{
    ...
    constexpr uint64_t kSkyRowPitch =
        ((static_cast<uint64_t>(kSkyWidth) * 8ull) + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1ull)
        & ~static_cast<uint64_t>(D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1ull);

    float SkyHalfToFloat(uint16_t bits)
    {
    ...
    struct SkyCapture
    {
        ...
        float At(uint32_t x, uint32_t y, uint32_t channel) const
        {
            const auto* row = reinterpret_cast<const uint16_t*>(
                data.data() + static_cast<size_t>(y) * kSkyRowPitch);
            return SkyHalfToFloat(row[x * 4 + channel]);
        }
...
bool EnhancedSceneRenderer::RunSkyBoxTest(std::string& outLog)
{
    ...
    if (!psoManager.Initialize(resources.GetDevice(), L"dx12_skybox.cache", error) ||
    ...
    ComPtr<ID3D12Resource> readback;
    {
        ...
        desc.Width = kSkyRowPitch * kSkyHeight;
        desc.Height = 1;
        ...
        -> bool
    {
        ...
        graph.AddPass("SkyBox.Readback",
            ...
            [&](const EnhancedRenderGraph::ExecuteContext& executeContext)
            {
                ...
                dst.PlacedFootprint.Footprint.Width = kSkyWidth;
                dst.PlacedFootprint.Footprint.Height = kSkyHeight;
                dst.PlacedFootprint.Footprint.Depth = 1;
                dst.PlacedFootprint.Footprint.RowPitch = static_cast<UINT>(kSkyRowPitch);
        ...
        void* mapped = nullptr;
        D3D12_RANGE range{ 0, static_cast<SIZE_T>(kSkyRowPitch * kSkyHeight) };
        ...
        outCapture.data.assign(static_cast<const uint8_t*>(mapped),
            static_cast<const uint8_t*>(mapped) + kSkyRowPitch * kSkyHeight);
