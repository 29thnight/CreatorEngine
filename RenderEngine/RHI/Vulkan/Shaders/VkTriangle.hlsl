...
// ★ 텍스처를 안 만지는 짝 (5c-4d). **레이아웃이 `Cbv(0)` 하나뿐인 파이프라인**
//   을 만들 수 있어야 하고, 그것이 그리드 패스의 레이아웃과 글자 그대로 같다.
//
//   왜 필요한가: 5c-4d 가 세운 것은 "업로드 링에서 자른 슬라이스를 상수
//   버퍼로 건다"인데, 그 경로를 **부르지 않으면 죽었는지 살았는지 알 수
//   없다**(T4). 위 `PSMain` 으로는 못 부른다 — 디스크립터 테이블을 함께
//   써야 하고 그쪽은 소비자가 없어 아직 미구현이기 때문이다.
//
//   틴트를 그대로 낸다. 디스크립터가 안 걸리면 상수가 0 이라 **검은 삼각형**
//   이 되므로, 픽셀 하나로 "닿았다"가 갈린다 — V8-a 가 같은 방법으로 상수
//   경로를 재고 "안 걸면 중앙이 (0,0,0)"이라고 적어 둔 그 자다.
float4 PSMainTint(VSOut input) : SV_Target0
{
