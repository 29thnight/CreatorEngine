...
void VolumetricFogPass::CreateRenderCommandList(RHICommandContext& context, RenderScene& scene, Camera& camera)
{
	...
	Mathf::Matrix viewProjMat = renderData->GetFrameSnapshot().view * renderData->GetFrameSnapshot().projection;
	...
	data.CameraPosition = XMFLOAT4{ renderData->GetFrameSnapshot().eyePosition.m128_f32[0], renderData->GetFrameSnapshot().eyePosition.m128_f32[1], renderData->GetFrameSnapshot().eyePosition.m128_f32[2], 1.0f };
	...
	mPrevViewProj = XMMatrixTranspose(renderData->GetFrameSnapshot().view * renderData->GetFrameSnapshot().projection);
	...
	compositeData.ViewProj = XMMatrixTranspose(renderData->GetFrameSnapshot().view * renderData->GetFrameSnapshot().projection);
	compositeData.InvView = renderData->GetFrameSnapshot().inverseView;
	compositeData.InvProj = renderData->GetFrameSnapshot().inverseProjection;
