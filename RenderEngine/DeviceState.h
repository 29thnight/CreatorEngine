...
namespace DirectX11
{
	class DeviceResourceManager : public DLLCore::Singleton<DeviceResourceManager>
	{
	private:
		friend class DLLCore::Singleton<DeviceResourceManager>;
		DeviceResourceManager() = default;
		~DeviceResourceManager() = default;

	public:
		ID3D11Device3*				g_pDevice{};
		ID3D11DeviceContext3*		g_pDeviceContext{};
		ID3D11DepthStencilView*		g_pDepthStencilView{};
		ID3D11DepthStencilView*		g_pEditorDepthStencilView{};
		...
		ID3DUserDefinedAnnotation*	g_annotation{};
		DirectX11::Sizef			g_ClientRect{};
		float						g_aspectRatio{};
		std::atomic<int>			g_renderCallCount{};
	};
	static auto DeviceStates = DeviceResourceManager::GetInstance();

	inline void ResetCallCount()
	{
		DirectX11::DeviceStates->g_renderCallCount = 0;
	}

	inline int GetDrawCallCount()
	{
		return DirectX11::DeviceStates->g_renderCallCount.load();
	}

	inline ID3D11Buffer* CreateBuffer(uint32 size, D3D11_BIND_FLAG bindFlag, const void* data = nullptr)
	{
		if (!DeviceStates->g_pDevice)
		{
	...
	inline void CreateSamplerState(D3D11_SAMPLER_DESC* pSamplerDesc, ID3D11SamplerState** ppSamplerState)
	{
		...
		DirectX11::ThrowIfFailed(
			DirectX11::DeviceStates->g_pDevice->CreateSamplerState(
				pSamplerDesc,
				ppSamplerState
			)
		);
	...
	{
		return DirectX11::DeviceStates->g_aspectRatio;
	}

	inline float GetWidth()
	{
		return DirectX11::DeviceStates->g_ClientRect.width;
	}

	inline float GetHeight()
	{
		return DirectX11::DeviceStates->g_ClientRect.height;
	}
#ifndef BUILD_FLAG
	//[unsafe]
	inline void BeginEvent(const std::wstring_view& name)
	{
		if (!DirectX11::DeviceStates->g_annotation)
		{
			Debug->LogError("[RenderEngine] -> Annotation is not initialized");
			return;
		}
		DirectX11::DeviceStates->g_annotation->BeginEvent(name.data());
	}
	...
	inline void EndEvent()
	{
		...
		DirectX11::DeviceStates->g_annotation->EndEvent();
	}
#else
	...
	inline void UpdateBuffer(ID3D11Buffer* buffer, const void* data)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->UpdateSubresource(buffer, 0, nullptr, data, 0, 0);
	}
	...
	inline void VSSetConstantBuffer(uint32 slot, uint32 numBuffers, ID3D11Buffer* const * buffer)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->VSSetConstantBuffers(slot, 1, buffer);
	}
	...
	inline void IASetVertexBuffers(uint32 startSlot, uint32 numBuffers, ID3D11Buffer* const* buffer, const uint32* stride, const uint32* offset)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->IASetVertexBuffers(startSlot, numBuffers, buffer, stride, offset);
	}
	...
	inline void IASetIndexBuffer(ID3D11Buffer* buffer, DXGI_FORMAT format, uint32 offset)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->IASetIndexBuffer(buffer, format, offset);
	}
	...
	inline void DrawIndexed(uint32 indexCount, uint32 startIndexLocation, int baseVertexLocation)
	{
		...
		DirectX11::DeviceStates->g_renderCallCount++;
		DirectX11::DeviceStates->g_pDeviceContext->DrawIndexed(indexCount, startIndexLocation, baseVertexLocation);
	}
	...
	inline void DrawIndexedInstanced(uint32 indexCountPerInstance, uint32 instanceCount, uint32 startIndexLocation, int baseVertexLocation, uint32 startInstanceLocation)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->DrawIndexedInstanced(indexCountPerInstance, instanceCount, startIndexLocation, baseVertexLocation, startInstanceLocation);
	}
	...
	inline void ClearRenderTargetView(ID3D11RenderTargetView* renderTargetView, const float color[4])
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->ClearRenderTargetView(renderTargetView, color);
	}
	...
	inline void ClearDepthStencilView(ID3D11DepthStencilView* depthStencilView, uint32 clearFlags, float depth, uint8 stencil)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->ClearDepthStencilView(depthStencilView, clearFlags, depth, stencil);
	}
	...
	inline void OMSetRenderTargets(uint32 numViews, ID3D11RenderTargetView* const* renderTargetViews, ID3D11DepthStencilView* depthStencilView)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->OMSetRenderTargets(numViews, renderTargetViews, depthStencilView);
	}
	...
	inline void PSSetConstantBuffer(uint32 slot, uint32 numBuffers, ID3D11Buffer* const* buffer)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->PSSetConstantBuffers(slot, numBuffers, buffer);
	}
	...
	inline void GSSetConstantBuffer(uint32 slot, uint32 numBuffers, ID3D11Buffer* const* buffer)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->GSSetConstantBuffers(slot, numBuffers, buffer);
	}
	...
	inline void CSSetConstantBuffer(uint32 slot, uint32 numBuffers, ID3D11Buffer* const* buffer)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->CSSetConstantBuffers(slot, numBuffers, buffer);
	}
	...
	inline void Dispatch(uint32 threadGroupCountX, uint32 threadGroupCountY, uint32 threadGroupCountZ)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->Dispatch(threadGroupCountX, threadGroupCountY, threadGroupCountZ);
	}
	...
	inline void PSSetShaderResources(uint32 startSlot, uint32 numViews, ID3D11ShaderResourceView* const* shaderResourceViews)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->PSSetShaderResources(startSlot, numViews, shaderResourceViews);
	}
	...
	inline void CSSetShaderResources(uint32 startSlot, uint32 numViews, ID3D11ShaderResourceView* const* shaderResourceViews)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->CSSetShaderResources(startSlot, numViews, shaderResourceViews);
	}
	...
	inline void CSSetUnorderedAccessViews(uint32 startSlot, uint32 numUAVs, ID3D11UnorderedAccessView* const* unorderedAccessViews, const uint32* initialCounts)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->CSSetUnorderedAccessViews(startSlot, numUAVs, unorderedAccessViews, initialCounts);
	}
    ...
    inline void Draw(uint32 vertexCount, uint32 startVertexLocation)
    {
		...
		DirectX11::DeviceStates->g_renderCallCount++;
        DirectX11::DeviceStates->g_pDeviceContext->Draw(vertexCount, startVertexLocation);
    }
	...
	inline void ExecuteCommandList(ID3D11CommandList* pCommandList, BOOL RestoreContextState)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->ExecuteCommandList(pCommandList, RestoreContextState);
	}
    ...
    inline void UnbindRenderTargets()
    {
        ...
        DirectX11::DeviceStates->g_pDeviceContext->OMSetRenderTargets(1, &nullRTV, nullptr);
    }
	...
	inline void VSSetShader(ID3D11VertexShader* vertexShader, ID3D11ClassInstance* const* classInstances, uint32 numClassInstances)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->VSSetShader(vertexShader, classInstances, numClassInstances);
	}
	...
	inline void PSSetShader(ID3D11PixelShader* pixelShader, ID3D11ClassInstance* const* classInstances, uint32 numClassInstances)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->PSSetShader(pixelShader, classInstances, numClassInstances);
	}
	...
	inline void CSSetShader(ID3D11ComputeShader* computeShader, ID3D11ClassInstance* const* classInstances, uint32 numClassInstances)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->CSSetShader(computeShader, classInstances, numClassInstances);
	}
	...
	inline void IASetInputLayout(ID3D11InputLayout* inputLayout)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->IASetInputLayout(inputLayout);
	}
	...
	inline void IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY topology)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->IASetPrimitiveTopology(topology);
	}
	...
	inline void OMSetDepthStencilState(ID3D11DepthStencilState* depthStencilState, uint32 stencilRef)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->OMSetDepthStencilState(depthStencilState, stencilRef);
	}
	...
	inline void OMSetBlendState(ID3D11BlendState* blendState, const float blendFactor[4], uint32 sampleMask)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->OMSetBlendState(blendState, blendFactor, sampleMask);
	}
	...
	inline void RSSetViewports(UINT NumViewports, const D3D11_VIEWPORT* pViewports)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->RSSetViewports(NumViewports, pViewports);
	}
	...
	inline void InitSetUp()
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->RSSetState(DirectX11::DeviceStates->g_pRasterizerState);
		...
		DirectX11::DeviceStates->g_pDeviceContext->OMSetRenderTargets(1, &rtv, DirectX11::DeviceStates->g_pDepthStencilView);
		DirectX11::DeviceStates->g_pDeviceContext->OMSetDepthStencilState(DirectX11::DeviceStates->g_pDepthStencilState, 1);
	...
	inline void CopyResource(ID3D11Resource* pDstResource, ID3D11Resource* pSrcResource)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->CopyResource(pDstResource, pSrcResource);
	}
	...
	inline void CSSetSamplers(UINT StartSlot, UINT NumSamplers, ID3D11SamplerState* const* ppSamplers)
	{
		...
		DirectX11::DeviceStates->g_pDeviceContext->CSSetSamplers(StartSlot, NumSamplers, ppSamplers);
	}
