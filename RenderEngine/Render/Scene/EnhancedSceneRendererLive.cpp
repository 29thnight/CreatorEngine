...
namespace
{
    ...
    // Vulkan 기본 지오메트리 라이브 경로. 에디터 창 자체는 아직 DX12 ImGui
    // 셸이므로 최종 HDR를 비동기 리드백해 RGBA8로 바꾼 뒤 셸에 넘긴다.
    ...
    struct VulkanLiveGeometryPipeline
    {
            ...
            for (uint32_t y = 0; y < image.height; ++y)
            {
                ...
                for (uint32_t x = 0; x < image.width; ++x)
                {
                    for (uint32_t channel = 0; channel < 3; ++channel)
                    {
                        ...
                        // 기본 라이브 경로의 ACES 후처리가 Vulkan에 아직 없으므로
                        // 표시 브리지에서만 단순 Reinhard + sRGB 감마를 적용한다.
            ...
            std::vector<EnhancedLight>& lights, std::string& outError)
        {
            ...
            if (!meshCache.Initialize(&resources, outError) ||
                !textureCache.Initialize(&resources, outError)) return false;
            ...
            if (!shadow.Initialize(frameContext, outError) ||
                !gbuffer.Initialize(frameContext, outError) ||
                !decal.Initialize(frameContext, outError) ||
                !ssao.Initialize(frameContext, outError) ||
                !deferred.Initialize(frameContext, outError) ||
                !forward.Initialize(frameContext, outError)) return false;
            for (View& view : views)
            {
                if (!view.ssgi.Initialize(frameContext, outError)) return false;
            }
            ...
            for (Slot& slot : slots)
            {
                if (!resources.CreateReadback(width, height,
                    EnhancedForwardPass::kOutputFormat, 1, slot.readback, outError))
                    return false;
            }
            return true;
        }
        ...
        void Shutdown()
        {
            ...
            forward.Shutdown();
            for (View& view : views) view.ssgi.Shutdown();
            deferred.Shutdown();
            ssao.Shutdown();
            decal.Shutdown();
            gbuffer.Shutdown();
        ...
        int FindOrAssignView(const Camera* camera, Camera* const* cameras,
            uint32_t cameraCount)
        {
            std::lock_guard<std::mutex> lock(viewMutex);
            for (uint32_t i = 0; i < EnhancedSceneRenderer::kMaxLiveCameraViews; ++i)
            {
                if (views[i].camera == camera &&
                    views[i].cameraGeneration == camera->m_generation) return static_cast<int>(i);
            }
            uint32_t selected = EnhancedSceneRenderer::kMaxLiveCameraViews;
            for (uint32_t i = 0; i < EnhancedSceneRenderer::kMaxLiveCameraViews; ++i)
            {
                if (nullptr == views[i].camera) { selected = i; break; }
            }
            if (EnhancedSceneRenderer::kMaxLiveCameraViews == selected)
            {
                for (uint32_t i = 0; i < EnhancedSceneRenderer::kMaxLiveCameraViews; ++i)
                {
                    ...
                    {
                        if (views[i].camera == cameras[j]) { stillVisible = true; break; }
                    }
                    if (!stillVisible) { selected = i; break; }
                }
            }
            if (EnhancedSceneRenderer::kMaxLiveCameraViews == selected) return -1;
            views[selected].camera = camera;
            views[selected].cameraGeneration = camera->m_generation;
            views[selected].ready = false;
            views[selected].ssgi.ResetHistory();
            return static_cast<int>(selected);
        }

        bool GetDisplayKey(const Camera* camera, uint64_t& outKey) const
        {
            std::lock_guard<std::mutex> lock(viewMutex);
            for (uint32_t i = 0; i < EnhancedSceneRenderer::kMaxLiveCameraViews; ++i)
            {
                if (views[i].camera == camera && views[i].ready)
                {
                    outKey = kDisplayKeyBase + i + 1u;
                    return true;
                }
            }
            return false;
            ...
            for (Slot& slot : slots)
            {
                ...
                if (resources.MapReadback(slot.readback, image, readbackError) && image.IsValid())
                {
                    bool belongsToView = false;
                    {
                        ...
                        belongsToView = view.camera == slot.camera &&
                            view.cameraGeneration == slot.cameraGeneration;
                        if (belongsToView) view.ready = true;
                    }
                    if (belongsToView)
                    {
        ...
        bool Render(uint32_t viewIndex, const Camera* camera,
            const std::vector<EnhancedDecalPass::Item>& decals, std::string& outError)
        {
            Slot* slot = nullptr;
            ...
            if (!shadow.PrepareFrame(frameContext, outError) ||
                ...
                !decal.PrepareFrame(frameContext, outError) ||
                !ssao.PrepareFrame(frameContext, outError) ||
                !deferred.PrepareFrame(frameContext, outError) ||
                !viewSsgi.PrepareFrame(frameContext, outError) ||
                !forward.PrepareFrame(frameContext, outError)) return false;
            ...
            decal.SetInputs(gbufferOutputs);
            decal.Declare(graph, frameContext);

            EnhancedSSAOPass::Inputs ssaoInputs{};
            ssaoInputs.depth = gbufferOutputs.depth;
            ssaoInputs.normal = gbufferOutputs.normal;
            ssao.SetInputs(ssaoInputs);
            ssao.SetFrameIndex(frameIndex);
            ssao.Declare(graph, frameContext);

            deferred.SetInputs(gbufferOutputs);
            deferred.SetShadow(shadowMap, shadow.GetShadowData());
            deferred.Declare(graph, frameContext);

            EnhancedSSGIPass::Inputs ssgiInputs{};
            ssgiInputs.depth = gbufferOutputs.depth;
            ssgiInputs.normal = gbufferOutputs.normal;
            ssgiInputs.diffuse = gbufferOutputs.diffuse;
            ssgiInputs.metalRough = gbufferOutputs.metalRough;
            ssgiInputs.lighting = deferred.GetOutput();
            ssgiInputs.ambientOcclusion = ssao.GetOutput();
            viewSsgi.SetInputs(ssgiInputs);
            viewSsgi.Declare(graph, frameContext);

            EnhancedForwardPass::Inputs forwardInputs{};
            forwardInputs.depth = gbufferOutputs.depth;
            forwardInputs.lighting = viewSsgi.GetOutput().IsValid()
                ? viewSsgi.GetOutput() : deferred.GetOutput();
            forward.SetInputs(forwardInputs);
            forward.SetShadow(shadowMap, shadow.GetShadowData());
            forward.Declare(graph, frameContext);

            RGHandle output = forward.GetOutput();
            if (!output.IsValid()) output = deferred.GetOutput();
            if (!output.IsValid()) { outError = "Vulkan 라이브 최종 지오메트리 출력이 없다"; return false; }

            graph.AddPass("VulkanLive.Readback",
                { { output, RHIResourceState::CopySource } },
                [slot, output](const EnhancedRenderGraph::ExecuteContext& executeContext)
                {
                    executeContext.encoder->CopyToReadback(
                        slot->readback, executeContext.ResolveHandle(output));
                }, true);
            ...
            slot->camera = camera;
            slot->cameraGeneration = camera->m_generation;
            slot->pending = true;
            return true;
        }
    };
    ...
    struct LiveState
    {
        ...
        std::unique_ptr<VulkanLiveGeometryPipeline> vulkanPipeline;
        ...
        // 외부 interop 격리소 — GPU fence가 아니라 공유 핸들을 읽는 외부 소비자의
        // 종료가 안전 조건이다. 따라서 completion retire queue에 섞지 않고
        // ShutdownLive(렌더 스레드 join 후)에서만 비운다.
        struct ExternalInteropRetired
        {
            ComPtr<ID3D12Resource>           sharedTexture;
            HANDLE                           sharedHandle{ nullptr };
        };
        std::vector<ExternalInteropRetired> externalInteropRetired;
        ...
        void PublishDebugSnapshot()
        {
            ...
            debugSnapshot.graveyardCount = externalInteropRetired.size();
        ...
        bool BuildVulkanPipeline(uint32_t newWidth, uint32_t newHeight,
            std::string& outError)
        {
            vulkanPipeline = std::make_unique<VulkanLiveGeometryPipeline>();
        ...
        void TeardownPipeline()
        {
            ...
            for (LivePipeline::CameraView& view : p.views)
            {
                ...
                for (LivePipeline::DisplaySlot& slot : view.slots)
                {
                    ...
                    ExternalInteropRetired retired;
                    retired.sharedTexture = std::move(slot.sharedTexture);
                    retired.sharedHandle = slot.sharedHandle;
                    externalInteropRetired.push_back(std::move(retired));
...
bool EnhancedSceneRenderer::SetLiveBackend(EnhancedLiveBackend backend,
    std::string& outError)
{
    LiveState& state = GetLiveState();
    if (state.backend == backend) return true;

    // 콘솔 명령은 게임 스레드 프레임 경계에서 실행된다. 이전 백엔드가 가진
    // 그래프·캐시를 completion 뒤에 내린 다음 선택만 바꾼다. 새 파이프라인은
    // 다음 TickLive가 현재 화면 크기와 씬 입력으로 만든다.
    state.TeardownPipeline();
    state.TeardownVulkanPipeline();
    state.backend = backend;
    state.lastError.clear();
    state.consecutiveFrameFailures = 0;
    state.vulkanFirstFrameReported = false;
    outError.clear();
    return true;
}
    ...
    uint32_t cameraCount, bool sceneLoading)
{
    ...
    // ── Vulkan editor TickLive 기본 지오메트리 경로 ──
    //
    // 씬/프록시 수집과 카메라별 밀봉은 위의 백엔드 중립 경로를 그대로 쓴다.
    // 갈리는 것은 여기부터다. 공통 패스 네 개가 Vulkan services/encoder/cache를
    // 받고, 완료된 timeline 슬롯만 CPU 표시 브리지로 승격한다.
    if (EnhancedLiveBackend::Vulkan == state.backend)
    {
        if (state.vulkanPipeline)
        {
            ...
            if (0 != promoted && !state.vulkanFirstFrameReported)
            {
                ...
                Debug->LogWarning("[vulkan.live] editor TickLive 첫 프레임 완성"
                    " · Shadow→GBuffer→Decal→SSAO→Deferred→Forward+"
        ...
        VulkanLiveGeometryPipeline& p = *state.vulkanPipeline;
        ...
        {
            const Camera* camera = cameras[(startIndex + step) % cameraCount];
            ...
            const int viewIndex = p.FindOrAssignView(camera, cameras, cameraCount);
            if (viewIndex < 0)
            {
            ...
            std::string error;
            if (!p.Render(static_cast<uint32_t>(viewIndex), camera, state.decals, error))
            {
...
uint64_t EnhancedSceneRenderer::GetLiveDisplayImTextureId(const Camera* camera)
{
    ...
    if (EnhancedLiveBackend::Vulkan == state.backend)
    {
        if (!state.vulkanPipeline) return 0;
        uint64_t key = 0;
        if (!state.vulkanPipeline->GetDisplayKey(camera, key)) return 0;
        return imGuiHost.GetCpuFrameTextureId(key);
    }

    if (nullptr == state.pipeline) return 0;

    for (const LivePipeline::CameraView& view : state.pipeline->views)
    {
        if (view.camera != camera) continue;
        if (view.displaySlot < 0) return 0;
        return imGuiHost.OpenSharedTexture(
...
std::string EnhancedSceneRenderer::GetLiveStatus()
{
    ...
    {
        char line[512]{};
        const VulkanLiveGeometryPipeline* pipeline = state.vulkanPipeline.get();
        ...
        std::snprintf(line, sizeof(line),
            "EnhancedRenderer(Vulkan) — %s · 기본 지오메트리 파이프라인 %s · %ux%u"
        ...
        status += std::string("\n  표시: Vulkan HDR readback → CPU tone-map → ") +
    ...
    std::snprintf(line, sizeof(line),
        ...
        state.externalInteropRetired.size());
...
void EnhancedSceneRenderer::ShutdownLive()
{
    ...
    for (LiveState::ExternalInteropRetired& retired : state.externalInteropRetired)
    {
        if (nullptr != retired.sharedHandle) ::CloseHandle(retired.sharedHandle);
        retired.sharedTexture.Reset();
    }
    state.externalInteropRetired.clear();
