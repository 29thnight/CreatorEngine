...
{
	// ShellExecute ?®Ïàò ?∏Ï∂ú
	HINSTANCE result = ShellExecute(NULL, L"open", filepath.c_str(), NULL, NULL, SW_SHOWNORMAL);

	// Î∞òÌôòÍ∞íÏù¥ 32 ?¥Ìïò??Í≤ΩÏö∞???§Î•òÎ•??òÌ??ÖÎãà??
	if ((int)result <= 32)
	{
		// ?§Î•ò Ï≤òÎ¶¨: ?àÎ? ?§Ïñ¥, ?êÎü¨ Î©îÏãúÏßÄ Ï∂úÎ†• ?±Ïùò Ï≤òÎ¶¨ Î°úÏßÅ Íµ¨ÌòÑ Í∞Ä??
		MessageBox(NULL, L"?åÏùº ?§Ìñâ???§Ìå®?àÏäµ?àÎã§.", L"Error", MB_OK | MB_ICONERROR);
	}
}
