...
void SpritePass::CreateRenderCommandList(RHICommandContext& context, RenderScene& scene, Camera& camera)
{
    ...
    for (auto& proxy : renderData->m_spriteRenderQueue)
    {
        ...
        if (proxy->m_billboardType != BillboardType::None)
        {
            ...
            {
				Mathf::Vector3 forward = renderData->GetFrameSnapshot().forward;
                world = Mathf::Matrix::CreateBillboard(pos, 
                    renderData->GetFrameSnapshot().eyePosition, proxy->m_billboardAxis, &forward);
            }
            else if (proxy->m_billboardType == BillboardType::Cylindrical)
            {
                ...
                // 1) "ī�޶��� ������ ������"�� �������� ��� (��ġ ����, �׻� -Forward)
                Mathf::Vector3 facing = -renderData->GetFrameSnapshot().forward;   // �ٽ�: look-at ����, -forward ����
                facing.Normalize();
                ...
                if (len2 < 1e-8f)
                {
                    // renderData->GetFrameSnapshot().right�� �ִٸ� �켱 ���
                    Mathf::Vector3 alt = Mathf::Vector3(renderData->GetFrameSnapshot().right).LengthSquared() > 0 ? Mathf::Vector3(renderData->GetFrameSnapshot().right) : Mathf::Vector3::Right;
                    alt.Normalize();
                ...
                //------------------------------------------------------------
                // 1) ���� z (ī�޶� ���� ���� ����)
                float z = (pos - Mathf::Vector3(renderData->GetFrameSnapshot().eyePosition)).Dot(renderData->GetFrameSnapshot().forward);
                z = std::max(z, 1e-3f); // 0/���� ����
                ...
                // �۽���Ƽ��: ���� �������� ���� = 2*z*tan(fovY/2)
                const float frustumH = 2.0f * z * tanf(renderData->GetFrameSnapshot().fov * 0.5f);
