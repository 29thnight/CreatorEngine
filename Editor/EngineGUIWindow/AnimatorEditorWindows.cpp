// 애니메이터 편집 창 셋의 본문 (PHASE 21 M4 3b).
//
// 옛 자리는 `ImGuiDrawHelperAnimator` 안이었다. 셋이 서로 다른 깊이에
// 중첩돼 있어서 — Event는 "animations" 접힘머리 안, Animation Controllers는
// 그 아래 블록, AvatarMask는 컨트롤러 창의 Layers 탭 안 — 부모가 접히거나
// 닫히면 자식 창도 함께 사라졌다. 셸이 프레임을 소유하면 그 종속이 없어지고
// 셋은 서로 독립한 창이 된다.
//
// 본문이 기대던 호출자 지역 변수는 `animator` 하나였다. 그것을 게시하는
// 대신 **다시 유도한다** — 선택 개체의 Animator가 곧 인스펙터가 그리던 그것이라
// 게시 시점과 그리기 시점 사이의 수명 문제가 아예 생기지 않는다.
//
// 창 사이를 건너는 편집 상태는 아바타 마스크가 여는 컨트롤러 번호 하나뿐이다.
// 나머지 세션 상태는 컨트롤러 창 본문의 함수 지역 static으로 남는다.
...
namespace editor::animator_editing
{
    ...
    Animator* current()
    {
        ...
        return selected->GetComponent<Animator>();
    }
...
// 선언 쪽이 부르는 다섯. 선언은 Editor/EditorWindow/Windows 가 하고 구현은
// 여기가 한다 — 유도가 이 파일 것이라 답도 여기서 해야 한다.
namespace editor::windows
{
    void draw_animator_event()        { DrawAnimatorEventWindow(); }
    void draw_animation_controllers() { DrawAnimationControllersWindow(); }
    void draw_avatar_mask()           { DrawAvatarMaskWindow(); }

    bool animator_selected()
    {
        return nullptr != animator_editing::current();
    }

    bool avatar_mask_target_valid()
    {
        // 옛 코드는 번호를 쓰는 자리 바로 위에서 대입했으므로 검사가 없었다.
        // 창이 형제가 되면 선택이 바뀌어 번호가 범위를 벗어난 채 남을 수 있다.
        Animator* animator = animator_editing::current();
        if (nullptr == animator) return false;

        const int index = animator_editing::selected_avatar_controller();
        return 0 <= index &&
               index < static_cast<int>(animator->m_animationControllers.size());
    }
}

namespace
{
    using editor::animator_editing::current;
    using editor::close_window;
    using editor::open_window;
    using editor::is_window_open;

    void toggle_window(const char* window)
    {
        if (is_window_open(window)) close_window(window);
        else                        open_window(window);
    }

    namespace animator_editing = editor::animator_editing;
}

// ── 이벤트 창 ────────────────────────────────────────────────────────────
//
// 옛 자리는 "animations" 접힘머리 안이었다. 머리를 접으면 이 창도 사라졌는데,
// 창을 열어 둔 채 인스펙터를 정리하려던 것이라면 그 종속은 우연이었다.
...
AnimatorClipOverride* clipOverride =
	animator->FindClipOverride(clipIndex);
	...
	for (auto& event : clipOverride->events)
	{
		...
		if (ImGui::InputInt("##frame key", &event.frameKey, 0, 0))
		{
			...
			const int totalKeyFrames = static_cast<int>(
				animator->GetClipFrameCount(clipIndex));
		...
		if(ImGui::Button("delete"))
		{
			...
			animator->DeleteClipEvent(clipIndex, eventIndex);
...
// ── 애니메이션 컨트롤러 창 ───────────────────────────────────────────────
//
// 세션 상태 아홉은 여기 함수 지역 static으로 남는다 — 이 본문 밖에서 읽는
// 것이 없다는 것을 전수로 확인했다. 창을 건너는 것은 아바타 마스크가 여는
// 컨트롤러 번호 하나뿐이고 그것만 편집 문맥으로 올렸다.
...
// ── 아바타 마스크 창 ─────────────────────────────────────────────────────
//
// 옛 자리는 컨트롤러 창의 Layers 탭 안이었다 — 부모 창을 닫거나 다른 탭으로
// 옮기면 함께 사라졌다. 지금은 여는 컨트롤러 번호가 유효한 동안 존재한다.
void DrawAvatarMaskWindow()
{
    ...
    auto& controllers = animator->m_animationControllers;
    const int index = animator_editing::selected_avatar_controller();
    if (index < 0 || index >= static_cast<int>(controllers.size())) return;
