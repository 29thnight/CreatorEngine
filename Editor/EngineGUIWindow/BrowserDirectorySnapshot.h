...
namespace editor
{
    ...
    /// PHASE 21 W2-B — 목록의 **내용**이 바뀔 때만 오르는 번호.
    ///
    /// 창은 결과(항목 포인터 목록)를 이 번호로 기억해 두고, 번호가 그대로면 다시
    /// 모으지도 정렬하지도 않는다. 그래서 이 번호는 두 가지를 함께 약속한다.
    ///   ① 어떤 폴더의 항목이 더해지거나 빠지거나 바뀌었으면 오른다.
    ///   ② 번호가 그대로인 동안 **항목의 주소가 바뀌지 않는다.** 낡아서 다시 훑었는데
    ///      내용이 같으면 목록 객체를 갈아 끼우지 않는다 — 갈아 끼우면 번호를 올린다.
