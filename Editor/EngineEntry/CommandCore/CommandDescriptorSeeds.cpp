...
namespace CommandCore
{
    namespace
    {
        ...
        constexpr DescriptorSeed kSeeds[] = {
            ...
            { "dump.show", CommandCost::Immediate, "[이름]", "가장 최근 덤프(또는 지정한 것)의 요약 내용까지 찍는다", CommandClass::Probe, CommandLiveness::Live },
            ...
            { "dx12.encoderbench", CommandCost::Frames, "", "인코더 오버헤드 실측(R3 착수 조건 · Release 전용)", CommandClass::Probe, CommandLiveness::Live },
            { "dx12.fog", CommandCost::Frames, "", "볼류메트릭 포그 검증(산란·누적 투과율·시간축 히스토리·합성)", CommandClass::Probe, CommandLiveness::Live },
            { "dx12.forward", CommandCost::Long, "", "DX12 forward 패스를 격리 씬에서 리드백으로 판정한다", CommandClass::Probe, CommandLiveness::Live },
            { "dx12.forwardscale", CommandCost::Long, "", "Forward+ 컬링이 광원 수 몇 개부터 이득인지 시간으로 잰다(벤치 · 기본 크기로는 못 돈다)", CommandClass::Probe, CommandLiveness::Live },
            { "dx12.forwardshade", CommandCost::Long, "", "DX12 forward 셰이딩 결과를 리드백으로 판정한다", CommandClass::Probe, CommandLiveness::Live },
            ...
            { "dx12.live", CommandCost::Immediate, "on|status", "EnhancedRenderer 메인 런타임 상태", CommandClass::EngineService, CommandLiveness::Live },
            ...
            { "dx12.postscale", CommandCost::Frames, "", "Uber post 한 패스 대 플래그별 4 패스의 시간을 비교한다(벤치)", CommandClass::Probe, CommandLiveness::Live },
            ...
            { "dx12.scene", CommandCost::Long, "", "씬 연결 검증(카메라 스냅샷·메시 업로드·실제 드로우)", CommandClass::Probe, CommandLiveness::Live },
            ...
            { "dx12.ssaoscale", CommandCost::Frames, "", "반해상도 SSAO 대 옛 64샘플 전해상도의 시간을 비교한다(벤치)", CommandClass::Probe, CommandLiveness::Live },
            ...
            { "mem.delta", CommandCost::Immediate, "[라벨]", "기준선 대비 CRT 블록·바이트 증감(기준선이 없으면 지금을 기준선으로 삼는다)", CommandClass::Probe, CommandLiveness::Live },
            { "mem.hook", CommandCost::Immediate, "on|stack|off|top|status", "CRT 할당 훅 — 호출 계수, stack 은 귀속까지(디버그 CRT 전용)", CommandClass::Probe, CommandLiveness::Live },
            { "mem.reset", CommandCost::Immediate, "", "churn 누계와 기준선을 0으로 — 구간 측정용", CommandClass::Probe, CommandLiveness::Live },
            { "mem.stats", CommandCost::Immediate, "[라벨]", "CRT 현재 live 블록·바이트를 찍는다(누계는 mem.hook 이 따로 센다)", CommandClass::Probe, CommandLiveness::Live },
            ...
            { "render.backend", CommandCost::Immediate, "status", "부팅 시 고정된 scene/ImGui RHI 조회(변경은 Settings)", CommandClass::EngineService, CommandLiveness::Live },
            { "render.livecheck", CommandCost::Immediate, "[너비 높이]", "resize·다중 뷰·표시 슬롯 회전 회귀 판정", CommandClass::Probe, CommandLiveness::Live },
            { "render.matmode", CommandCost::Immediate, "<오브젝트> <opaque|transparent>", "오브젝트 재질의 렌더링 모드를 바꾼다", CommandClass::EngineService, CommandLiveness::Live },
            { "render.rtinfo", CommandCost::Immediate, "", "창·뷰포트·추종 텍스처 크기를 나란히 찍는다", CommandClass::EngineService, CommandLiveness::Live },
            { "render.shadowinfo", CommandCost::Immediate, "", "그림자 캐스케이드 계산 결과를 출력한다(스냅샷 검증용)", CommandClass::EngineService, CommandLiveness::Live },
            ...
            { "selftest", CommandCost::Long, "[<검사 이름>]", "등록된 검증 검사를 이름으로 돌린다(인자 없으면 목록)", CommandClass::Probe, CommandLiveness::Live },
            ...
            { "vk.decal", CommandCost::Long, "", "Decal 공용 패스 — GBuffer snapshot·depth-read·MRT blend 대조", CommandClass::Probe, CommandLiveness::Live },
            ...
            { "vk.gbuffer", CommandCost::Long, "", "GBuffer 공용 패스 — MRT5·texture·sampler·mesh DX12/Vulkan 대조", CommandClass::Probe, CommandLiveness::Live },
