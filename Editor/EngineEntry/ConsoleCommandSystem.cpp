...
namespace ConsoleCmd
{
    ...
    const Table& GetTable()
    {
        static const Table table = []
        {
			...
			regResult({ "scene.transformwritestats" }, [](const ConsoleCommandContext& c) { if (c.parts.size() > 2 || (c.parts.size() == 2 && c.parts[1] != "0" && c.parts[1] != "1" && c.parts[1] != "print")) return CommandCore::InvalidArguments("scene.transformwritestats accepts 0|1|print", "arguments.invalid"); return HandleSceneTransformWriteStats(c.parts); });
			regResult({ "scene.sparseresolver" }, [](const ConsoleCommandContext& c) { if (c.parts.size() > 2 || (c.parts.size() == 2 && c.parts[1] != "0" && c.parts[1] != "1" && c.parts[1] != "print")) return CommandCore::InvalidArguments("scene.sparseresolver accepts 0|1|print", "arguments.invalid"); return HandleSceneSparseResolver(c.parts); });
			regResult({ "scene.transformpull" }, [](const ConsoleCommandContext& c) { if (c.parts.size() > 2 || (c.parts.size() == 2 && c.parts[1] != "print")) return CommandCore::InvalidArguments("scene.transformpull accepts print", "arguments.invalid"); return HandleSceneTransformPull(c.parts); });
