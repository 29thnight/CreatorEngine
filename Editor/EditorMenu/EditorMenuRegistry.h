...
namespace editor
{
    ...
    /// 제품 표를 옆으로 치우고 빈 표를 남긴다.
    ///
    /// ★ 이것이 없으면 자가 검사는 **부팅 전 한순간에만** 돌 수 있다. 합성 선언이
    ///   "표에 항목 1개"를 단정하므로 제품 등록이 끝난 뒤에는 성립하지 않고, 그러면
    ///   도는 회귀 세트에 넣을 수 없다 — 이 저장소에서 "게이트가 도는 세트에 없으면
    ///   없는 것"으로 두 번 데었다(창 쪽 자가 검사도 M4 4단계에서 같은 장치를 달았다).
    ///
    /// 이미 치워져 있으면 `false`. 겹쳐 치우면 앞서 치운 것이 사라지므로 막는다.
    bool stash_menu_registry();

    /// 치워 둔 표를 되돌린다. 현재 표는 버린다. 치워 둔 것이 없으면 `false`.
