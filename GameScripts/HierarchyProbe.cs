...
public sealed partial class HierarchyProbe : Behaviour
{
    ...
    public override void OnInitialized()
    {
        ...
        Assert("Index >= 0", GameObject.Index >= 0, $"{GameObject.Index}");
        ...
        foreach (GameObject child in GameObject.Children)
        {
            ...
            Assert($"'{child.Name}'의 부모가 자신", child.Parent == GameObject,
                $"{child.Parent.Name} != {GameObject.Name}");
        }
        Assert("Children 순회 개수 일치", walked == childCount, $"{walked} vs {childCount}");

        // 범위를 벗어난 인덱스는 죽지 않고 무효 핸들을 준다.
        Assert("범위 밖 자식은 무효", !GameObject.GetChild(childCount + 10).IsAlive, "살아 있다고 나옴");

        // 인덱스로 되찾기.
        Assert("FindByIndex 왕복", GameObject.FindByIndex(GameObject.Index) == GameObject, "다른 오브젝트가 나옴");

        // 자손 탐색. 자기 자신 포함 여부에 따라 결과가 달라야 한다.
        var withSelf = GameObject.GetComponentsInChildren<HierarchyProbe>();
        var withoutSelf = GameObject.GetComponentsInChildren<HierarchyProbe>(includeSelf: false);
        ...
        Assert("GetComponentInParent가 자신을 찾음",
            GameObject.GetComponentInParent<HierarchyProbe>() is not null, "null이 나옴");
