...
#include <math.h>
namespace Mathf {
constexpr float EasingS = 1.70158f;
constexpr float EasingA = 1;
constexpr float EasingP = 0.3f;
class Easing {
public:
...
public:
	...
	inline static float EaseInBack(float t) {
		return t * t * ((EasingS + 1) * t - EasingS);
	}
	inline static float EaseOutBack(float t) {
		t -= 1;
		return t * t * ((EasingS + 1) * t + EasingS) + 1;
	}
	inline static float EaseInOutBack(float t) {
		if (t < 0.5f) {
			t *= 2;
			return 0.5f * (t * t * ((EasingS + 1) * t - EasingS));
		}
		else {
			t = 2 * t - 2;
			return 0.5f * (t * t * ((EasingS + 1) * t + EasingS) + 2);
		}
	}
	inline static float EaseInElastic(float t) {
		if (t == 0 || t == 1) return t;
		float s = EasingP / 4;
		return -(EasingA * pow(2, 10 * (t - 1)) * sin((t - 1 - s) * (2 * M_PI) / EasingP));
	}
	inline static float EaseOutElastic(float t) {
		if (t == 0 || t == 1) return t;
		float s = EasingP / 4;
		return EasingA * pow(2, -10 * t) * sin((t - s) * (2 * M_PI) / EasingP) + 1;
	}
	inline static float EaseInOutElastic(float t) {
		if (t == 0 || t == 1) return t;
		float s = EasingP / 4;
		if (t < 0.5f) {
			return -0.5f * (EasingA * pow(2, 20 * t - 10) * sin((20 * t - 11.125) * (2 * M_PI) / EasingP));
		}
		else {
			return EasingA * pow(2, -20 * t + 10) * sin((20 * t - 11.125) * (2 * M_PI) / EasingP) + 1;
		}
	}
	...
	inline static float EaseInElasticCustom(float t) {
		if (t == 0 || t == 1) return t;
		float s = EasingP / 4;
		return -(EasingA * pow(2, 10 * (t - 1)) * sin((t - 1 - s) * (2 * M_PI) / EasingP));
	}
	inline static float EaseOutElasticCustom(float t) {
		if (t == 0 || t == 1) return t;
		float s = EasingP / 4;
		return EasingA * pow(2, -10 * t) * sin((t - s) * (2 * M_PI) / EasingP) + 1;
	}
	inline static float EaseInOutElasticCustom(float t) {
		if (t == 0 || t == 1) return t;
		float s = EasingP / 4;
		if (t < 0.5f) {
			return -0.5f * (EasingA * pow(2, 20 * t - 10) * sin((20 * t - 11.125) * (2 * M_PI) / EasingP));
	...
	inline static std::function<float(float)> EasingFunction[static_cast<int>(EasingType::EasingEffectEnd)] =
	{
		// Linear
		&Linear,
