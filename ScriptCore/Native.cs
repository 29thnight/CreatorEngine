...
internal unsafe struct ScriptApiTable
{
    ...
    // EffectComponent (획득 50회로 네이티브 1위)
    public delegate* unmanaged<ObjectHandle, int> Effect_Exists;
    public delegate* unmanaged<ObjectHandle, void> Effect_Initialize;
    public delegate* unmanaged<ObjectHandle, void> Effect_Apply;
    public delegate* unmanaged<ObjectHandle, void> Effect_Stop;
    public delegate* unmanaged<ObjectHandle, void> Effect_Pause;
    public delegate* unmanaged<ObjectHandle, void> Effect_Resume;
    public delegate* unmanaged<ObjectHandle, void> Effect_ForceFinish;
    public delegate* unmanaged<ObjectHandle, byte*, void> Effect_ChangeEffect;
    public delegate* unmanaged<ObjectHandle, byte*, void> Effect_PlayByName;
    public delegate* unmanaged<ObjectHandle, byte*, int, int> Effect_GetTemplateName;
    public delegate* unmanaged<ObjectHandle, byte*, void> Effect_SetTemplateName;
    public delegate* unmanaged<ObjectHandle, int> Effect_IsPlaying;
    public delegate* unmanaged<ObjectHandle, int> Effect_IsPaused;
    public delegate* unmanaged<ObjectHandle, int> Effect_GetLoop;
    public delegate* unmanaged<ObjectHandle, int, void> Effect_SetLoop;
    public delegate* unmanaged<ObjectHandle, float> Effect_GetDuration;
    public delegate* unmanaged<ObjectHandle, float, void> Effect_SetDuration;
    public delegate* unmanaged<ObjectHandle, float> Effect_GetTimeScale;
    public delegate* unmanaged<ObjectHandle, float, void> Effect_SetTimeScale;
    public delegate* unmanaged<ObjectHandle, float> Effect_GetCurrentTime;
...
internal static unsafe class Native
{
    ...
    public static bool HasEffect(ObjectHandle h)
        => _ready && _api.Effect_Exists != null && _api.Effect_Exists(h) != 0;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void EffectInitialize(ObjectHandle h)
    {
        if (_ready && _api.Effect_Initialize != null) _api.Effect_Initialize(h);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void EffectApply(ObjectHandle h)
    {
        if (_ready && _api.Effect_Apply != null) _api.Effect_Apply(h);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void EffectStop(ObjectHandle h)
    {
        if (_ready && _api.Effect_Stop != null) _api.Effect_Stop(h);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void EffectPause(ObjectHandle h)
    {
        if (_ready && _api.Effect_Pause != null) _api.Effect_Pause(h);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void EffectResume(ObjectHandle h)
    {
        if (_ready && _api.Effect_Resume != null) _api.Effect_Resume(h);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void EffectForceFinish(ObjectHandle h)
    {
        if (_ready && _api.Effect_ForceFinish != null) _api.Effect_ForceFinish(h);
    }

    public static void EffectChangeEffect(ObjectHandle h, string name)
    {
        if (!_ready || _api.Effect_ChangeEffect == null) return;

        byte* buffer = stackalloc byte[NameBufferSize];
        EncodeName(name, buffer);
        _api.Effect_ChangeEffect(h, buffer);
    }

    public static void EffectPlayByName(ObjectHandle h, string name)
    {
        if (!_ready || _api.Effect_PlayByName == null) return;

        byte* buffer = stackalloc byte[NameBufferSize];
        EncodeName(name, buffer);
        _api.Effect_PlayByName(h, buffer);
    }

    public static string EffectGetTemplateName(ObjectHandle h)
    {
        if (!_ready || _api.Effect_GetTemplateName == null) return string.Empty;

        byte* buffer = stackalloc byte[NameBufferSize];
        int len = _api.Effect_GetTemplateName(h, buffer, NameBufferSize);
        return len > 0 ? System.Text.Encoding.UTF8.GetString(buffer, len) : string.Empty;
    }

    public static void EffectSetTemplateName(ObjectHandle h, string name)
    {
        if (!_ready || _api.Effect_SetTemplateName == null) return;

        byte* buffer = stackalloc byte[NameBufferSize];
        EncodeName(name, buffer);
        _api.Effect_SetTemplateName(h, buffer);
    }

    public static bool EffectIsPlaying(ObjectHandle h)
        => _ready && _api.Effect_IsPlaying != null && _api.Effect_IsPlaying(h) != 0;

    public static bool EffectIsPaused(ObjectHandle h)
        => _ready && _api.Effect_IsPaused != null && _api.Effect_IsPaused(h) != 0;

    public static bool EffectGetLoop(ObjectHandle h)
        => _ready && _api.Effect_GetLoop != null && _api.Effect_GetLoop(h) != 0;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void EffectSetLoop(ObjectHandle h, bool loop)
    {
        if (_ready && _api.Effect_SetLoop != null) _api.Effect_SetLoop(h, loop ? 1 : 0);
    }

    public static float EffectGetDuration(ObjectHandle h)
        => _ready && _api.Effect_GetDuration != null ? _api.Effect_GetDuration(h) : 0f;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void EffectSetDuration(ObjectHandle h, float duration)
    {
        if (_ready && _api.Effect_SetDuration != null) _api.Effect_SetDuration(h, duration);
    }

    public static float EffectGetTimeScale(ObjectHandle h)
        => _ready && _api.Effect_GetTimeScale != null ? _api.Effect_GetTimeScale(h) : 0f;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void EffectSetTimeScale(ObjectHandle h, float timeScale)
    {
        if (_ready && _api.Effect_SetTimeScale != null) _api.Effect_SetTimeScale(h, timeScale);
    }

    public static float EffectGetCurrentTime(ObjectHandle h)
        => _ready && _api.Effect_GetCurrentTime != null ? _api.Effect_GetCurrentTime(h) : 0f;
