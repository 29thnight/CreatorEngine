...
/// <summary>
/// 이름으로 <see cref="AniBehavior"/>를 만들고 인스턴스를 들고 있는다.
///
/// <see cref="ScriptFactory"/>와 같은 구조지만 목록을 따로 둔다 — 두 계열은 수명이
/// 완전히 다르다. Component는 오브젝트에 붙어 살고, AniBehavior는 애니메이션 상태가
/// 만들고 지운다.
/// </summary>
internal static class AniBehaviorFactory
{
    private static readonly Dictionary<string, Func<AniBehavior>> _creators = new();
    private static readonly Dictionary<int, AniBehavior> _instances = new();

    private static int _nextInstanceId = 1;

    public static void Register(string typeName, Func<AniBehavior> creator) => _creators[typeName] = creator;
    ...
    public static AniBehavior? Find(int instanceId)
