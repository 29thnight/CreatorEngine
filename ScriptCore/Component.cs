...
/// ── 이름이 왜 Component인가 ──
///
/// 네이티브 <c>Component</c>(Component.h)와 같은 층이기 때문이다. 그쪽에서 Transform도
/// MeshRenderer도 스크립트를 담는 ScriptComponent도 전부 Component를 상속하고,
/// 6단계 생명주기를 갖는 것도 그 층이다. 관리 측이 같은 이름·같은 축을 쓰면 두 계층을
/// 대조할 때 옮겨 적을 것이 없다.
///
/// 예전 이름은 <c>Behaviour</c>였다. Unity 관례를 따른 것으로, 옛 C++ 스크립트 348개를
/// 구조 변경 없이 옮기려던 계획(설계 문서 02절)의 산물이다. 그 스크립트들은 포팅되지
/// 않고 폐기됐고, 남은 것은 엔진 구조와 어긋나는 이름뿐이라 버렸다.
///
/// 라이프사이클은 네이티브가 스크립트마다 부르지 않는다. 틱마다 한 번만 경계를 넘고,
/// 순회는 <see cref="ScriptRegistry"/>가 관리 영역에서 수행한다.
/// </summary>
public abstract class Component
{
    /// <summary>이 컴포넌트가 붙어 있는 오브젝트.</summary>
    ...
    /// ── 없는 오브젝트에서 부르면 예외다 ──
    ///
    /// UI/Canvas는 Transform을 갖지 않는다(S3). 예전에는 이 자리가 값 뷰라 그 경우에도
    /// 멀쩡한 객체를 돌려줬고, 거기에 값을 쓰면 네이티브가 공유 더미로 받아 로그 한 줄만
    /// 남기고 버렸다 — 스크립트에서는 성공과 구분되지 않았다. 예외는 그 조용한 소실을
    /// 소란스러운 실패로 바꾼다.
    ///
    /// 없을 수 있는 자리를 다루려면 <see cref="HasTransform"/>으로 먼저 묻거나
    /// <c>Entity.GetComponent&lt;Transform&gt;()</c>을 쓴다(그쪽은 null을 돌려준다).
    ///
    /// 생성 시 한 번 잡아 둔다 — 매번 조회하면 실측 1위인 이 경로에 부재 판정
    /// P/Invoke가 접근마다 하나씩 붙는다. 오브젝트가 Transform을 갖는지는 생성 시점에
    /// 정해지므로 캐시가 낡을 여지는 없다.
    /// </summary>
    public Transform Transform => _transform ?? throw new InvalidOperationException(
        $"'{Entity.Name}'에는 Transform이 없다(UI/Canvas는 RectTransformComponent만 갖는다). " +
        $"{GetType().Name}이 공간을 다루려면 HasTransform으로 먼저 확인하라.");

    /// <summary>Transform을 갖는가. UI/Canvas는 갖지 않는다(S3).</summary>
    public bool HasTransform => _transform is not null;
    ...
    ///
    /// 고아 청소(<see cref="ScriptRegistry.SweepOrphans"/>)가 그 구분을 필요로 한다 —
    ...
    /// 구독은 여기 걸어 두면 OnEndSimulation 직전에 <see cref="ScriptRegistry"/>가
    ...
    ///
    /// 구현은 ScriptGenerator가 만든다 — 스크립트의 public 무인자 void 메서드를
    ...
    public List<T> GetComponents<T>() where T : Component => Entity.GetComponents<T>();
