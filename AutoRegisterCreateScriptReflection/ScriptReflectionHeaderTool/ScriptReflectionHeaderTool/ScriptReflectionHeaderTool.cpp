...
std::string GenerateMacroBlock(const std::string& className, const std::string& inheritance, const std::vector<std::string>& properties, const std::vector<MethodEntry>& methods) {
    ...
    if (!inheritance.empty()) {
        out << "ReflectionScriptFieldInheritance(" << className << ", " << inheritance << ") \\\n{ \\\n";
    }
    else {
        out << "ReflectionScriptField(" << className << ") \\\n{ \\\n";
    }
    ...
    if (!inheritance.empty()) mode += "ScriptInheritance";
...
void ProcessHeaderToMacroFile(const fs::path& filepath)
{
    ...
    std::regex serializableRegex(R"(^\s*\[\[ScriptReflectionField(?:\(Inheritance:(\w+)\))?\]\])");

    // Constructor line에서 클래스 이름 추출
    std::regex constructorRegex(R"(^\s*([A-Za-z_]\w*)\s*\(\s*\)\s*(?:=\s*default)?\s*[;{]?)");

    // MODULE_BEHAVIOR_BODY 매크로에서 클래스 이름 추출
    std::regex generatedBodyRegex(R"(^\s*MODULE_BEHAVIOR_BODY\s*\((\w+)\)\s*;?)");

    std::regex serialRegex(R"(\[\[ScriptReflectionField(?:\(Inheritance:(\w+)\))?\]\])");
...
int main()
{
    ...
    std::filesystem::path root = exeDir.parent_path().parent_path() / "Dynamic_CPP\\Assets\\Script";
    ...
    std::regex serializableRegex(R"(^\s*\[\[ScriptReflectionField(\(.*\))?\]\])");
    std::regex constructorRegex(R"(^\s*([A-Za-z_]\w*)\s*\(\s*\)\s*(?:=\s*default)?\s*[;{]?)");
	...
	std::cout << "Reflection header generation complete.\n";
    return 0;
}
