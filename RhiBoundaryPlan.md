...
| 4 | 자가 검증 전체 통과 · 검증 레이어 0건 | **부분** — 33종 중 27 통과 · 4 계측 · 2 실패. 실패 둘은 이 축 밖의 선재 문제다(`dx12.scene`은 그 검사가 명시한 전제조건, `dx12.gizmoscene`은 착수 전 커밋에서 출력이 글자까지 같음을 확인). stderr 33종 모두 0바이트 |
| 5 | CPU ms 회귀 없음 | 미측정(슬라이스 판정은 판정 줄 대조로 해 왔다) |
| 6 | 구 RHI 제거 | **미착수 · 비용 0에 가까움** — §4 R5 참고 |

★ **완료 기준 넷이 남았는데 그중 둘(1·6)이 사실상 삭제·이동 작업이다.**
남은 실제 설계 몫은 2의 잔여(그래프 서명 = R4-3)와 4의 검증 정리뿐이다.
