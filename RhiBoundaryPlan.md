...
- ~~**패스 셰이더 재작성 없음.** HLSL과 루트 시그니처 레이아웃은 그대로다.~~
  ...
  그러나 `D3DCompile` 93건과 루트 시그니처 기술 160건은 Vulkan에서 각각
...
| 셰이더 컴파일 `D3DCompile`·`ID3DBlob` | 93 | §2가 "범위 밖" | DXC → SPIR-V |
...
| **V5** | 셰이더 컴파일 — `RHIShaderCompiler` | 93 | HLSL 소스는 유지, 컴파일과 리플렉션만 인터페이스로. V4 뒤인 것은 셰이더가 레이아웃과 짝이기 때문 |
| **V6** | 파이프라인 상태 기술 — 래스터·블렌드·뎁스·샘플러 | 107 | 나머지 기술. 여기까지 오면 패스에 `D3D12_`가 남지 않는다 |
| **V7** | R0 — 상위 개념을 `RenderEngine/Render/`로 | 헤더 21 | 이동. 여기서 §6-1이 검증 가능해진다(`Render/`에 `d3d12.h` 0) |
| **V8** | Vulkan 백엔드 골격 | — | **여기서 처음으로 계약이 맞았는지 증명된다** |
...
#### V2-b 크기 실측 (2026-08-10, 착수 후 되돌림)

**계획이 "desc 31곳"이라고 적은 것이 틀렸다.** 실제로 해 보니 **131곳 · 19파일**이다.
착수한 것은 `wip/v2b` 브랜치에 두고 master는 V2-c 완료 지점으로 되돌렸다.
...
이미 된 것(브랜치에 있음): `RHIBindingDesc`·`RHIDepthTargetDesc`가 핸들을
받고, `DX12TextureEntry`가 핸들을 내며(캐시가 표에 등록·은퇴 시 놓기),
`CreateBindings`가 `ResolveBinding`으로 푼다. `Dim::Buffer`만 `bufferResource`
칸을 보게 텍스처/버퍼를 갈라 뒀다.

★ **되돌린 이유.** 131곳은 한 번에 끝내야 하는 덩어리인데(중간은 빌드가
깨진 상태다) 그것을 끝낼 여력이 없는 채로 두면 master가 깨진 채 남는다.
"검증된 지점까지만 master에 둔다"가 이 계획서의 규칙이다.
...
3. `dx12.live status`의 패스 이름 목록이 착수 전과 문자 그대로 같다.
4. 자가 검증 33종 판정이 착수 전과 같다(현재 기준선: 28 통과 · 4 계측 · 1 실패).
