...
| # | 예상 | 결과 |
|---|---|---|
| 1 | 링 핸들 `Resolve` 비용이 `Allocate` 의 175ns 에 묻힌다 — encoderbench 로 잰다 | **결론은 맞고 재는 자가 틀렸다.** encoderbench 는 이 경로를 안 지난다(아래 ★). Release 전후 대조로 다시 재서 확인했다 |
...
#### Release 로 다시 재서 닫았다 (2026-08-11)

Debug 로는 단정할 수 없어 **A-5a 전후를 각각 Release 로 빌드해** 같은 자로 쟀다.
`dx12.forwardscale` 의 광원 256 지점이 대조점이다 — Release 에서 재현성이 높다.

| 광원 256 | 참조 | 셰이딩 |
|---|---|---|
| A-5a 전 | 1.425 | **0.093** |
| A-5a 후 | 1.425 · 1.430 | **0.094 · 0.095** |

★ **참조 경로가 같은 값으로 나온 상태에서** 셰이딩이 1~2% 차이다. 참조는 이
변경과 무관한 단순 루프이므로, 그것이 일치한다는 것이 **기계 상태가 같았다는
증거**다. Debug 에서 20% 차이가 났던 것은 그 조건이 안 맞았기 때문이었다.

  → **표 조회 비용은 측정 분산 안이다.** 예상 1 이 옳았고, 다만 그것을 재는
  자가 encoderbench 가 아니었다.

★ **첫 실행은 버린다.** 워밍업 없이 재면 광원 256 셰이딩이 0.267 로 나온다
(안정값의 2.9배). 두 번째 실행부터 재현된다 — `Invoke-Dx12Suite.ps1` 의
워밍업이 프레임을 도는 것과 같은 이유이고, Release 계측은 **프로세스도**
한 번 데워야 한다.

`dx12.bench11`(Release)도 함께 돌렸다. 원시 경로라 이 슬라이스를 지나지
않지만 기준선 대조는 된다 — 일괄 경로가 "드로우 256건부터 끝까지 DX11보다
싸다"로 판정이 같고, 16384 배율이 1.28 → 1.37 이다.
...
#### A-5b 완료 — 그리고 '모델 교체'가 아니었다 (2026-08-11)

`RHIBindingTable`·`RHISamplerTable` 의 `D3D12_GPU_DESCRIPTOR_HANDLE` 이
**불투명 64비트**가 됐다. 소비처 11곳(인코더 4 · 생성 2 · 원시 경로 5).

★ **V8-b 가 "핸들화가 아니라 모델 교체"라고 적은 것이 절반만 맞았다.** 그때
잰 것은 **레이아웃**이다 — DX12 의 루트 파라미터 하나가 Vulkan 에서 셋
레이아웃의 binding N개로 펼쳐진다. 그런데 그 펼침은 `VulkanPipelineCache` 가
레이아웃을 만들 때 이미 하고 있고, **거는 시점에 넘기는 것은 양쪽 다 하나다**
(DX12 는 GPU 핸들, Vulkan 은 `VkDescriptorSet` — 둘 다 64비트 한 값).

  즉 `RHIRenderTargetBinding`(인덱스 셋 → 뷰 목록)과 이것은 **다른 부류**였다.
  저쪽은 필드 수가 3 → 5 로 늘지만 이쪽은 타입만 갈린다. 자를 세우고 나서야
  둘이 갈렸고, 세우기 전이었다면 "테이블도 모델 교체"로 잡아 크게 벌였을 것이다.

| | 전 | 후 |
|---|---|---|
| `RenderFrameServices.h` 의 `D3D12_GPU_DESCRIPTOR_HANDLE` | 2 (코드) | **0** (주석 2만 남음) |

검증: 솔루션 Debug 오류 0 · dx12 전수 35종 판정 줄 차이 0.

★ 변환 헬퍼(`DX12ToGpuHandle`)는 `DX12DescriptorHeaps.h` 에 뒀다 — **이 값의
출처가 디스크립터 힙**이기 때문이다. 인코더에 두면 "거는 쪽이 뜻을 안다"가
되는데, 뜻을 주는 것은 만드는 쪽이다.

#### A-6 완료 — 인코더에 뷰 둘만 남았다 (2026-08-11)

| 서명 | 전 | 후 |
|---|---|---|
| `UavBarrier` | `span<ID3D12Resource* const>` | `span<const RHITextureHandle>` |
| `CopyResource` | `ID3D12Resource*` ×2 | `RHITextureHandle` ×2 |
| `ClearRenderTargetRect` | `D3D12_RECT` | **`RHIRect`**(신설) |

`RHIEncoder.h` 의 DX12 심볼이 **6 → 2**. 소비처는 넷이었고(Decal 1 · SSGI 2 ·
자가 검증 1) 전부 `Resolve(...)` 로 포인터를 만들어 넘기던 자리라, 핸들이 되면서
**그 `Resolve` 호출도 함께 사라졌다** — `ResolveHandle` 이 답이라고 V2-c 가
적어 둔 그대로다.

★ **`RHIRect` 의 필드 뜻을 DX12 모양으로 정했다.** Vulkan 의 `VkRect2D` 는
오프셋+크기이고 DX12 는 좌상단+우하단이라 어느 한쪽을 골라야 한다. 실사용이
"띠를 잘라 지운다" 하나뿐이고 그 코드가 경계로 생각하므로 경계를 택했다 —
**호출부가 이미 쓰는 어휘를 고르는 것**이 V4 가 어휘를 실사용에서 뽑은 규칙과
같다. Vulkan 백엔드가 뺄셈을 한 번 하면 된다.

★ **`UavBarrier` 는 소비처가 0인 채로 중립화했다.** §3.2 가 이 API 를 둔
근거(그래프가 구조적으로 볼 수 없는 한 패스 안의 UAV 배리어)는 그대로 살아
있고, 지금 0인 것은 패스들이 그 상황을 안 만들 뿐이다. 지우는 대신 중립화한
이유는 **Vulkan 이 구현할 수 있는 서명이 하나 느는 값**이 지우는 값보다 크기
때문이다. 다시 0이 유지되면 A 완료 시점에 재고한다.

**남은 둘은 A-4 의 몫이다.** `SetVertexBuffer`·`SetIndexBuffer` 의 원시
오버로드인데, 벤치만 쓰는 줄 알았더니 **패스 셋이 쓴다**(Forward · GBuffer ·
Shadow) — 메시 캐시가 `DX12MeshEntry` 로 D3D12 뷰를 들고 있기 때문이다.
즉 이 둘은 인코더의 문제가 아니라 **자산 캐시 중립화**(`IRenderMeshCache` 의
반환형)에 딸린 것이고, A-4 에서 함께 닫힌다.
