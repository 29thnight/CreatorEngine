...
| 1 | **A-2** 상태 어휘 잔여 — `initialState` 를 `RHIResourceState` 로 | 5 | 기계적. V3 가 어휘를 이미 만들어 뒀다 |
| 2 | **V8-b** — 삼각형에 텍스처 하나 · 샘플러 하나 · 프레임마다 바뀌는 상수를 물린다 | — | 여기서 **A-5 의 자가 선다**. 디스크립터 셋/풀 모델이 DX12 링과 어디서 갈리는지가 실물로 나온다 |
