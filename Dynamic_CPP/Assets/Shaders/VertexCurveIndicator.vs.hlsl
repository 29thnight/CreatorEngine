...
VertexShaderOutput main(AppData IN)
{
    ...
    OUT.normal = normalize(mul(view, float4(IN.normal, 0)).xyz);
    OUT.tangent = normalize(mul(view, normalize(float4(IN.tangent, 0))).xyz);
    OUT.binormal = normalize(mul(view, normalize(float4(IN.binormal, 0))).xyz);
    return OUT;
}
