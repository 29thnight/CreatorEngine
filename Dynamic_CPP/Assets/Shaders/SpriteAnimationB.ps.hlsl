...
float4 main(VSOutput input) : SV_TARGET
{
    ...
    float uvX = input.TexCoord.x;
    float uvY = input.TexCoord.y;
    
    float clampY = clamp(uvY, 0, 0.5);
    float lerpY = lerp(uvY - 1, 1 - uvY , uvX);
    float oneminus = 1 - dissolve;
    
    float k = 1 - clampY;
    float s = smoothstep(clampY, k, lerpY + oneminus);
