...
GBufferOutput main(PixelShaderInput IN)
{
    ...
    if (useTerrainLayers && numLayers > 0)
    {
        ...
        roughness = 1.0;
        bit = 1 << 8;
        bit |= 1 << 9; // 터레인 레이어 비트 설정d
        ior = 1.0f;
    }
    
    //float s2 = sin(IN.wPosition.y) * 5.f + sin(IN.wPosition.x) * 5.f;
    float s = smoothstep(pos1, pos2, IN.wPosition.x);
    
    
    float3 c = lerp(color1, color2, s);
    c = SRGBtoLINEAR(float4(c, 1)).rgb;
