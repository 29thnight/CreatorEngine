...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float3 color = (      UpDualFilteringTexture.SampleLevel(LinearSampler, (uv + float2( 1.0, 0.0)) * invSize, 0)
                 +        UpDualFilteringTexture.SampleLevel(LinearSampler, (uv + float2(-1.0, 0.0)) * invSize, 0)
                 +        UpDualFilteringTexture.SampleLevel(LinearSampler, (uv + float2( 0.0, 1.0)) * invSize, 0)
                 +        UpDualFilteringTexture.SampleLevel(LinearSampler, (uv + float2( 0.0,-1.0)) * invSize, 0)
                 +  2.0 * UpDualFilteringTexture.SampleLevel(LinearSampler, (uv + float2( 0.5, 0.5)) * invSize, 0)
                 +  2.0 * UpDualFilteringTexture.SampleLevel(LinearSampler, (uv + float2(-0.5, 0.5)) * invSize, 0)
                 +  2.0 * UpDualFilteringTexture.SampleLevel(LinearSampler, (uv + float2( 0.5,-0.5)) * invSize, 0)
                 +  2.0 * UpDualFilteringTexture.SampleLevel(LinearSampler, (uv + float2(-0.5,-0.5)) * invSize, 0)) / 12.0;
