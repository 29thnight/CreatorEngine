...
float4 qmul(float4 q2, float4 q1)
{
    return float4(
...
VertexOutput main(VertexInput input)
{
    ...
    // 쿼터니언 방식으로 회전 처리
    float4 localEuler = EulerToQuaternion(float3(3.141592653 / 2, 0, 0));
    float3x3 localMat = QuaternionToMatrix(localEuler);
    
    float4 quaternion = EulerToQuaternion(particle.rotation);
    quaternion = qmul(quaternion, localEuler);
    float3x3 rotMatrix = localMat; //QuaternionToMatrix(quaternion);
    ...
    float3 viewDir = float3(gView._13, gView._23, gView._33); //normalize(gCameraPosition - particle.position);
    
    float3 cameraRight = float3(gView._11, gView._21, gView._31);
    float3 cameraUp = float3(gView._12, gView._22, gView._32);
    
    finalPos = particle.position +
                  rotatedPos.x * cameraRight +
                  rotatedPos.y * cameraUp;
