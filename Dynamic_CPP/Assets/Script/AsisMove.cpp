...
void AsisMove::Update(float tick)
{
	...
	Vector3 predictNextPosition = currentPosition + currentForward * moveSpeed * predictNextTime;

	Vector3 dir = Mathf::Normalize(points[currentPointIndex] - points[nextPointIndex]); // B → A 방향 단위 벡터
	Vector3 endResult = points[nextPointIndex] + dir * pathRadius;
	Vector3 startResult = points[currentPointIndex] - dir * pathRadius;
	...
	if (Mathf::Distance(predictNextPosition, closestPoint) > pathRadius) {
		...
		Quaternion newRot = Quaternion::Slerp(currentRotation, rot, rotateSpeed * tick);
