	...
	case EPatternPhase::Spawning:
	{
		...
		while (BPTimer >= BP0013delay)
		{
			if (pattenIndex < 3)
			{
				...
				dir.Normalize();
				Mathf::Vector3 pos = ownerPos + (dir * 2.0f) + (Mathf::Vector3(0, 1, 0) * 0.5);
				ShootProjectile(pattenIndex, pos, dir); //--> 에니메이션에서 발사 프레임에 맞춰서 호출하는 방식으로 변경 가능
...
void TBoss1::Update_BP0021(float tick)
{
	...
	case EPatternPhase::Spawning:
	{
		...
		// 패턴 시작시 타겟을 바라보게 회전 하는데 여기서 타겟을 따라가는게 맞는가? 
		Mathf::Vector3 ownerPos = GetOwner()->GetComponent<Transform>()->GetWorldPosition();
		//Mathf::Vector3 targetPos = m_target->GetComponent<Transform>()->GetWorldPosition();
		...
		//SweepAttack(ownerPos, dir); // --> 에니메이션에서 스윕 공격 프레임에 맞춰서 호출하는 방식으로 변경 가능
		if (BPTimer >= 2.0f){ //에니메이션 2초로 가정
			SweepAttack(ownerPos, forward); // --> 에니메이션에서 스윕 공격 프레임에 맞춰서 호출하는 방식으로 변경 가능
			BPTimer -= 2.0f;
		}
...
void TBoss1::SelectTarget()
{
	...
	if (has1P && !has2P) {
		//1P만 존재
		m_target = BB->GetValueAsGameObject("1P"); //무조건 1P
		return;
	}
