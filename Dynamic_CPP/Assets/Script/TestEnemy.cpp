...
void TestEnemy::Update(float tick)
{
	...
	Mathf::Vector3 enemyPos = Owner->Transform_().GetWorldPosition();
	/*for (auto& a : asis) {
		Mathf::Vector3 asisPos = a->GetOwner()->Transform_().GetWorldPosition();
	...
	for (auto& p : players) {
		Mathf::Vector3 playerPos = p->GetOwner()->Transform_().GetWorldPosition();
