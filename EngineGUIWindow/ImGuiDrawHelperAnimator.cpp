...
// DataSystemsê°€ ì—¬ê¸° ìžˆë‹¤. ìœ ë‹ˆí‹° ë¹Œë“œì—ì„œëŠ” ì•žì„  íŒŒì¼ì´ ê³µê¸‰í–ˆë‹¤.
		...
		//if (!animator->m_animationControllers.empty())
		{
			...
			if (showControllersWindow)
			{
				...
				if(ImGui::BeginPopup("AniBehaviorSelect"))
				{
					...
					// C# ì• ë‹ˆë©”ì´ì…˜ ìƒíƒœ ìŠ¤í¬ë¦½íŠ¸ ëª©ë¡. ë“±ë¡ëœ íƒ€ìž… ì´ë¦„ì€ ClrHostê°€ ë‚´ì¤€ë‹¤
					// (êµ¬ C++ íŒ©í† ë¦¬ ëª©ë¡ì„ ëŒ€ì²´í•œë‹¤ â€” 9-4).
					const auto aniTypeNames = ClrHost::Get().GetAniBehaviourTypeNames();
					if (aniTypeNames.empty())
					{
						ImGui::TextDisabled(ClrHost::Get().IsReady()
							? "ë“±ë¡ëœ AniBehaviourê°€ ì—†ìŠµë‹ˆë‹¤"
							: "CLRì´ ì¤€ë¹„ë˜ì§€ ì•Šì•˜ìŠµë‹ˆë‹¤");
					}
