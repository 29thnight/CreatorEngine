	...
	m_sceneRenderer(ptr)
{
	ImGui::ContextRegister(ICON_FA_BARS_STAGGERED "  Hierarchy", [&]()
	{
		ImGui::BringWindowToDisplayBack(ImGui::GetCurrentWindow());
                m_searchFilter.Draw(ICON_FA_MAGNIFYING_GLASS " Search", ImGui::GetContentRegionAvail().x);
			...
			else if (ImGui::TreeNodeEx(SceneIcon.c_str(), ImGuiTreeNodeFlags_DefaultOpen))
			{
				for (auto& obj : scene->m_SceneObjects)
				{
					if (!obj || 0 == obj->m_index || obj->m_parentIndex > 0) continue;
                                        ...
                                        if (!PassFilterRecursive(obj)) continue;
		...
		if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("SCENE_OBJECT"))
		{
			...
			if (draggedIndex != obj->m_index) // 자기 자신에 드롭하는 것 방지
			{
				...
				const auto& oldParent = scene->GetGameObject(draggedObj->m_parentIndex);
                        ...
                        if (!PassFilterRecursive(child)) continue;

bool HierarchyWindow::PassFilterRecursive(const std::shared_ptr<GameObject>& obj)
{
    if (!obj) return false;
    if (m_searchFilter.PassFilter(obj->m_name.ToString().c_str()))
        return true;
    auto scene = SceneManagers->GetActiveScene();
    for (auto childIndex : obj->m_childrenIndices)
    {
        auto child = scene->GetGameObject(childIndex);
        if (PassFilterRecursive(child)) return true;
    }
    return false;
}
